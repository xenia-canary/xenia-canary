...
void EmulatorWindow::DisplayConfigDialog::OnDraw(ImGuiIO& io) {
  ...
  if (command_processor) {
    if (ImGui::TreeNodeEx(
      ...
      auto new_swap_post_effect =
          static_cast<gpu::CommandProcessor::SwapPostEffect>(
              new_swap_post_effect_index);
  ...
  if (presenter) {
    ...
    if (ImGui::TreeNodeEx(
      ...
      new_presenter_config.SetEffect(
          static_cast<ui::Presenter::GuestOutputPaintConfig::Effect>(
              new_effect_index));
      ...
      if (new_presenter_config.GetEffect() ==
        ...
        if (new_presenter_config.GetEffect() ==
          ...
          fsr_sharpness_reduction = std::sqrt(2.f * fsr_sharpness_reduction);
          ImGui::SliderFloat(
...
bool EmulatorWindow::Initialize() {
  ...
  {
    file_menu->AddChild(MenuItem::Create(MenuItem::Type::kString, "&Open...",
                                         "Ctrl+O", [this] { FileOpen(); }));
    file_menu->AddChild(std::move(recent_menu));
    file_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
    file_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                         "Install Content...",
                                         [this] { InstallContent(); }));
    zar_menu->AddChild(MenuItem::Create(MenuItem::Type::kString, "Create",
                                        [this] { CreateZarchive(); }));
    zar_menu->AddChild(MenuItem::Create(MenuItem::Type::kString, "Extract",
                                        [this] { ExtractZarchive(); }));
  ...
  auto profile_menu = MenuItem::Create(MenuItem::Type::kPopup, "&Profile");
  {
    profile_menu->AddChild(
        MenuItem::Create(MenuItem::Type::kString, "&Show Profile Menu", "",
  ...
  auto cpu_menu = MenuItem::Create(MenuItem::Type::kPopup, "&CPU");
  {
    cpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "&Reset Time Scalar", "Numpad *",
                                        [this] { CpuTimeScalarReset(); }));
    cpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "Time Scalar /= 2", "Numpad -",
                                        [this] { CpuTimeScalarSetHalf(); }));
    cpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "Time Scalar *= 2", "Numpad +",
                                        [this] { CpuTimeScalarSetDouble(); }));
  }
  ...
  cpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
  {
    cpu_menu->AddChild(MenuItem::Create(
        ...
        "Pause/Break", [this] { CpuBreakIntoDebugger(); }));
    cpu_menu->AddChild(MenuItem::Create(
        MenuItem::Type::kString, "&Break into Host Debugger",
        "Ctrl+Pause/Break", [this] { CpuBreakIntoHostDebugger(); }));
  }
  ...
  {
    gpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kString, "&Trace Frame",
                                        "F4", [this] { GpuTraceFrame(); }));
  }
  gpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
  {
    gpu_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "&Clear Runtime Caches", "F5",
                                        [this] { GpuClearCaches(); }));
  }
  ...
  auto display_menu = MenuItem::Create(MenuItem::Type::kPopup, "&Display");
  {
    display_menu->AddChild(
        MenuItem::Create(MenuItem::Type::kString, "&Post-processing settings",
                         "F6", [this] { ToggleDisplayConfigDialog(); }));
  }
  display_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
  {
    display_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                            "&Fullscreen", "F11",
  ...
  auto hid_menu = MenuItem::Create(MenuItem::Type::kPopup, "&HID");
  {
    ...
    hid_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "&Display controller hotkeys", "",
  ...
  {
    xmp_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                        "&Show XMP Menu", "",
  ...
  {
    help_menu->AddChild(MenuItem::Create(MenuItem::Type::kString, "FA&Q...",
                                         "F1", [this] { ShowFAQ(); }));
    help_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
    help_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                         "Game &compatibility...",
                                         [this] { ShowCompatibility(); }));
    help_menu->AddChild(MenuItem::Create(MenuItem::Type::kSeparator));
    help_menu->AddChild(MenuItem::Create(MenuItem::Type::kString,
                                         "Build commit on GitHub...", "F2",
                                         [this] { ShowBuildCommit(); }));
...
void EmulatorWindow::ApplyDisplayConfigForCvars() const {
  gpu::GraphicsSystem* graphics_system = emulator_->graphics_system();
  if (!graphics_system) {
    return;
  }
...
void EmulatorWindow::InstallContent() {
  ...
  for (const auto& path : paths) {
    content_installation_status->emplace_back(path);
  }
...
void EmulatorWindow::ExtractZarchive() const {
...
void EmulatorWindow::CreateZarchive() const {
...
void EmulatorWindow::ShowContentDirectory() const {
...
void EmulatorWindow::CpuTimeScalarReset() const {
  Clock::set_guest_time_scalar(1.0);
  UpdateTitle();
}

void EmulatorWindow::CpuTimeScalarSetHalf() const {
  Clock::set_guest_time_scalar(Clock::guest_time_scalar() / 2.0);
  UpdateTitle();
}

void EmulatorWindow::CpuTimeScalarSetDouble() const {
  Clock::set_guest_time_scalar(Clock::guest_time_scalar() * 2.0);
  UpdateTitle();
}

void EmulatorWindow::CpuBreakIntoDebugger() const {
  if (!cvars::debug) {
...
void EmulatorWindow::GpuTraceFrame() const {
  emulator()->graphics_system()->RequestFrameTrace();
}

void EmulatorWindow::GpuClearCaches() const {
  emulator()->graphics_system()->ClearCaches();
}

void EmulatorWindow::SetFullscreen(bool fullscreen) const {
...
void EmulatorWindow::ToggleXMPConfigDialog() {
  if (!xmp_config_dialog_) {
    xmp_config_dialog_ =
        std::make_unique<XMPConfigDialog>(imgui_drawer_.get(), *this);
  } else {
    xmp_config_dialog_.reset();
  }
}

void EmulatorWindow::ToggleControllerVibration() const {
...
void EmulatorWindow::ShowCompatibility() const {
...
void EmulatorWindow::UpdateTitle() const {
  xe::StringBuffer sb;
...
EmulatorWindow::ControllerHotKey EmulatorWindow::ProcessControllerHotkey(
  ...
  if ((button_combination.function == ButtonFunctions::IncTitleSelect ||
      ...
      !recently_launched_titles_.empty()) {
    selected_title_index =
...
void EmulatorWindow::DisplayHotKeysConfig() {
  ...
  for (const auto& val : controller_hotkey_map | std::views::values) {
...
void EmulatorWindow::FillRecentlyLaunchedTitlesMenu(
  ...
  for (int i = 0; i < recently_launched_titles_.size(); ++i) {
    ...
    recent_menu->AddChild(MenuItem::Create(
        ...
        [this, path = entry.path_to_file] { return RunTitle(path); }));
  }
}
...
void EmulatorWindow::AddRecentlyLaunchedTitle(
    const std::filesystem::path& path_to_file, const std::string& title_name) {
