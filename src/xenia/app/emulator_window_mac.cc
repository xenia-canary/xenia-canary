...
DEFINE_bool(disable_doubleclick_fullscreen, false,
            ...
            "General");
DEFINE_bool(auto_create_default_profile, true,
            "Automatically create and log in a default profile if none exist. "
            "This can help avoid title stalls waiting for sign-in UI.",
            "General");
...
void EmulatorWindow::SetupGraphicsSystemPresenterPainting() {
  ...
  // Force an initial paint now that the presenter is attached.
  window_->RequestPaint();
  window_->RequestPresenterUIPaintFromUIThread();

  app_context_.CallInUIThreadDeferredTagged(
      "SetupGraphicsSystemPresenterDrawer",
      [this, presenter]() {
        if (!window_ || GetGraphicsSystemPresenter() != presenter) {
          return;
        }
        ...
        immediate_drawer_ = graphics_system->provider()->CreateImmediateDrawer();
        if (!immediate_drawer_) {
          return;
        }
...
void EmulatorWindow::OnEmulatorInitialized() {
  auto profile_manager = emulator_->kernel_state()
                             ->xam_state()
                             ->profile_manager();
  const uint32_t profile_count =
      profile_manager ? profile_manager->GetAccountCount() : 0;
  XELOGI("ProfileManager: {} profiles available", profile_count);
  if (!profile_count) {
    if (cvars::auto_create_default_profile && profile_manager) {
      XELOGW("DEBUG: No profiles found - auto-creating default profile");
      bool created =
          profile_manager->CreateProfile("Player", true, false);
      XELOGI("ProfileManager: auto-create default profile {}",
...
xe::X_STATUS EmulatorWindow::RunTitle(
  ...
  // CRITICAL: Log at the ABSOLUTE START before anything else
  std::ostringstream oss;
  oss << std::this_thread::get_id();
  fprintf(stderr,
          "DEBUG: EmulatorWindow::RunTitle ENTRY - path: %s (thread %s, "
          "IsInUIThread=%s)\n",
          path_to_file.c_str(), oss.str().c_str(),
          app_context().IsInUIThread() ? "YES" : "NO");
  fflush(stderr);
  XELOGI("DEBUG: EmulatorWindow::RunTitle ENTRY - path: '{}'",
         path_to_file.string());
  ...
  if (path_to_file.empty() || !titleExists) {
    printf("DEBUG: File validation failed\n");
    std::string log_msg =
  ...
  printf("DEBUG: About to check if title is open\n");
  if (emulator_->is_title_open()) {
    printf("DEBUG: Title is already open - attempting termination\n");
    // Terminate the current title and start a new title.
    XELOGI(
        "DEBUG: Title already open, attempting to terminate before launching "
        "new title");

    X_STATUS terminate_result = emulator_->TerminateTitle();
    XELOGI("DEBUG: TerminateTitle returned: {:08X}", terminate_result);

    if (terminate_result == X_STATUS_SUCCESS) {
      XELOGI(
          "DEBUG: Title terminated successfully, proceeding with new title "
          "launch");
      // Clear any existing dialogs before launching new title
      ClearDialogs();
      return RunTitle(path_to_file);
    } else {
      XELOGE("DEBUG: Failed to terminate existing title: {:08X}",
             terminate_result);
      xe::ui::ImGuiDialog::ShowMessageBox(
          imgui_drawer_.get(), "Title Switch Failed!",
          "Failed to terminate the currently running title.\n\nClose Xenia and restart to launch a different game.");
      return terminate_result;
    }
  }
  printf("DEBUG: Title not open or termination successful - proceeding with launch\n");
  ...
  printf("DEBUG: File extension validation passed\n");

  XELOGI("EmulatorWindow::RunTitle calling LaunchPath with: {}", abs_path.string());
  printf("DEBUG: About to call emulator_->LaunchPath\n");
  auto result = emulator_->LaunchPath(abs_path);
  printf("DEBUG: LaunchPath returned result\n");
  XELOGI("EmulatorWindow::RunTitle LaunchPath returned: {:08X}", result);
  ...
  printf("DEBUG: LaunchPath returned result: %08X\n", result);
  if (result) {
    printf("DEBUG: Launch failed - showing error dialog\n");
    ...
    printf("DEBUG: Launch succeeded - adding to recent titles\n");
  }

  printf("DEBUG: RunTitle returning result: %08X\n", result);
  return result;
}
