  ...
  const std::unique_ptr<vfs::Device> Emulator::CreateVfsDeviceBasedOnPath(
    ...
    if (extension == ".xex" || extension == ".elf" || extension == ".exe") {
      ...
      return std::make_unique<vfs::HostPathDevice>(
          ...
          !cvars::allow_game_relative_writes);  // chrispy: added this for
                                                // easily generating test data
                                                // to compare with orig
    } else {
...
void Emulator::Pause() {
  ...
  bool Emulator::SaveToFile(const std::filesystem::path& path) {
    ...
    auto map =
        MappedMemory::Open(path, MappedMemory::Mode::kReadWrite, 0, 2_GiB);
    if (!map) {
      return false;
    }
  ...
  bool Emulator::TitleRequested() {
    auto xam =
        kernel_state()->GetKernelModule<kernel::xam::XamModule>("xam.xex");
  ...
  void Emulator::RemoveGameConfigLoadCallback(GameConfigLoadCallback *
                                              callback) {
    assert_not_null(callback);
    // Game config load callbacks handling is entirely in the UI thread.
    assert_true(!display_window_ ||
