...
DEFINE_uint32(
    gpu_bootstrap_wait_object_watch, 0,
    "Debug: Guest dispatcher object pointer to watch in wait/signal APIs. "
    "0 = auto-arm from first main-thread event wait.",
    "Threading");
DEFINE_bool(
  gpu_bootstrap_wait_timeout_recover, false,
  "Debug: If watched/bootstrap wait objects keep timing out, issue a "
  "throttled event set nudge to break potential wait-loop deadlocks.",
  "Threading");
DEFINE_uint32(
  gpu_bootstrap_wait_timeout_recover_interval_ms, 1000,
  "Minimum interval between timeout recovery nudges for watched/bootstrap "
  "wait objects.",
  "Threading");
...
std::atomic<uint32_t> gpu_bootstrap_auto_wait_object_watch{0};
constexpr uint32_t kGpuBootstrapKnownWaitObject = 0x30028030u;

static uint32_t GetWaitWatchObjectPtrImpl() {
  uint32_t watch_object_ptr = cvars::gpu_bootstrap_wait_object_watch;
  if (watch_object_ptr != 0) {
    return watch_object_ptr;
  }
  return gpu_bootstrap_auto_wait_object_watch.load(std::memory_order_relaxed);
}

bool IsGpuBootstrapGuestThreadCandidate(XThread* thread) {
  ...
  uint32_t start = thread->start_address();
  switch (start) {
    case 0x82582A98u:
    case 0x82582AC8u:
    case 0x8258A410u:
    ...
    case 0x825828B0u:
      return true;
    default:
      return thread->thread_id() == 6;
  }
}

bool ShouldLogGpuBootstrapWait(XThread* thread, uint32_t wait_object_guest_ptr,
                               int64_t timeout_ms) {
  if (IsGpuBootstrapGuestThreadCandidate(thread)) {
    return true;
  }
  uint32_t watch_object_ptr = GetWaitWatchObjectPtr();
  if (watch_object_ptr && wait_object_guest_ptr == watch_object_ptr) {
    return true;
  }
  if (wait_object_guest_ptr == kGpuBootstrapKnownWaitObject) {
    return true;
  }
  return timeout_ms != 0;
}

uint32_t TimeoutTicksToMsForDebug(int64_t timeout_ticks) {
  if (timeout_ticks > 0) {
    // NetDll_WSAWaitForMultipleEvents provides timeout in form of MS.
    return static_cast<uint32_t>(timeout_ticks);
  } else if (timeout_ticks < 0) {
    // Relative time.
    return static_cast<uint32_t>(-timeout_ticks / 10000);  // Ticks -> MS
  } else {
    return 0;
  }
}

const char* GetXObjectTypeName(XObject::Type type) {
  switch (type) {
...
void QueryEventStateForDebug(XObject* object, bool* out_valid, uint32_t* out_type,
                             uint32_t* out_state) {
  bool valid = false;
  uint32_t type = 0;
  uint32_t state = 0;
...
void MaybeArmAutoWaitWatchObject(uint32_t object_guest_ptr, XObject* object,
                                 const char* source) {
  if (cvars::gpu_bootstrap_wait_object_watch != 0) {
    return;
  }
  if (!object_guest_ptr || !object || object->type() != XObject::Type::Event) {
    return;
  }
  uint32_t expected = 0;
  if (gpu_bootstrap_auto_wait_object_watch.compare_exchange_strong(
          expected, object_guest_ptr, std::memory_order_relaxed)) {
    XELOGW(
        "RING BUFFER: wait watch armed obj_ptr={:08X} source={} type={} "
        "thread_id={} pc={:08X} lr={:08X}",
        object_guest_ptr, source ? source : "<unknown>",
        GetXObjectTypeName(object->type()), XThread::GetCurrentThreadId(),
        GetCurrentThreadPcForDebug(), GetCurrentThreadLrForDebug());
  }
}

void LogWaitObjectDetail(const char* wait_api, uint32_t index, XObject* object,
                         uint32_t object_guest_ptr, uint32_t waited_handle,
                         bool waited_handle_valid, uint32_t watch_object_ptr) {
  bool event_state_valid = false;
  uint32_t event_type = 0;
  uint32_t event_state = 0;
  QueryEventStateForDebug(object, &event_state_valid, &event_type, &event_state);
  bool watch_match = watch_object_ptr != 0 && object_guest_ptr == watch_object_ptr;
  const char* object_name = "<unnamed>";
  if (object && !object->name().empty()) {
    object_name = object->name().c_str();
  }
  XELOGI(
      "RING BUFFER: {} wait_obj[{}] ptr={:08X} obj_handle={:08X} "
      "wait_handle_valid={} wait_handle={:08X} type={}({}) "
      "event_state_valid={} event_type={:08X} event_state={:08X} "
      "watch_obj={:08X} watch_match={} name='{}'",
      wait_api ? wait_api : "Wait", index, object_guest_ptr,
      object ? object->handle() : 0, waited_handle_valid ? "YES" : "NO",
      waited_handle,
      object ? GetXObjectTypeName(object->type()) : "<null>",
      object ? static_cast<uint32_t>(object->type()) : 0, event_state_valid ? "YES" : "NO",
      event_type, event_state, watch_object_ptr, watch_match ? "YES" : "NO",
      object_name);
}

void LogWatchedEventStateChange(const char* api, const char* operation,
                                uint32_t object_guest_ptr, uint32_t handle,
                                XEvent* event, bool previous_state_valid,
                                uint32_t previous_state, uint32_t increment,
                                uint32_t wait) {
  uint32_t watch_object_ptr = GetWaitWatchObjectPtr();
  if (!watch_object_ptr || object_guest_ptr != watch_object_ptr) {
    return;
  }
  bool event_state_valid = false;
  uint32_t event_type = 0;
  uint32_t event_state = 0;
  QueryEventStateForDebug(event, &event_state_valid, &event_type, &event_state);
  XELOGW(
      "RING BUFFER: {} {} WATCH_HIT obj_ptr={:08X} handle={:08X} "
      "watch_obj={:08X} prev_state_valid={} prev_state={:08X} "
      "event_state_valid={} event_type={:08X} event_state={:08X} "
      "increment={} wait={} thread_id={} pc={:08X} lr={:08X}",
      api ? api : "EventOp", operation ? operation : "<unknown>",
      object_guest_ptr, handle, watch_object_ptr,
      previous_state_valid ? "YES" : "NO", previous_state,
      event_state_valid ? "YES" : "NO", event_type, event_state, increment,
      wait, XThread::GetCurrentThreadId(), GetCurrentThreadPcForDebug(),
      GetCurrentThreadLrForDebug());
}
}  // namespace

uint32_t GetWaitWatchObjectPtr() {
  return GetWaitWatchObjectPtrImpl();
}
...
uint32_t ExCreateThread(xe::be<uint32_t>* handle_ptr, uint32_t stack_size,
  ...
  auto* caller_thread = XThread::GetCurrentThread();
  bool caller_is_main = caller_thread && caller_thread->main_thread();
  bool caller_is_bootstrap = IsGpuBootstrapGuestThreadCandidate(caller_thread);
  if (caller_is_main || caller_is_bootstrap) {
    auto* processor = kernel_state_var ? kernel_state_var->processor() : nullptr;
    const char* startup_module_name = "<unknown>";
    const char* startup_function_name = "<unknown>";
    uint32_t startup_function_address = 0;
    if (processor && xapi_thread_startup) {
      auto startup_module = processor->LookupModule(xapi_thread_startup);
      if (startup_module) {
        startup_module_name = startup_module->name().c_str();
      }
      auto startup_function = processor->LookupFunction(xapi_thread_startup);
      if (startup_function) {
        startup_function_name = startup_function->name().c_str();
        startup_function_address = startup_function->address();
      }
    }
    ...
    XELOGI(
        "RING BUFFER: ExCreateThread caller_tid={} caller_main={} "
        "caller_bootstrap={} xapi_thread_startup={:08X} "
        "xapi_module={} xapi_fn={}({:08X}) start_address={:08X} "
        "start_module={} start_fn={}({:08X}) start_context={:08X} "
        "creation_flags={:08X}",
        caller_thread ? caller_thread->thread_id() : 0,
        caller_is_main ? "YES" : "NO",
        caller_is_bootstrap ? "YES" : "NO", xapi_thread_startup,
        startup_module_name, startup_function_name, startup_function_address,
        start_address, start_module_name, start_function_name,
        start_function_address, start_context, creation_flags);
    if (!xapi_thread_startup) {
      XELOGW(
          "RING BUFFER: ExCreateThread received xapi_thread_startup=0 "
          "(thread will dispatch raw start_address).");
    }
  }
  ...
  // Validate that threads created via ExCreateThread are GUEST threads
  // (This is called from game code, so all threads should be guest threads)
  if (!thread->is_guest_thread()) {
    XELOGE("DEBUG: FATAL: ExCreateThread created a HOST thread!");
    XELOGE("Threads created by game code MUST be GUEST threads.");
    XELOGE("start_address={:08X}", start_address);
    std::abort();
  }
...
uint32_t ExTerminateThread(uint32_t exit_code) {
  ...
  // Enhanced debugging - focus on GPU Commands thread termination
  if (thread->handle() == 0x01000010) {
    XELOGI("DEBUG: ExTerminateThread called on CRITICAL handle 01000010:");
    XELOGI("Thread name: '{}'", thread->name());
    XELOGI("Thread ID: {}", thread->thread_id());
    XELOGI("Exit code: {}", exit_code);
    XELOGI("Thread state: {}", thread->is_running() ? "RUNNING" : "STOPPED");
    XELOGI("DEBUG: CRITICAL: GPU Commands thread being terminated!");
    XELOGI("This likely explains the emulator hang after this point");

    if (auto current_thread = xe::kernel::XThread::GetCurrentThread()) {
      XELOGI("Termination called from thread: '{}'", current_thread->name());
    }
  }
...
uint32_t KeDelayExecutionThread(uint32_t processor_mode, uint32_t alertable,
  ...
  XELOGI("DEBUG: BREADCRUMB: KeDelayExecutionThread returning result={:08X}", result);
  return result;
}
...
void KeInitializeEvent_entry(pointer_t<X_KEVENT> event_ptr, dword_t event_type,
  ...
  XELOGI("DEBUG: KeInitializeEvent: ptr={:08X} type={} state={} handle={:08X}",
         event_ptr.guest_address(), (uint32_t)event_type,
         (uint32_t)initial_state, ev->handle());
}
...
uint32_t xeKeSetEvent(X_KEVENT* event_ptr, uint32_t increment, uint32_t wait) {
  ...
  uint32_t was_signalled = ev->Set(increment, !!wait);
  uint32_t object_guest_ptr = ev->guest_object();
  XELOGI("DEBUG: xeKeSetEvent: ptr={:08X} handle={:08X} was_signalled={} wait={}",
         kernel_memory()->HostToGuestVirtual(event_ptr), ev->handle(),
         was_signalled, wait);
  XELOGI("DEBUG: BREADCRUMB: xeKeSetEvent handle={:08X}", ev->handle());
  LogWatchedEventStateChange("KeSetEvent", "set", object_guest_ptr, ev->handle(),
                             ev.get(), true, was_signalled, increment, wait);
...
dword_result_t KePulseEvent_entry(pointer_t<X_KEVENT> event_ptr,
  ...
  uint32_t was_signalled = ev->Pulse(increment, !!wait);
  LogWatchedEventStateChange("KePulseEvent", "pulse", ev->guest_object(),
                             ev->handle(), ev.get(), true, was_signalled,
                             increment, wait);
  return was_signalled;
}
...
dword_result_t KeResetEvent_entry(pointer_t<X_KEVENT> event_ptr) {
  ...
  uint32_t was_signalled = ev->Reset();
  XELOGI("DEBUG: KeResetEvent: ptr={:08X} handle={:08X} was_signalled={}",
         event_ptr.guest_address(), ev->handle(), was_signalled);
  LogWatchedEventStateChange("KeResetEvent", "reset", ev->guest_object(),
                             ev->handle(), ev.get(), true, was_signalled, 0,
                             0);
  return was_signalled;
}
...
dword_result_t NtCreateEvent_entry(
  ...
  if (handle_ptr) {
    *handle_ptr = ev->handle();
    XELOGI("DEBUG: BREADCRUMB: NtCreateEvent created handle={:08X}", ev->handle());
  }
  return X_STATUS_SUCCESS;
}
...
uint32_t xeNtSetEvent(uint32_t handle, xe::be<uint32_t>* previous_state_ptr) {
  ...
  if (ev) {
    ...
    LogWatchedEventStateChange("NtSetEvent", "set", ev->guest_object(),
                               handle, ev.get(), true,
                               static_cast<uint32_t>(was_signalled), 0, 0);
  } else {
    result = X_STATUS_INVALID_HANDLE;
  }
...
dword_result_t NtPulseEvent_entry(dword_t handle,
  ...
  if (ev) {
    ...
    LogWatchedEventStateChange("NtPulseEvent", "pulse", ev->guest_object(),
                               handle, ev.get(), true,
                               static_cast<uint32_t>(was_signalled), 0, 0);
  } else {
    result = X_STATUS_INVALID_HANDLE;
...
uint32_t xeNtClearEvent(uint32_t handle) {
  ...
  if (ev) {
    uint32_t was_signalled = ev->Reset();
    LogWatchedEventStateChange("NtClearEvent", "reset", ev->guest_object(),
                               handle, ev.get(), true, was_signalled, 0, 0);
  } else {
    result = X_STATUS_INVALID_HANDLE;
  }
...
uint32_t xeKeWaitForSingleObject(void* object_ptr, uint32_t wait_reason,
  ...
  // Debug: log waits to catch stalls.
  uint64_t timeout_ticks = timeout_ptr ? *timeout_ptr : 0;
  int64_t timeout_ms = -1;
  if (timeout_ptr) {
    timeout_ms = static_cast<int64_t>(
        TimeoutTicksToMsForDebug(static_cast<int64_t>(timeout_ticks)));
  }

  auto* current_thread = XThread::GetCurrentThread();
  bool is_main = current_thread && current_thread->main_thread();
  bool is_bootstrap_thread = IsGpuBootstrapGuestThreadCandidate(current_thread);
  uint32_t wait_object_guest_ptr = object->guest_object();
  uint32_t pc = GetCurrentThreadPcForDebug();
  uint32_t lr = GetCurrentThreadLrForDebug();
  bool should_log_wait =
      ShouldLogGpuBootstrapWait(current_thread, wait_object_guest_ptr,
                                timeout_ms);
  if (is_main || is_bootstrap_thread ||
      wait_object_guest_ptr == kGpuBootstrapKnownWaitObject) {
    MaybeArmAutoWaitWatchObject(wait_object_guest_ptr, object.get(),
                                "KeWaitForSingleObject");
  }
  uint32_t watch_object_ptr = GetWaitWatchObjectPtr();

  // Log waits for bootstrap focus threads/objects and long/periodic waits.
  if (should_log_wait) {
    static std::atomic<uint64_t> wait_count{0};
    uint64_t wait_id = wait_count.fetch_add(1) + 1;
    if (is_main || is_bootstrap_thread ||
        wait_object_guest_ptr == kGpuBootstrapKnownWaitObject ||
        (wait_id % 256) == 0) {
      XELOGI(
          "RING BUFFER: KeWaitForSingleObject tid={} main={} obj_ptr={:08X} "
          "bootstrap_thread={} guest_obj={:08X} type={} handle={:08X} "
          "wait_reason={} timeout_ms={} pc={:08X} lr={:08X} watch_obj={:08X}",
          current_thread ? current_thread->thread_id() : 0,
          is_main ? "YES" : "NO",
          static_cast<uint32_t>(reinterpret_cast<uintptr_t>(object_ptr)),
          is_bootstrap_thread ? "YES" : "NO",
          wait_object_guest_ptr, static_cast<uint32_t>(object->type()),
          object->handle(), wait_reason, timeout_ms, pc, lr, watch_object_ptr);
      if (is_main || is_bootstrap_thread ||
          wait_object_guest_ptr == kGpuBootstrapKnownWaitObject) {
        LogWaitObjectDetail("KeWaitForSingleObject", 0, object.get(),
                            wait_object_guest_ptr, 0, false, watch_object_ptr);
      }
    }
  }
  ...
  if (should_log_wait) {
    XELOGI("RING BUFFER: KeWaitForSingleObject result={:08X}", result);
  }
  if (cvars::gpu_bootstrap_wait_timeout_recover &&
      result == X_STATUS_TIMEOUT && object->type() == XObject::Type::Event) {
    bool bootstrap_wait_target =
        wait_object_guest_ptr == kGpuBootstrapKnownWaitObject ||
        (watch_object_ptr != 0 && wait_object_guest_ptr == watch_object_ptr);
    if (bootstrap_wait_target) {
      static std::atomic<uint64_t> timeout_recover_last_ms{0};
      uint64_t now_ms = Clock::QueryHostUptimeMillis();
      uint64_t interval_ms = static_cast<uint64_t>(
          cvars::gpu_bootstrap_wait_timeout_recover_interval_ms);
      uint64_t last_ms =
          timeout_recover_last_ms.load(std::memory_order_relaxed);
      bool recover_due =
          last_ms == 0 || interval_ms == 0 || now_ms < last_ms ||
          now_ms - last_ms >= interval_ms;
      if (recover_due) {
        timeout_recover_last_ms.store(now_ms, std::memory_order_relaxed);
        auto* event = static_cast<XEvent*>(object.get());
        bool before_valid = false;
        uint32_t before_type = 0;
        uint32_t before_state = 0;
        QueryEventStateForDebug(event, &before_valid, &before_type,
                                &before_state);
        event->Set(0, false);
        bool after_valid = false;
        uint32_t after_type = 0;
        uint32_t after_state = 0;
        QueryEventStateForDebug(event, &after_valid, &after_type,
                                &after_state);
        XELOGW(
            "RING BUFFER: wait timeout recovery nudge "
            "api=KeWaitForSingleObject tid={} obj_ptr={:08X} "
            "obj_handle={:08X} watch_obj={:08X} before_state_valid={} "
            "before_state={:08X} after_state_valid={} after_state={:08X}",
            current_thread ? current_thread->thread_id() : 0,
            wait_object_guest_ptr, object->handle(), watch_object_ptr,
            before_valid ? "YES" : "NO", before_state,
            after_valid ? "YES" : "NO", after_state);
      }
    }
  }
  if (alertable) {
  ...
  if (object) {
    ...
    if (should_log_wait) {
      ...
      if (is_main || is_bootstrap_thread ||
          ...
          (wait_id % 256) == 0) {
        XELOGI(
            "RING BUFFER: NtWaitForSingleObjectEx tid={} main={} "
            "bootstrap_thread={} handle={:08X} guest_obj={:08X} type={} "
            "wait_mode={} alertable={} "
            "timeout_ms={} pc={:08X} lr={:08X} watch_obj={:08X}",
            current_thread ? current_thread->thread_id() : 0,
            is_main ? "YES" : "NO",
            is_bootstrap_thread ? "YES" : "NO",
            object_handle,
            wait_object_guest_ptr, static_cast<uint32_t>(object->type()),
            wait_mode, alertable, timeout_ms, pc, lr, watch_object_ptr);
        if (is_main || is_bootstrap_thread ||
            wait_object_guest_ptr == kGpuBootstrapKnownWaitObject) {
          LogWaitObjectDetail("NtWaitForSingleObjectEx", 0, object.get(),
                              wait_object_guest_ptr, object_handle, true,
                              watch_object_ptr);
        }
      }
    }
    ...
    if (should_log_wait) {
      XELOGI("RING BUFFER: NtWaitForSingleObjectEx result={:08X}", result);
    }
    if (cvars::gpu_bootstrap_wait_timeout_recover &&
        result == X_STATUS_TIMEOUT && object->type() == XObject::Type::Event) {
      bool bootstrap_wait_target =
      ...
      if (bootstrap_wait_target) {
        ...
        if (recover_due) {
          ...
          XELOGW(
              "RING BUFFER: wait timeout recovery nudge "
              "api=NtWaitForSingleObjectEx tid={} obj_ptr={:08X} "
              "handle={:08X} watch_obj={:08X} before_state_valid={} "
              "before_state={:08X} after_state_valid={} after_state={:08X}",
              current_thread ? current_thread->thread_id() : 0,
              wait_object_guest_ptr, object_handle, watch_object_ptr,
              before_valid ? "YES" : "NO", before_state,
              after_valid ? "YES" : "NO", after_state);
        }
      }
...
dword_result_t KeWaitForMultipleObjects_entry(
  ...
  bool is_main = current_thread && current_thread->main_thread();
  bool is_bootstrap_thread = IsGpuBootstrapGuestThreadCandidate(current_thread);
  uint64_t timeout_ticks = timeout_ptr ? static_cast<uint64_t>(*timeout_ptr) : 0;
  int64_t timeout_ms = -1;
  if (timeout_ptr) {
    timeout_ms = static_cast<int64_t>(
        TimeoutTicksToMsForDebug(static_cast<int64_t>(timeout_ticks)));
  ...
  if (is_main || is_bootstrap_thread) {
    for (uint32_t n = 0; n < count; ++n) {
      MaybeArmAutoWaitWatchObject(objects[n]->guest_object(), objects[n].get(),
                                  "KeWaitForMultipleObjects");
    }
    watch_object_ptr = GetWaitWatchObjectPtr();
  }
  if (is_main || is_bootstrap_thread) {
    XELOGI(
        "RING BUFFER: KeWaitForMultipleObjects tid={} main={} bootstrap_thread={} count={} "
        "wait_type={} wait_reason={} alertable={} timeout_ms={} "
        "pc={:08X} lr={:08X} watch_obj={:08X}",
        current_thread->thread_id(), is_main ? "YES" : "NO",
        is_bootstrap_thread ? "YES" : "NO", static_cast<uint32_t>(count),
        static_cast<uint32_t>(wait_type), static_cast<uint32_t>(wait_reason),
        static_cast<uint32_t>(alertable), timeout_ms, pc, lr,
        watch_object_ptr);
    for (uint32_t n = 0; n < count; ++n) {
      LogWaitObjectDetail("KeWaitForMultipleObjects", n, objects[n].get(),
                          objects[n]->guest_object(), objects_ptr[n], false,
                          watch_object_ptr);
    }
  }
  uint64_t timeout = timeout_ptr ? static_cast<uint64_t>(*timeout_ptr) : 0u;
  ...
  if (is_main || is_bootstrap_thread) {
    XELOGI("RING BUFFER: KeWaitForMultipleObjects result={:08X}", result);
  }
  if (alertable) {
    if (result == X_STATUS_USER_APC) {
      xeProcessUserApcs(nullptr);
...
uint32_t xeNtWaitForMultipleObjectsEx(uint32_t count, xe::be<uint32_t>* handles,
  ...
  */
  ...
  if (is_main || is_bootstrap_thread) {
    XELOGI(
        "RING BUFFER: NtWaitForMultipleObjectsEx tid={} main={} bootstrap_thread={} count={} "
        "wait_type={} wait_mode={} alertable={} timeout_ms={} "
        "pc={:08X} lr={:08X} watch_obj={:08X}",
        current_thread->thread_id(), is_main ? "YES" : "NO",
        is_bootstrap_thread ? "YES" : "NO", count, wait_type, wait_mode,
        alertable, timeout_ms, pc, lr, watch_object_ptr);
    for (uint32_t n = 0; n < count; ++n) {
      LogWaitObjectDetail("NtWaitForMultipleObjectsEx", n, objects[n].get(),
                          objects[n]->guest_object(), handles[n], true,
                          watch_object_ptr);
    }
  }
  ...
  if (is_main || is_bootstrap_thread) {
    XELOGI("RING BUFFER: NtWaitForMultipleObjectsEx result={:08X}", result);
  }
  if (alertable) {
    if (result == X_STATUS_USER_APC) {
      xeProcessUserApcs(nullptr);
...
dword_result_t NtSignalAndWaitForSingleObjectEx_entry(dword_t signal_handle,
  ...
  uint32_t watch_object_ptr = GetWaitWatchObjectPtr();
  uint32_t signal_guest_ptr = signal_object ? signal_object->guest_object() : 0;
  uint32_t wait_guest_ptr = wait_object ? wait_object->guest_object() : 0;
  bool watch_hit_signal =
      watch_object_ptr != 0 && signal_guest_ptr == watch_object_ptr;
  bool watch_hit_wait =
      watch_object_ptr != 0 && wait_guest_ptr == watch_object_ptr;
  if (watch_hit_signal || watch_hit_wait) {
    bool signal_event_state_valid = false;
    uint32_t signal_event_type = 0;
    uint32_t signal_event_state = 0;
    QueryEventStateForDebug(signal_object.get(), &signal_event_state_valid,
    ...
    QueryEventStateForDebug(wait_object.get(), &wait_event_state_valid,
                            &wait_event_type, &wait_event_state);
    XELOGW(
        "RING BUFFER: NtSignalAndWaitForSingleObjectEx WATCH_HIT "
        "watch_obj={:08X} signal_handle={:08X} signal_obj={:08X} "
        "signal_type={}({}) signal_event_state_valid={} signal_event_type={:08X} "
        "signal_event_state={:08X} wait_handle={:08X} wait_obj={:08X} "
        "wait_type={}({}) wait_event_state_valid={} wait_event_type={:08X} "
        "wait_event_state={:08X} alertable={} timeout_ptr={} thread_id={} "
        "pc={:08X} lr={:08X}",
        watch_object_ptr, static_cast<uint32_t>(signal_handle), signal_guest_ptr,
        signal_object ? GetXObjectTypeName(signal_object->type()) : "<null>",
        signal_object ? static_cast<uint32_t>(signal_object->type()) : 0,
        signal_event_state_valid ? "YES" : "NO", signal_event_type,
        signal_event_state, static_cast<uint32_t>(wait_handle), wait_guest_ptr,
        wait_object ? GetXObjectTypeName(wait_object->type()) : "<null>",
        wait_object ? static_cast<uint32_t>(wait_object->type()) : 0,
        wait_event_state_valid ? "YES" : "NO", wait_event_type,
        wait_event_state, static_cast<uint32_t>(alertable),
        timeout_ptr ? "NON_NULL" : "NULL",
        XThread::GetCurrentThreadId(), GetCurrentThreadPcForDebug(),
        GetCurrentThreadLrForDebug());
  }
  if (signal_object && wait_object) {
    ...
    if (watch_hit_signal || watch_hit_wait) {
      ...
      QueryEventStateForDebug(wait_object.get(), &wait_event_state_valid,
                              &wait_event_type, &wait_event_state);
      XELOGW(
          "RING BUFFER: NtSignalAndWaitForSingleObjectEx WATCH_HIT result={:08X} "
          ...
          result, signal_event_state_valid ? "YES" : "NO", signal_event_type,
...
uint32_t xeKeKfAcquireSpinLock(PPCContext* ctx, X_KSPINLOCK* lock,
                               bool change_irql) {
  printf(
      "DEBUG: BREADCRUMB: xeKeKfAcquireSpinLock ENTRY for lock %p "
      "(change_irql=%d)\n",
      reinterpret_cast<void*>(lock), static_cast<int>(change_irql));
  auto old_irql = change_irql ? xeKfRaiseIrql(ctx, 2) : 0;

  if (!lock) {
    printf("DEBUG: BREADCRUMB: xeKeKfAcquireSpinLock ERROR - lock is NULL\n");
    return old_irql;
  }
  ...
  uint32_t lock_guest_address =
      ctx->processor->memory()->HostToGuestVirtual(lock);
  while (!ctx->processor->GuestAtomicCAS32(
      ctx, 0, static_cast<uint32_t>(ctx->r[13]), lock_guest_address)) {
    // Spin!
    if (++spin_count % 1000000 == 0) {
      printf("DEBUG: BREADCRUMB: xeKeKfAcquireSpinLock STILL SPINNING on lock %p\n",
             reinterpret_cast<void*>(lock));
    }
    xe::threading::MaybeYield();
  }
  printf("DEBUG: BREADCRUMB: xeKeKfAcquireSpinLock SUCCESS for lock %p\n",
         reinterpret_cast<void*>(lock));
