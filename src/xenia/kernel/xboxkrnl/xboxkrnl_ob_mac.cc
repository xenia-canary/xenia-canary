...
dword_result_t ObReferenceObjectByHandle_entry(dword_t handle,
                                                ...
                                                lpdword_t out_object_ptr) {
  // handle 01000010 is the GPU Commands thread???
  // this was an area in which I had to explicitly deal with this
  if (handle == 0x01000010) {
    XELOGI("DEBUG: ObReferenceObjectByHandle called on CRITICAL handle 01000010:");
    XELOGI("Requested object type: {:#010X}", object_type_ptr.value());
    XELOGI("Current thread ID: {}", std::this_thread::get_id());
    
    if (auto current_thread = xe::kernel::XThread::GetCurrentThread()) {
      XELOGI("   Current thread: '{}'", current_thread->name());
    }
    XELOGI("   Object table lookup starting...");
  }
  ...
  *out_object_ptr = 0;
  ...
  if (handle == 0x01000010) {
    if (object) {
      XELOGI("Object found: {}", typeid(*object).name());
      XELOGI("Object pointer: {}", static_cast<void*>(object.get()));
    } else {
      XELOGI("OBJECT NOT FOUND - Handle 01000010 missing!");
      XELOGI("This indicates GPU Commands thread was destroyed or never created");
    }
  }
  
  if (!object) {
    if (handle == 0x01000010) {
      XELOGI("Returning X_STATUS_INVALID_HANDLE for handle 01000010");
    }
...
void xeObDereferenceObject(PPCContext* context, uint32_t native_ptr) {
  // Enhanced debugging - focus on GPU Commands thread object dereferencing
  // We need to extract handle from the object to check if it's 01000010
  if (native_ptr != 0xDEADF00D && native_ptr != 0) {
    auto temp_object = XObject::GetNativeObject<XObject>(
        kernel_state(), kernel_memory()->TranslateVirtual(native_ptr));
    if (temp_object && temp_object->handle() == 0x01000010) {
      XELOGI("DEBUG: ObDereferenceObject called on CRITICAL handle 01000010:");
      XELOGI("Native ptr: {:08X}", native_ptr);
      XELOGI("Object: {}", typeid(*temp_object).name());
      XELOGI("Current thread: {}", std::this_thread::get_id());
      
      if (auto current_thread = xe::kernel::XThread::GetCurrentThread()) {
        XELOGI("Current thread: '{}'", current_thread->name());
      }
      
      XELOGI("Releasing handle reference - may lead to destruction");
    }
  }
  ...
  auto object = XObject::GetNativeObject<XObject>(
      kernel_state(), kernel_memory()->TranslateVirtual(native_ptr));
  if (object) {
    if (object->handle() == 0x01000010) {
      XELOGI("DEBUG: Releasing handle reference on 01000010:");
      XELOGI("Object type: {}", typeid(*object).name());
      XELOGI("This could lead to thread destruction");
    }
...
dword_result_t ObIsTitleObject_entry(dword_t object_handle) {
  // Check if the object is a title object
  // For now, return false (0) for all objects
  return 0; // Not a title object
}
