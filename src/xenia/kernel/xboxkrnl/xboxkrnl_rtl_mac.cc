...
static uint16_t BoundedAnsiStringLength(const char* source) {
  constexpr size_t kMaxLength = 0xFFFEu;
  size_t length = 0;
  while (length < kMaxLength && source[length] != '\0') {
    ++length;
  }
...
dword_result_t RtlCompareMemoryUlong_entry(lpvoid_t source, dword_t length,
  ...
  auto* host_source =
      reinterpret_cast<const uint8_t*>(source.host_address());

  for (uint32_t aligned_length = length & 0xFFFFFFFCU; aligned_length;
       num_compared_bytes += 4) {
    uint32_t source_pattern;
    std::memcpy(&source_pattern, host_source + num_compared_bytes,
                sizeof(source_pattern));
    if (source_pattern != swapped_pattern) {
      break;
    }
...
void RtlInitAnsiString_entry(pointer_t<X_ANSI_STRING> destination,
  ...
  if (source) {
    ...
    uint16_t length = BoundedAnsiStringLength(source_ptr);
...
dword_result_t RtlCaptureContext_entry(lpvoid_t context_ptr) {
  if (!context_ptr) {
    return 0;
  }
  
  // Simplified implementation - just zero out the context and return success
  std::memset(context_ptr, 0, 128); // Basic context size
  
  return 1; // Success
}
DECLARE_XBOXKRNL_EXPORT1(RtlCaptureContext, kDebug, kImplemented);
