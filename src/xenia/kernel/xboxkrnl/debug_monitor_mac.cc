...
void KeDebugMonitorCallback(cpu::ppc::PPCContext* ppc_context,
  ...
  XELOGI("RING BUFFER: KeDebugMonitorCallback id={} arg={:08X} kernel_pix={}",
         static_cast<uint32_t>(id), arg, cvars::kernel_pix ? "ON" : "OFF");

  if (!cvars::kernel_pix) {
    XELOGI(
        "RING BUFFER: KeDebugMonitorCallback returning no-op success "
        "(kernel_pix=OFF)");
