...
int32_t format_core(PPCContext* ppc_context, FormatData& data, ArgList& args,
  ...
  restart:
        ...
        switch (c) {
          ...
          integer:
            ...
            while (precision-- > 0 || value != 0) {
              ...
              value /= radix;
              assert_true(digit < static_cast<uint64_t>(radix));
              *--start = digits[digit];
            }
          ...
          floatingpoint:
            ...
            double value;
            std::memcpy(&value, &dummy, sizeof(value));
          ...
          case 's': {
            ...
            if (pointer == 0) {
              ...
              text.buffer = nullstr;
              text.length =
                  std::min(static_cast<int32_t>(sizeof(nullstr) - 1), cap);
              text.is_wide = false;
            } else {
              ...
              if (!is_wide) {
                length = 0;
                for (auto s = reinterpret_cast<const uint8_t*>(str);
                     cap > 0 && *s; ++s, cap--) {
                  length++;
                }
              } else {
                length = 0;
                for (auto s = reinterpret_cast<const uint16_t*>(str);
                     cap > 0 && *s; ++s, cap--) {
                  length++;
                }
              }
    ...
    if (!text.is_wide) {
      // it's a const char*
      auto b = reinterpret_cast<const uint8_t*>(text.buffer);
      while (remaining-- > 0) {
        if (!data.put(*b++)) {
          return -1;
        }
      ...
      auto b = reinterpret_cast<const uint16_t*>(text.buffer);
      if (text.swap_wide) {
        while (remaining-- > 0) {
...
class WideCountFormatData : public FormatData {
 public:
  explicit WideCountFormatData(const uint16_t* input)
...
SHIM_CALL DbgPrint_entry(PPCContext* ppc_context) {
  ...
  auto format = reinterpret_cast<const uint8_t*>(SHIM_MEM_ADDR(format_ptr));
  ...
  // Enhanced debugging on GPU-related debug messages
  if (auto current_thread = xe::kernel::XThread::GetCurrentThread()) {
    if (str.find("GPU") != std::string::npos || 
        str.find("Vulkan") != std::string::npos ||
        str.find("Metal") != std::string::npos ||
        str.find("Graphics") != std::string::npos ||
        (current_thread->handle() == 0x01000010)) {
      XELOGI("DEBUG: DbgPrint from GPU-related context:");
      XELOGI("DEBUG: Thread: '{}'", current_thread->name());
      XELOGI("DEBUG: Thread handle: {:08X}", current_thread->handle());
      XELOGI("DEBUG: Debug message: '{}'", str);
    }
  }
...
SHIM_CALL _snprintf_entry(PPCContext* ppc_context) {
  ...
  auto buffer = reinterpret_cast<uint8_t*>(SHIM_MEM_ADDR(buffer_ptr));
  auto format = reinterpret_cast<const uint8_t*>(SHIM_MEM_ADDR(format_ptr));
...
SHIM_CALL _snwprintf_entry(PPCContext* ppc_context) {
  ...
  auto buffer = reinterpret_cast<uint16_t*>(SHIM_MEM_ADDR(buffer_ptr));
  auto format = reinterpret_cast<const uint16_t*>(SHIM_MEM_ADDR(format_ptr));
  ...
  if (count < 0) {
    ...
    xe::copy_and_swap(
        buffer, reinterpret_cast<const uint16_t*>(data.wstr().c_str()),
        count);
    if (count < buffer_count) {
      buffer[count] = '\0';
    }
