...
namespace {
struct RequiredVdExport {
  uint16_t ordinal;
  const char* name;
};

bool IsCriticalGpuBootstrapOrdinal(uint16_t ordinal) {
  return ordinal == 0x01C3 || ordinal == 0x01D5 || ordinal == 0x025B;
}

constexpr std::array<RequiredVdExport, 13> kRequiredVdExports{{
    {0x01B6, "VdEnableRingBufferRPtrWriteBack"},
    {0x01BC, "VdGetGraphicsAsicID"},
    {0x01C2, "VdInitializeEngines"},
    {0x01BD, "VdGetSystemCommandBuffer"},
    {0x01BE, "VdGlobalDevice"},
    {0x01BF, "VdGlobalXamDevice"},
    {0x01C3, "VdInitializeRingBuffer"},
    {0x01C6, "VdIsHSIOTrainingSucceeded"},
    {0x01D5, "VdSetGraphicsInterruptCallback"},
    {0x025B, "VdSwap"},
    {0x0268, "VdInitializeEDRAM"},
    {0x0269, "VdRetrainEDRAM"},
    {0x026A, "VdRetrainEDRAMWorker"},
}};

void ValidateRequiredVdExports(std::vector<xe::cpu::Export*>& exports) {
  for (const auto& required : kRequiredVdExports) {
    xe::cpu::Export* current = nullptr;
    if (required.ordinal < exports.size()) {
      current = exports[required.ordinal];
    }

    auto log_critical_export_state = [&](const char* state) {
      if (!IsCriticalGpuBootstrapOrdinal(required.ordinal)) {
        return;
      }
      if (!current) {
        XELOGW(
            "RING BUFFER: export verify critical ordinal=0x{:03X} "
            "name={} state={} present=NO",
            required.ordinal, required.name, state ? state : "unknown");
        return;
      }
      const bool is_function =
          current->get_type() == xe::cpu::Export::Type::kFunction;
      XELOGW(
          "RING BUFFER: export verify critical ordinal=0x{:03X} "
          "required_name={} export_name={} state={} type={} "
          "implemented={} tags=0x{:08X} trampoline={:p}",
          required.ordinal, required.name,
          current->name ? current->name : "<null>", state ? state : "unknown",
          is_function ? "function": "variable",
          current->is_implemented() ? "YES" : "NO", current->tags,
          is_function
              ? reinterpret_cast<const void*>(current->function_data.trampoline)
              : nullptr);
    };

    if (current && current->name &&
        std::strcmp(current->name, required.name) == 0) {
      if (cvars::trace_gpu_bootstrap) {
        XELOGI("DEBUG: Export verify: xboxkrnl ordinal=0x{:03X} name={} OK",
               required.ordinal, required.name);
      }
      log_critical_export_state("ordinal-match");
      continue;
    }

    xe::cpu::Export* by_name = nullptr;
    for (auto* export_entry : exports) {
      if (export_entry && export_entry->name &&
          std::strcmp(export_entry->name, required.name) == 0) {
    ...
    if (by_name) {
      XELOGW(
          "DEBUG: Export verify: xboxkrnl ordinal 0x{:03X} remapped to {} "
          "(previous={})",
          required.ordinal, required.name, current ? current->name : "<null>");
      exports[required.ordinal] = by_name;
      current = by_name;
      log_critical_export_state("remapped-by-name");
    } else {
      XELOGE(
          "DEBUG: Export verify: missing required xboxkrnl export {} at ordinal "
          "0x{:03X}",
          required.name, required.ordinal);
      log_critical_export_state("missing");
    }
  }
}
}  // namespace
...
#undef XE_MODULE_EXPORT_GROUP
  ...
  // Xenia Fix: Some games (like Geometry Wars) spin in a loop waiting for
  // KeDebugMonitorData to be non-zero. The official kernel usually sets this
  // to 0 when no debugger is attached, but we'll provide a dummy struct
  // to let these games proceed.
  ...
  // Some titles spin in startup until KeDebugMonitorData->callback_fn (0x18)
  // is non-zero. Always expose a callback trampoline so startup can proceed.
  lpKeDebugMonitorData->callback_fn =
      GenerateTrampoline("KeDebugMonitorCallback", KeDebugMonitorCallback);

  lpKeDebugMonitorData->unk_00 = pKeDebugMonitorData + 4;
  lpKeDebugMonitorData->unk_04 = 1;
  lpKeDebugMonitorData->unk_08 = 1;
  lpKeDebugMonitorData->unk_0C = 1;
  lpKeDebugMonitorData->unk_10 = 1;
  lpKeDebugMonitorData->unk_14 = 1;
  lpKeDebugMonitorData->unk_1C = 1;
  lpKeDebugMonitorData->unk_20 = 1;

  XELOGI(
      "RING BUFFER: KeDebugMonitorData initialized ptr={:08X} struct={:08X} "
      "callback={:08X} kernel_debug_monitor={} kernel_pix={}",
      pKeDebugMonitorData, pKeDebugMonitorData + 4,
      static_cast<uint32_t>(lpKeDebugMonitorData->callback_fn),
      cvars::kernel_debug_monitor ? "ON" : "OFF",
      cvars::kernel_pix ? "ON" : "OFF");
  ...
  // XboxKrnlBaseVersion (8b)
  uint32_t pXboxKrnlBaseVersion =
      memory_->SystemHeapAlloc(sizeof(KernelVersion));
  auto lpXboxKrnlBaseVersion = memory_->TranslateVirtual(pXboxKrnlBaseVersion);
  export_resolver_->SetVariableMapping(
      "xboxkrnl.exe", ordinals::XboxKrnlBaseVersion, pXboxKrnlBaseVersion);
  std::memcpy(lpXboxKrnlBaseVersion, kernel_state_->GetKernelVersion(),
              sizeof(KernelVersion));

  // XboxKrnlVersion4Digit (2b)
  uint32_t pXboxKrnlVersion4Digit = memory_->SystemHeapAlloc(2);
  auto lpXboxKrnlVersion4Digit =
      memory_->TranslateVirtual<xe::be<uint16_t>*>(pXboxKrnlVersion4Digit);
  export_resolver_->SetVariableMapping(
      "xboxkrnl.exe", ordinals::XboxKrnlVersion4Digit, pXboxKrnlVersion4Digit);
  *lpXboxKrnlVersion4Digit = kernel_state_->GetKernelVersion()->build;

  // Vd globals are initialized in RegisterVideoExports. Avoid remapping them
  // here so imports always bind to a single authoritative allocation.
...
#undef EXPORT_KVAR
  ...
  XELOGI("DEBUG: BREADCRUMB: XboxkrnlModule::XboxkrnlModule constructor complete");
}
...
#include "xenia/kernel/util/export_table_post.inc"
  ...
  ValidateRequiredVdExports(xboxkrnl_exports);
