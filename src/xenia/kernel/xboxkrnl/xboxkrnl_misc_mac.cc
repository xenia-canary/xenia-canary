...
void RtlUnwind_entry(lpvoid_t target_frame, lpvoid_t target_ip,
  ...
  XELOGI("!! RtlUnwind stub called - no-op");
}
DECLARE_XBOXKRNL_EXPORT1(RtlUnwind, kNone, kStub);

void RtlUnwind2_entry(lpvoid_t target_frame, lpvoid_t target_ip,
                      lpvoid_t exception_record, lpvoid_t return_value,
                      lpvoid_t context_record) {
  XELOGI("!! RtlUnwind2 stub called - no-op");
}
DECLARE_XBOXKRNL_EXPORT1(RtlUnwind2, kNone, kStub);

dword_result_t __C_specific_handler_entry(lpvoid_t exception_record,
                                          lpvoid_t establisher_frame,
                                          lpvoid_t context_record,
                                          lpvoid_t dispatcher_context) {
  XELOGI("!! __C_specific_handler stub called - returning ContinueSearch");
