...
DEFINE_int32(
    ...
    "Content");
DEFINE_bool(xam_log_content_license, true,
            "Log XamContentGetLicenseMask calls and results.", "XAM");
DEFINE_int32(xam_log_content_license_every_n, 1,
             "Log every N XamContentGetLicenseMask calls when logging is enabled.",
             "XAM");
...
dword_result_t XamContentGetLicenseMask_entry(lpdword_t mask_ptr,
  ...
  bool log_call =
      cvars::xam_log_content_license &&
      (call_id <= 5 ||
       (cvars::xam_log_content_license_every_n > 0 &&
        (call_id % cvars::xam_log_content_license_every_n) == 0));
  if (!mask_ptr) {
  ...
  if (!overlapped_ptr) {
    ...
    if (log_call) {
      XELOGI(
          "DEBUG: XamContentGetLicenseMask -> result={} mask=0x{:08X} "
          "deployment_type={} extended_error=0x{:08X}",
          static_cast<uint32_t>(result),
          static_cast<uint32_t>(mask_ptr.value()),
          static_cast<uint32_t>(kernel_state()->deployment_type_),
          extended_error);
    }
    return result;
  } else {
    ...
    if (log_call) {
      XELOGI("DEBUG: XamContentGetLicenseMask -> IO_PENDING (overlapped)");
    }
  ...
  if (XSUCCEEDED(result)) {
    ...
    if (buffer_ptr) {
      ...
      if (buffer_size < buffer.size()) {
        ...
        // Copy data.
        std::memcpy(buffer_ptr.as<uint8_t*>(), buffer.data(), buffer.size());
      }
    }
  }
...
dword_result_t XamContentSetThumbnail_entry(
  ...
  // Buffer is PNG data.
  auto* buffer_data = buffer_ptr.as<uint8_t*>();
  auto buffer = std::vector<uint8_t>(buffer_data, buffer_data + buffer_size);
