...
DEFINE_bool(xam_log_user_signin, true,
            "Log XamUser sign-in/name/XUID queries and results.", "XAM");
DEFINE_int32(xam_log_user_signin_every_n, 1,
             "Log every N XamUser sign-in related calls when logging is enabled.",
             "XAM");
...
X_HRESULT_result_t XamUserGetXUID_entry(dword_t user_index, dword_t type_mask,
  ...
  bool log_call =
      cvars::xam_log_user_signin &&
      (call_id <= 5 ||
       (cvars::xam_log_user_signin_every_n > 0 &&
        (call_id % cvars::xam_log_user_signin_every_n) == 0));
  ...
  *xuid_ptr = 0;
  ...
  if (!kernel_state()->xam_state()->IsUserSignedIn(user_index)) {
    if (log_call) {
      XELOGI("DEBUG: XamUserGetXUID(user_index={}, mask={}) -> NO_SUCH_USER",
  ...
  *xuid_ptr = xuid;
  if (log_call) {
    XELOGI("DEBUG: XamUserGetXUID(user_index={}, mask={}) -> result={} xuid={:016X}",
           static_cast<uint32_t>(user_index), static_cast<uint32_t>(type_mask),
           static_cast<uint32_t>(result), xuid);
  }
  return result;
}
...
dword_result_t XamUserGetSigninState_entry(dword_t user_index) {
  ...
  if (user_index >= XUserMaxUserCount) {
    if (log_call) {
      XELOGI("DEBUG: XamUserGetSigninState(user_index={}) -> INVALID (0)",
  ...
  if (log_call) {
    XELOGI("DEBUG: XamUserGetSigninState(user_index={}) -> {}",
...
dword_result_t XamUserGetName_entry(dword_t user_index, dword_t buffer,
  ...
  if (user_index >= XUserMaxUserCount) {
    if (log_call) {
      XELOGI("DEBUG: XamUserGetName(user_index={}) -> INVALID_PARAMETER",
             static_cast<uint32_t>(user_index));
    }
  ...
  if (!kernel_state()->xam_state()->IsUserSignedIn(user_index)) {
    ...
    if (log_call) {
      XELOGI("DEBUG: XamUserGetName(user_index={}) -> NO_SUCH_USER",
             static_cast<uint32_t>(user_index));
    }
  ...
  if (log_call) {
    XELOGI("DEBUG: XamUserGetName(user_index={}) -> SUCCESS name='{}'",
           static_cast<uint32_t>(user_index), user_name);
  }
  return X_ERROR_SUCCESS;
}
...
dword_result_t XamParseGamerTileKey_entry(pointer_t<X_USER_DATA> key_ptr,
  ...
  if (title_id_ptr) {
    // const bool is_from_dash = IsGamerPictureFromDash(*title_id_ptr);
    // const bool is_avatar = IsGamerPictureAvatar(*title_id_ptr);
    // const bool is_custom = IsGamerPictureCustom(*title_id_ptr);
  }
