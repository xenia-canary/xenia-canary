...
bool Property::Deserialize() { return false; }
...
void Property::CreateUserData(uint8_t* data_ptr) {
  switch (data_type_) {
    case X_USER_DATA_TYPE::CONTENT: {
      data_ = std::make_unique<BinaryUserData>(value_);
      break;
    }
    case X_USER_DATA_TYPE::INT32: {
      data_ = std::make_unique<Int32UserData>(*data_ptr);
      break;
    }
    case X_USER_DATA_TYPE::INT64: {
      data_ = std::make_unique<Int64UserData>(*data_ptr);
      break;
    }
    case X_USER_DATA_TYPE::DOUBLE: {
      data_ = std::make_unique<DoubleUserData>(*data_ptr);
      break;
    }
    case X_USER_DATA_TYPE::WSTRING: {
      data_ = std::make_unique<UnicodeUserData>(
          std::u16string(reinterpret_cast<char16_t*>(data_ptr), value_size_));
      break;
    }
    case X_USER_DATA_TYPE::FLOAT: {
      data_ = std::make_unique<FloatUserData>(*data_ptr);
      break;
    }
    case X_USER_DATA_TYPE::BINARY: {
      data_ = std::make_unique<BinaryUserData>(value_);
    ...
    case X_USER_DATA_TYPE::DATETIME: {
      data_ = std::make_unique<DateTimeUserData>(*data_ptr);
      break;
    }
    default:
      break;
