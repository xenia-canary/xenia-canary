...
X_STATUS ObjectTable::AddHandle(XObject* object, X_HANDLE* out_handle) {
  ...
  uint32_t handle = 0;
  {
    ...
    if (XSUCCEEDED(result)) {
      ...
      // Enhanced debugging - focus on handle 01000010 and provide comprehensive context
      if (handle == 0x01000010) {
        XELOGI("DEBUG: Creating CRITICAL handle 01000010:");
        XELOGI("Object type: {}", typeid(*object).name());
        XELOGI("Object pointer: {}", static_cast<void*>(object));
        XELOGI("Host object: {}", host_object ? "YES" : "NO");
        XELOGI("Slot: {}", slot);
        XELOGI("Thread ID: {}", std::this_thread::get_id());
        
        // Try to get more specific object information
        if (auto thread = dynamic_cast<xe::kernel::XThread*>(object)) {
          XELOGI("Thread name: {}", thread->name());
          XELOGI("Thread ID: {}", thread->thread_id());
        XELOGI("Is host thread: {}", thread->is_guest_thread() ? "NO" : "YES");
        }
        
        // Get current thread context - TLS safe for construction
        // CRITICAL: Removed GetCurrentThread() call here to prevent lock inversion deadlock on macOS.
        XELOGI("Thread ID: {}", std::this_thread::get_id());
        
        // Stack trace for creation context
        
        // Stack trace for creation context
        XELOGI("Creation stack trace:");
        // Note: Could add backtrace() here if available
        
        XELOGI("Object handle count after creation: {}", object->handles().size());
      }
...
X_STATUS ObjectTable::RemoveHandle(X_HANDLE handle) {
  ...
  // Enhanced debugging - track removal of handle 01000010
  if (handle == 0x01000010) {
    XELOGI("DEBUG: Attempting to remove CRITICAL handle 01000010:");
    XELOGI("Thread ID: {}", std::this_thread::get_id());
    ...
    XELOGI("DEBUG: CRITICAL: GPU Command Processor Thread being removed!");
    XELOGI("This may explain why emulator hangs after this point");
  }

  ObjectTableEntry* entry = LookupTableInLock(handle);
  if (!entry) {
    if (handle == 0x01000010) {
      XELOGI("DEBUG: FAILED: Handle 01000010 not found in table!");
    }
    return X_STATUS_INVALID_HANDLE;
  }
  ...
  if (entry->object) {
    ...
    // Enhanced debugging for handle 01000010
    if (handle == 0x01000010) {
      XELOGI("Object being removed: {}", typeid(*object).name());
      XELOGI("Object pointer: {}", static_cast<void*>(object));
      XELOGI("Handle ref count before removal: {}", entry->handle_ref_count);
      XELOGI("Object handles count: {}", object->handles().size());
      
      if (auto thread = dynamic_cast<xe::kernel::XThread*>(object)) {
        XELOGI("Thread name: {}", thread->name());
        XELOGI("Thread state: {}", thread->is_running() ? "RUNNING" : "STOPPED");
        XELOGI("Is guest thread: {}", thread->is_guest_thread());
      }
      
      XELOGI("Object name: '{}'", object->name());
    }
    ...
    // Enhanced debugging for handle 01000010
    if (handle == 0x01000010) {
      XELOGI("DEBUG: Handle 01000010 successfully removed from table");
      XELOGI("Remaining object handles: {}", object->handles().size());
      XELOGI("Object ref count will be decremented after release");
    }
...
XObject* ObjectTable::LookupObject(X_HANDLE handle, bool already_locked) {
  ...
  // Enhanced debugging - track access to handle 01000010
  if (handle == 0x01000010) {
    XELOGI("DEBUG: Accessing CRITICAL handle 01000010:");
    XELOGI("Thread ID: {}", std::this_thread::get_id());
    ...
    // Try to get stack trace or call context
    XELOGI("Access context: LookupObject called with already_locked={}", already_locked);
  }
  ...
  // Enhanced debugging - track what we found
  if (handle == 0x01000010) {
    if (object) {
      XELOGI("DEBUG: Object found: {}", typeid(*object).name());
      XELOGI("Object pointer: {}", static_cast<void*>(object));
      ...
      XELOGI("DEBUG: OBJECT NOT FOUND - Handle 01000010 is missing!");
      XELOGI("This indicates the object was removed or never properly added");
    }
  }
