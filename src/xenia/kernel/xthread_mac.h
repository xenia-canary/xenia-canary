...
#ifndef XENIA_KERNEL_XTHREAD_MAC_H_
#define XENIA_KERNEL_XTHREAD_MAC_H_
...
class XThread : public XObject, public cpu::Thread {
 public:
...
#ifdef __APPLE__
  // macOS TLS backup and restore utilities for critical threads
  static void BackupTLS();
  static void RestoreTLS();
  static bool VerifyTLS();
  static XThread* GetTLSBackup();
  static bool IsTLSBackupValid();
#endif
  ...
  // Thread role validation - ensures critical infrastructure threads are HOST threads
  static void ValidateThreadRole(const char* thread_name, bool is_guest_thread,
                                  X_HANDLE handle);
  static void TerminateOnThreadRoleMismatch(const char* thread_name,
...
#ifdef __APPLE__
// Thread state manager for critical thread protection
class ThreadStateManager {
public:
  static ThreadStateManager& Instance();
  
  void RegisterCriticalThread(uint32_t handle, XThread* thread);
  void UnregisterCriticalThread(uint32_t handle);
  bool VerifyCriticalThread(uint32_t handle);
  void ForceTLSSync(XThread* thread);
  
private:
  std::mutex state_mutex_;
  std::unordered_map<uint32_t, XThread*> critical_threads_;
...
#endif  // XENIA_KERNEL_XTHREAD_MAC_H_
