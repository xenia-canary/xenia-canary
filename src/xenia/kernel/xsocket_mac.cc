...
X_STATUS XSocket::SetOption(uint32_t level, uint32_t optname, void* optval_ptr,
  ...
  int ret = setsockopt(static_cast<int>(native_handle_), native_level,
                       native_optname, reinterpret_cast<const char*>(optval_ptr),
                       static_cast<socklen_t>(optlen));
#else
  int ret = setsockopt(native_handle_, native_level, native_optname,
                       reinterpret_cast<char*>(optval_ptr), optlen);
#endif
...
#ifdef XE_PLATFORM_WIN32
  int ret =
      ioctlsocket(native_handle_, cmd, reinterpret_cast<u_long*>(arg_ptr));
  if (ret < 0) {
    // TODO: Get last error
    return X_STATUS_UNSUCCESSFUL;
  }
  return X_STATUS_SUCCESS;
#elif XE_PLATFORM_LINUX && !XE_PLATFORM_MACOS
  int native_cmd = cmd;
...
#else
  int ret = connect(native_handle_, reinterpret_cast<sockaddr*>(name),
                    name_len);
#endif
  if (ret < 0) {
    return X_STATUS_UNSUCCESSFUL;
...
#else
  int ret =
      bind(native_handle_, reinterpret_cast<sockaddr*>(name), name_len);
#endif
...
int XSocket::RecvFrom(uint8_t* buf, uint32_t buf_len, uint32_t flags,
  ...
  /*
  {
    ...
    if (incoming_packets_.size()) {
      ...
      uint8_t* pkt_ui8 = reinterpret_cast<uint8_t*>(pkt);
      delete[] pkt_ui8;
  ...
  */
  ...
  int ret = recvfrom(native_handle_, reinterpret_cast<char*>(buf), buf_len,
                     flags, reinterpret_cast<sockaddr*>(&nfrom), &nfromlen);
#endif
  if (from) {
...
int XSocket::SendTo(uint8_t* buf, uint32_t buf_len, uint32_t flags,
  ...
  */
  ...
  return static_cast<int>(sendto(static_cast<int>(native_handle_),
                                 reinterpret_cast<char*>(buf),
                                 static_cast<size_t>(buf_len), flags,
                                 to ? reinterpret_cast<const sockaddr*>(&nto)
                                    : nullptr,
                                 static_cast<socklen_t>(to_len)));
#else
  return sendto(native_handle_, reinterpret_cast<char*>(buf), buf_len, flags,
                to ? reinterpret_cast<const sockaddr*>(&nto) : nullptr,
                to_len);
#endif
}
...
#else
  int ret =
      getsockname(native_handle_, &sa, reinterpret_cast<socklen_t*>(buf_len));
#endif
  if (ret < 0) {
    return X_STATUS_UNSUCCESSFUL;
  }
