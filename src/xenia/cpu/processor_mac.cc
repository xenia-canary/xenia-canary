...
DEFINE_bool(break_on_start, false, "Break into the debugger on startup.",
            "CPU");
DEFINE_bool(log_main_loop_state, false,
            "Debug: Log main-loop gate state for 0x82096008.", "CPU");
DEFINE_uint32(log_main_loop_state_every_n, 1,
              "Debug: Log every N main-loop gate samples.", "CPU");
DEFINE_bool(log_main_loop_calls, false,
            "Debug: Log return values for main-loop call targets.", "CPU");
DEFINE_uint32(log_main_loop_calls_every_n, 1,
              "Debug: Log every N main-loop call return samples.", "CPU");
DEFINE_bool(force_main_loop_gate, false,
            "Force the main-loop gate value at 0x821F4898 after a delay. "
            "Disabled by default to avoid masking or introducing title "
            "startup stalls; enable only for targeted debugging.",
            "CPU");
DEFINE_uint32(force_main_loop_gate_after_ms, 2000,
              "Debug: Delay before forcing main-loop gate value.", "CPU");
DEFINE_uint32(force_main_loop_gate_value, 1,
              "Debug: Value to write to the main-loop gate.", "CPU");
DEFINE_bool(force_main_loop_call_return, false,
            "Debug: Override return value (r3) for main-loop call targets. "
            "Disabled by default to keep title flow guest-driven.",
            "CPU");
DEFINE_uint32(force_main_loop_call_return_value, 1,
              "Debug: Value to force into r3 after main-loop calls.", "CPU");

// Forward declaration of cvar defined in xenia_main.cc
DECLARE_bool(log_cpu_features);
...
class BuiltinModule : public Module {
 public:
  ...
  bool ContainsAddress(uint32_t address) override {
    return (address & 0xFFFF0000u) == 0xFFFF0000u;
  }
...
bool Processor::Setup(std::unique_ptr<backend::Backend> backend) {
  XELOGI("Processor::Setup() starting...");
  // TODO(benvanik): query mode from debugger?
  debug_info_flags_ = 0;

  XELOGI("Processor::Setup() creating frontend...");
  ...
  XELOGI("Processor::Setup() creating builtin module...");
  ...
  if (frontend_ || backend_) {
    XELOGE("Processor::Setup() failed - frontend_ or backend_ already initialized");
    return false;
  }

  if (!backend) {
    XELOGE("Processor::Setup() failed - backend is null");
    return false;
  }
  
  XELOGI("Processor::Setup() initializing backend...");
  if (!backend->Initialize(this)) {
    XELOGE("Processor::Setup() failed - backend->Initialize() returned false");
    return false;
  }
  
  XELOGI("Processor::Setup() initializing frontend...");
  if (!frontend->Initialize()) {
    XELOGE("Processor::Setup() failed - frontend->Initialize() returned false");
    return false;
  }
  
  XELOGI("Processor::Setup() backend and frontend initialization completed");

  XELOGI("Processor::Setup() moving backend and frontend to member variables...");
  backend_ = std::move(backend);
  frontend_ = std::move(frontend);

  XELOGI("Processor::Setup() setting up stack walker...");
  ...
  if (code_cache) {
    XELOGI("Processor::Setup() code cache available, creating stack walker...");
    stack_walker_ = StackWalker::Create(code_cache);
  } else {
    XELOGI("Processor::Setup() no code cache available");
  }
  if (!stack_walker_) {
    XELOGI("Processor::Setup() stack walker creation failed, disabling debug features...");
    ...
    XELOGI("Processor::Setup() stack walker created successfully");
  }

  XELOGI("Processor::Setup() setting up function trace path...");
  // Open the trace data path, if requested.
  functions_trace_path_ = cvars::trace_function_data_path;
  if (!functions_trace_path_.empty()) {
    XELOGI("Processor::Setup() opening function trace file: '{}'", functions_trace_path_);
    functions_trace_file_ =
        ChunkedMappedMemoryWriter::Open(functions_trace_path_, 32_MiB, true);
  } else {
    XELOGI("Processor::Setup() no function trace path specified");
  }

  // Log VMX/AltiVec capability confirmation for PowerPC execution
  auto& cpu = CpuFeatures::Get();
  if (cvars::log_cpu_features) {
    XELOGI("DEBUG: PowerPC Execution Capabilities");
    XELOGI("DEBUG: VMX/AltiVec Support: {}",
           cpu.has_vmx2neon() ? "AVAILABLE" : "NOT AVAILABLE");
    if (cpu.has_vmx2neon()) {
      XELOGI("PowerPC SIMD instructions will be translated to ARM NEON");
      XELOGI("Xbox 360 VMX code execution: ENABLED");
    } else {
      XELOGI("PowerPC SIMD instructions will NOT be available");
      XELOGI("Xbox 360 VMX code execution: DISABLED - Some games may not work");
    }
  }

  XELOGI("Processor::Setup() completed successfully!");
  return true;
}
...
bool Processor::AddModule(std::unique_ptr<Module> module) {
  ...
  const std::string module_name = module ? module->name() : std::string("<null>");
  XELOGI("DEBUG: Processor::AddModule begin module='{}'", module_name);
  auto global_lock = global_critical_region_.Acquire();
  modules_.push_back(std::move(module));
  XELOGI("DEBUG: Processor::AddModule end module='{}' elapsed={} ms", module_name,
...
bool Processor::Execute(ThreadState* thread_state, uint32_t address) {
  ...
  if (IsGpuBootstrapStallTarget(address)) {
    static std::atomic<uint64_t> stall_entry_count{0};
    uint64_t stall_id = stall_entry_count.fetch_add(1) + 1;
    if (stall_id <= 50 || (stall_id % 200) == 0) {
      ...
      XELOGW(
          "DEBUG: GPU stall call entry #{}: addr={:08X} instr_valid={} "
          "instr={:08X} thread_id={} pc={:08X} lr={:08X} ctr={:08X} "
          "r1={:016X} r3={:016X} r4={:016X} r13={:016X}",
          stall_id, address, instr_valid ? "YES" : "NO", instr,
          thread_state ? thread_state->thread_id() : 0,
          context ? context->current_pc : 0,
          context ? static_cast<uint32_t>(context->lr) : 0,
          context ? static_cast<uint32_t>(context->ctr) : 0,
          context ? context->r[1] : 0, context ? context->r[3] : 0,
          context ? context->r[4] : 0, context ? context->r[13] : 0);
    }
  }

  // Xenia Fix: Diagnostics for Geometry Wars initialization loop.
  // The guest polls at 82096008/82097950.
  if (address == 0x82096008 || address == 0x82097950) {
    static std::atomic<uint32_t> sample_count{0};
    uint32_t count = sample_count.fetch_add(1);
    if (count < 60) {
      auto context = thread_state->context();
      XELOGI("DEBUG: POLL SCAN: pc={:08X} count={} r3={:016X} r4={:016X} lr={:08X}",
             address, count, context->r[3], context->r[4], (uint32_t)context->lr);
      if (context) {
        // `cr` is exposed via a getter; XER is split into three bytes
        // (`xer_ca`, `xer_ov`, `xer_so`) so compose a simple value for logging.
        uint32_t cr_val = static_cast<uint32_t>(context->cr());
        uint32_t xer_val = (static_cast<uint32_t>(context->xer_ca) & 0x1) |
               ((static_cast<uint32_t>(context->xer_ov) & 0x1) << 1) |
               ((static_cast<uint32_t>(context->xer_so) & 0x1) << 2);
        XELOGI(
            "DEBUG: POLL SCAN regs: r1={:016X} r5={:016X} r6={:016X} "
            "r8={:016X} ctr={:08X} cr={:08X} xer={:08X} msr={:016X}",
            static_cast<uint64_t>(context->r[1]),
            static_cast<uint64_t>(context->r[5]),
            static_cast<uint64_t>(context->r[6]),
            static_cast<uint64_t>(context->r[7]),
            static_cast<uint64_t>(context->r[8]),
            static_cast<uint32_t>(context->ctr), cr_val, xer_val,
            static_cast<uint64_t>(context->msr));
      }
      auto memory = thread_state ? thread_state->memory() : nullptr;
      if (context && memory) {
  ...
  if (cvars::log_main_loop_state && address == 0x82096008) {
    static std::atomic<uint64_t> gate_count{0};
    uint64_t count = gate_count.fetch_add(1) + 1;
    uint32_t every_n = cvars::log_main_loop_state_every_n;
    if (every_n == 0 || (count % every_n) == 0) {
      ...
      XELOGW(
          "DEBUG: MainLoop gate #{}: addr={:08X} value=0x{:08X} valid={} "
          "pc={:08X} lr={:08X} r3={:016X} r4={:016X} thread_id={}",
          count, gate_addr, gate_value, gate_valid ? "YES" : "NO", address,
          context ? static_cast<uint32_t>(context->lr) : 0,
          context ? context->r[3] : 0, context ? context->r[4] : 0,
          thread_state ? thread_state->thread_id() : 0);
    }
  }
  if (cvars::force_main_loop_gate && address == 0x82096008) {
    static std::atomic<uint64_t> gate_first_ms{0};
    static std::atomic<bool> gate_forced{false};
    auto context = thread_state->context();
    auto memory = thread_state ? thread_state->memory() : nullptr;
    if (context && memory) {
      uint64_t now_ms = Clock::QueryHostUptimeMillis();
      uint64_t first_ms = gate_first_ms.load(std::memory_order_relaxed);
      if (first_ms == 0) {
        gate_first_ms.store(now_ms, std::memory_order_relaxed);
        first_ms = now_ms;
      }
      uint64_t delay_ms =
          static_cast<uint64_t>(cvars::force_main_loop_gate_after_ms);
      if (!gate_forced.load(std::memory_order_relaxed) &&
  ...
  if (context->current_pc == 0) {
    // Seed early startup diagnostics before the translated block updates
    // current_pc from its first emitted source offset.
    context->current_pc = address;
  }
  ...
  if (cvars::force_main_loop_call_return && IsMainLoopCallTarget(address)) {
    uint32_t forced_value = cvars::force_main_loop_call_return_value;
    context->r[3] = forced_value;
    static std::atomic<uint64_t> forced_count{0};
    uint64_t count = forced_count.fetch_add(1) + 1;
    if (count <= 10 || (count % 100) == 0) {
      XELOGW(
          "DEBUG: MainLoop call forced (count={}): addr={:08X} r3=0x{:08X} pc={:08X} "
          "thread_id={}",
          count, address, forced_value, context->current_pc,
          thread_state ? thread_state->thread_id() : 0);
    }
  }
  if (cvars::log_main_loop_calls && IsMainLoopCallTarget(address)) {
    static std::atomic<uint64_t> call_count{0};
    uint64_t call_id = call_count.fetch_add(1) + 1;
    uint32_t every_n = cvars::log_main_loop_calls_every_n;
    if (every_n == 0 || (call_id % every_n) == 0) {
      XELOGW(
          "DEBUG: MainLoop call ret #{}: addr={:08X} r3={:016X} r4={:016X} "
          "r5={:016X} pc={:08X} lr={:08X} thread_id={}",
          call_id, address, context->r[3], context->r[4], context->r[5],
          context->current_pc, static_cast<uint32_t>(context->lr),
          thread_state ? thread_state->thread_id() : 0);
    }
  }
  context->r[1] += 64 + 112;
  ...
  if (IsGpuBootstrapStallTarget(address)) {
    static std::atomic<uint64_t> stall_ret_count{0};
    uint64_t stall_id = stall_ret_count.fetch_add(1) + 1;
    if (stall_id <= 50 || (stall_id % 200) == 0) {
      XELOGW(
          "DEBUG: GPU stall call return #{}: addr={:08X} result={} "
          "thread_id={} pc={:08X} lr={:08X} ctr={:08X} "
          "r1={:016X} r3={:016X} r4={:016X}",
          stall_id, address, result ? "SUCCESS" : "FAIL",
          thread_state ? thread_state->thread_id() : 0, context->current_pc,
          static_cast<uint32_t>(context->lr), static_cast<uint32_t>(context->ctr),
          context->r[1], context->r[3], context->r[4]);
    }
  }
...
#ifdef __APPLE__
  uint32_t thread_id = Thread::GetCurrentThreadId();
  HostThreadContext* thread_context = 
      reinterpret_cast<HostThreadContext*>(ex->thread_context());
  
  // Just call it - can't check success/failure if it returns void
  UpdateThreadExecutionStates(thread_id, thread_context);
  
  // Debug output (always print since we don't know if it succeeded)
  fprintf(stderr, "DEBUG: Thread update called: ID=0x%08X, ctx=%p\n",
...
uint32_t Processor::StepIntoGuestBranchTarget(uint32_t thread_id, uint32_t pc) {
  ...
  xe::cpu::ppc::SetLastPPCDecodeAddress(d.address);
...
uint32_t Processor::StepToGuestSafePoint(uint32_t thread_id, bool ignore_host) {
  ...
  if (first_frame.type == cpu::StackFrame::Type::kGuest) {
    ...
    if (export_data && export_data->tags & cpu::ExportTag::kBlocking) {
      ...
      xe::cpu::ppc::SetLastPPCDecodeAddress(first_pc);
