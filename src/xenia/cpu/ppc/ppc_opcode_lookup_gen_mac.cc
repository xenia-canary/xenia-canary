...
#define PPC_DECODER_MISS return DecoderMiss(code)
#define PPC_DECODER_HIT(name) return PPCOpcode::name;

DEFINE_bool(
    ppc_assert_on_unknown_opcode, false,
    "Assert when an unknown PPC opcode is encountered (otherwise log and "
    "treat as invalid).",
    "CPU");
DEFINE_bool(
    ppc_ignore_sentinel_unknown_opcode, true,
    "Ignore unknown opcodes when the decode address matches the 0xBCBC**** "
    "sentinel range (used for LR return sentinels).",
    "CPU");

inline PPCOpcode DecoderMiss(uint32_t code) {
  uint32_t primary = ExtractBits(code, 0, 5);
  uint32_t last_address = GetLastPPCDecodeAddress();
  if (last_address) {
    XELOGE("Unknown PPC opcode: {:08X} at {:08X} (primary={})", code,
           last_address, primary);
    fprintf(stderr,
            "Unknown PPC opcode: %08X at %08X (primary=%u)\n",
            code, last_address, primary);
  } else {
    XELOGE("Unknown PPC opcode: {:08X} (primary={})", code, primary);
    fprintf(stderr,
            "Unknown PPC opcode: %08X (primary=%u)\n",
            code, primary);
  }
  fflush(stderr);
  if ((last_address & 0xFFFF0000u) == 0xBCBC0000u &&
      cvars::ppc_ignore_sentinel_unknown_opcode) {
    XELOGW("Ignoring unknown opcode at LR sentinel address {:08X} (code={:08X})",
           last_address, code);
    fprintf(stderr,
            "LR sentinel decode: addr=%08X code=%08X (ignored)\n",
            last_address, code);
    fflush(stderr);
    return PPCOpcode::kInvalid;
  }
  if (cvars::ppc_assert_on_unknown_opcode) {
    assert_always();
  }
  return PPCOpcode::kInvalid;
}
