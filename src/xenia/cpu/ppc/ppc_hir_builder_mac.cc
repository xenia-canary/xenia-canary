...
DEFINE_bool(
    ...
    "CPU");
DEFINE_bool(
    ppc_unknown_opcode_as_nop, true,
    "Treat unknown PPC opcodes as NOP (log and continue instead of failing).",
    "CPU");
...
xe::Memory* PPCHIRBuilder::memory() const { return frontend_->memory(); }
...
bool PPCHIRBuilder::Emit(GuestFunction* function, uint32_t flags) {
  ...
  instr_offset_list_ =
      reinterpret_cast<Instr**>(arena_->Alloc(list_size, alignof(void*)));
  label_list_ =
      reinterpret_cast<Label**>(arena_->Alloc(list_size, alignof(void*)));
  ...
  for (uint32_t address = start_address, offset = 0; address <= end_address;
    ...
    if (opcode == PPCOpcode::kInvalid) {
      ...
      Comment("INVALID!");
      if (cvars::ppc_unknown_opcode_as_nop) {
        // Emit a no-op so execution can continue and we can discover more
        // unknown opcodes in a single run.
        Nop();
      }
...
void PPCHIRBuilder::AnnotateLabel(uint32_t address, Label* label) {
  ...
  label->name = reinterpret_cast<char*>(arena_->Alloc(sizeof(name_buffer), 1));
