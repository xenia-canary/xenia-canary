...
// Include VMX2NEON headers when on Apple Silicon
#if defined(__APPLE__) && defined(__aarch64__)
#include "../../third_party/vmx2neon/altivec_macos.h"
#endif
...
DEFINE_bool(ignore_trap_instructions, true,
            ...
            "CPU");
DEFINE_bool(inline_savegprlr, false,
            "Inline __savegprlr_N prolog helpers instead of calling them. "
            "Debug fallback for prolog stalls.",
            "CPU");
...
bool ParseSaveGprLrStart(const std::string& name, int* start_reg_out) {
  static constexpr const char* kPrefix = "__savegprlr_";
  constexpr size_t prefix_len = sizeof("__savegprlr_") - 1;
  if (name.size() < prefix_len ||
      name.compare(0, prefix_len, kPrefix) != 0) {
    return false;
  }
  const char* num_start = name.c_str() + prefix_len;
  if (*num_start == '\0') {
    return false;
  }
  char* end = nullptr;
  long parsed = std::strtol(num_start, &end, 10);
  if (!end || *end != '\0') {
    return false;
  }
  ...
  *start_reg_out = static_cast<int>(parsed);
  return true;
}

bool TryDecodeSaveGprLrStartFromGuestMemory(PPCHIRBuilder& f, uint32_t target,
                                            int* start_reg_out) {
  if (!f.memory()) {
    return false;
  }
  ...
  for (size_t i = 0; i < kSaveGprLrCodeValues.size(); ++i) {
    if (code == kSaveGprLrCodeValues[i]) {
      *start_reg_out = 14 + static_cast<int>(i);
      return true;
    }
  }
  return false;
...
bool TryInlineSaveGprLr(PPCHIRBuilder& f, uint64_t cia, uint32_t target) {
  const bool force_gpu_bootstrap_inline =
      IsGpuBootstrapStartupFunction(f.function()->address());
  if (!::cvars::inline_savegprlr && !force_gpu_bootstrap_inline) {
    return false;
  }
  auto fn = f.LookupFunction(target);
  int start_reg = -1;
  bool decoded_from_guest_memory = false;
  std::string helper_name;
  if (fn && fn->behavior() == cpu::Function::Behavior::kProlog &&
      ParseSaveGprLrStart(fn->name(), &start_reg)) {
    helper_name = fn->name();
  } else if (TryDecodeSaveGprLrStartFromGuestMemory(f, target, &start_reg)) {
    decoded_from_guest_memory = true;
    helper_name = "__savegprlr_" + std::to_string(start_reg);
  } else {
    if (force_gpu_bootstrap_inline) {
      XELOGW(
          "DEBUG: Unable to inline early GPU bootstrap savegprlr target {:08X} "
          "in {:08X} (lookup={} behavior={} name={})",
          target, f.function()->address(), fn ? "YES" : "NO",
  ...
  if (force_gpu_bootstrap_inline) {
    XELOGI(
        "DEBUG: Inlining {} at {:08X} in early GPU bootstrap function {:08X} "
        "(caller {:08X}, decode_source={})",
        helper_name, target, f.function()->address(), static_cast<uint32_t>(cia),
        decoded_from_guest_memory ? "guest-code" : "function-metadata");
  } else {
    XELOGI("DEBUG: Inlining {} at {:08X} (caller {:08X}, decode_source={})",
           helper_name, target, static_cast<uint32_t>(cia),
           decoded_from_guest_memory ? "guest-code" : "function-metadata");
  }

  Value* base = f.LoadGPR(1);
  for (int reg = start_reg; reg <= 31; ++reg) {
    int32_t offset = -8 * (32 - reg) - 8;
    Value* addr = f.Add(base, f.LoadConstantInt64(offset));
    Value* reg_val = f.LoadGPR(static_cast<uint32_t>(reg));
    f.Store(addr, reg_val);
  }
  // r12 is saved as a 32-bit value at -0x8(r1).
  Value* addr12 = f.Add(base, f.LoadConstantInt64(-8));
  Value* r12 = f.Truncate(f.LoadGPR(12), INT32_TYPE);
  f.Store(addr12, r12);
  return true;
}
}  // namespace
...
int InstrEmit_branch(PPCHIRBuilder& f, const char* src, uint64_t cia,
  ...
  if (nia->IsConstant()) {
    ...
    if (lk && TryInlineSaveGprLr(f, cia, nia_value)) {
      return 0;
    }
