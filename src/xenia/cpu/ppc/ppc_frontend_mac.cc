...
bool PPCFrontend::Initialize() {
  XELOGI("PPCFrontend::Initialize() starting...");
  
  XELOGI("PPCFrontend::Initialize() defining global lock builtins...");
  ...
  XELOGI("PPCFrontend::Initialize() defining syscall handler builtin...");
  builtins_.syscall_handler = processor_->DefineBuiltin(
      "SyscallHandler", SyscallHandler, nullptr, nullptr);
  
  XELOGI("PPCFrontend::Initialize() completed successfully!");
  return true;
}
