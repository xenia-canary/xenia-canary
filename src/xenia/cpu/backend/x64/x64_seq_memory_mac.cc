...
struct LOAD_OFFSET_I32
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    if (IsPossibleMMIOInstruction(e, i.instr)) {
      void* addrptr =
          reinterpret_cast<void*>(&MMIOAwareLoad<uint32_t, false>);

      if (i.instr->flags & LoadStoreFlags::LOAD_STORE_BYTE_SWAP) {
        addrptr = reinterpret_cast<void*>(&MMIOAwareLoad<uint32_t, true>);
      }
...
struct STORE_OFFSET_I32
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    if (IsPossibleMMIOInstruction(e, i.instr)) {
      void* addrptr =
          reinterpret_cast<void*>(&MMIOAwareStore<uint32_t, false>);

      if (i.instr->flags & LoadStoreFlags::LOAD_STORE_BYTE_SWAP) {
        addrptr = reinterpret_cast<void*>(&MMIOAwareStore<uint32_t, true>);
      }
      if (i.src1.is_constant) {
