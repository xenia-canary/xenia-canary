...
DECLARE_bool(log_main_loop_state);
DECLARE_uint32(log_main_loop_state_every_n);
...
DEFINE_bool(log_gpu_import_thunks, false,
            "Debug: Log calls to GPU-related import thunks (Vd* ordinals).",
            "CPU");
DEFINE_uint32(log_gpu_import_thunks_every_n, 1,
              "Debug: Log every N GPU import thunk calls.", "CPU");
DEFINE_bool(log_extern_calls, false,
            "Debug: Log all extern thunk calls (imports).", "CPU");
DEFINE_uint32(log_extern_calls_every_n, 1,
              "Debug: Log every N extern thunk calls.", "CPU");
DEFINE_bool(emit_source_annotations, false,
            "Add extra movs and nops to make disassembly easier to read.",
            "CPU");
DEFINE_bool(track_guest_pc, false,
            "Write current guest PC into PPC context at each source offset "
            "(expensive, debug only).",
            "CPU");
DEFINE_uint32(guest_pc_watch, 0,
              "Guest PC address to log every time it executes (0 disables).",
              "Debug");
DEFINE_uint32(
    guest_pc_watch2, 0,
    "Second guest PC address to log every time it executes (0 disables).",
    "Debug");
DEFINE_uint32(
    guest_pc_watch3, 0,
    "Third guest PC address to log every time it executes (0 disables).",
    "Debug");
DEFINE_uint32(
    guest_pc_watch4, 0,
    "Fourth guest PC address to log every time it executes (0 disables).",
    "Debug");
DEFINE_uint32(guest_pc_watch_every, 1,
              "Log every Nth hit for guest_pc_watch(1-4).", "Debug");
DEFINE_bool(log_vex_translation_missing, true,
            "Log VEX/EVEX instructions emitted by the x64 JIT so missing "
            "translation hooks can be implemented.",
            "CPU");
DEFINE_uint32(
    log_vex_translation_limit, 0,
    "Maximum number of unique VEX instructions to log per run (0 = unlimited).",
    "CPU");
...
uint64_t DebugGuestPcWatch(void* raw_context, uint64_t pc_u64) {
  auto ctx = reinterpret_cast<ppc::PPCContext*>(raw_context);
  uint32_t pc = static_cast<uint32_t>(pc_u64);
  uint32_t watch = cvars::guest_pc_watch;
  uint32_t watch2 = cvars::guest_pc_watch2;
  uint32_t watch3 = cvars::guest_pc_watch3;
  uint32_t watch4 = cvars::guest_pc_watch4;
  uint32_t every = cvars::guest_pc_watch_every;
  if (every == 0) {
    every = 1;
  }
  static std::atomic<uint64_t> watch_counter{0};
  static std::atomic<uint64_t> watch_counter2{0};
  static std::atomic<uint64_t> watch_counter3{0};
  static std::atomic<uint64_t> watch_counter4{0};
  std::atomic<uint64_t>* counter_ptr = &watch_counter;
  if (pc == watch) {
    counter_ptr = &watch_counter;
  } else if (pc == watch2) {
    counter_ptr = &watch_counter2;
  } else if (pc == watch3) {
    counter_ptr = &watch_counter3;
  } else if (pc == watch4) {
    counter_ptr = &watch_counter4;
  }
  ...
  uint64_t r3 = ctx->r[3];
  uint64_t r4 = ctx->r[4];
  uint64_t r12 = ctx->r[12];
  uint64_t msr = ctx->msr;

  uint32_t instr = 0;
  auto instr_ptr = ctx->TranslateVirtual<uint32_t*>(pc);
  instr = xe::load_and_swap<uint32_t>(instr_ptr);
  uint32_t target = 0;
  bool lk = false;
  bool aa = false;
  bool is_branch = DecodeBranchTarget(pc, instr, &target, &lk, &aa);

  XELOGI(
      "DEBUG: PC WATCH hit={} pc={:08X} instr={:08X} lr={:08X} ctr={:08X} "
      "r1={:016X} r3={:016X} r4={:016X} r12={:016X} msr={:016X}",
      count, pc, instr, lr, ctr, static_cast<uint64_t>(r1),
      static_cast<uint64_t>(r3), static_cast<uint64_t>(r4),
      static_cast<uint64_t>(r12), static_cast<uint64_t>(msr));
  if (is_branch) {
    XELOGI(
        "DEBUG: PC WATCH branch pc={:08X} -> target={:08X} link={} "
        "absolute={}",
        pc, target, lk ? "YES" : "NO", aa ? "YES" : "NO");
  }
  return 0;
}

uint64_t DebugMainLoopPc(void* raw_context, uint64_t pc_u64) {
  if (!::cvars::log_main_loop_state) {
    return 0;
  }
  auto ctx = reinterpret_cast<ppc::PPCContext*>(raw_context);
  uint32_t pc = static_cast<uint32_t>(pc_u64);
  static std::atomic<uint64_t> loop_count{0};
  static std::atomic<uint64_t> sub_count{0};
  std::atomic<uint64_t>* counter_ptr = &loop_count;
  if (pc == 0x82097950) {
    counter_ptr = &sub_count;
  }
  uint64_t count = counter_ptr->fetch_add(1) + 1;
  uint32_t every = ::cvars::log_main_loop_state_every_n;
  if (every == 0) {
    every = 1;
  }
  if (count > 30 && (count % every) != 0) {
    return 0;
  }
  uint32_t lr = ctx ? static_cast<uint32_t>(ctx->lr) : 0;
  uint32_t ctr = ctx ? static_cast<uint32_t>(ctx->ctr) : 0;
  uint64_t r1 = ctx ? ctx->r[1] : 0;
  uint64_t r3 = ctx ? ctx->r[3] : 0;
  uint64_t r4 = ctx ? ctx->r[4] : 0;
  uint32_t gate_addr = 0x821F4898;
  uint32_t gate_value = 0;
  bool gate_valid = false;
  if (ctx && ctx->thread_state) {
    auto memory = ctx->thread_state->memory();
    if (memory) {
  ...
  XELOGW(
      "DEBUG: MAIN LOOP PC hit #{} pc={:08X} lr={:08X} ctr={:08X} r1={:016X} "
      "r3={:016X} r4={:016X} gate[{:08X}]=0x{:08X} valid={}",
      count, pc, lr, ctr, static_cast<uint64_t>(r1), static_cast<uint64_t>(r3),
      static_cast<uint64_t>(r4), gate_addr, gate_value,
      gate_valid ? "YES" : "NO");
  return 0;
}

bool IsYmmOrZmmReg(x86_reg reg) {
  return (reg >= X86_REG_YMM0 && reg <= X86_REG_YMM31) ||
         (reg >= X86_REG_ZMM0 && reg <= X86_REG_ZMM31);
}

void LogVexTranslationMissing(const uint8_t* code, size_t size,
                              uint32_t guest_function) {
  if (!cvars::log_vex_translation_missing || !code || size == 0) {
    return;
  }
  ...
  if (cvars::log_vex_translation_limit &&
      seen.size() >= cvars::log_vex_translation_limit) {
    return;
  }
  ...
  for (size_t i = 0; i < count; ++i) {
    ...
    uint8_t first = cur.bytes[0];
    if (first != 0xC4 && first != 0xC5 && first != 0x62) {
      continue;  // Not VEX/EVEX prefix.
    }
    if (!cur.mnemonic || cur.mnemonic[0] == '\0') {
      continue;
    }
    std::string key = cur.mnemonic;
    ...
    if (cur.detail) {
      ...
      for (uint8_t op_i = 0; op_i < x86.op_count; ++op_i) {
        if (x86.operands[op_i].type == X86_OP_REG) {
          if (IsYmmOrZmmReg(static_cast<x86_reg>(x86.operands[op_i].reg))) {
            uses_ymm = true;
            break;
          }
        }
    ...
    XELOGW("DEBUG: VEX emit: {} {} (guest {:08X})", cur.mnemonic,
           cur.op_str ? cur.op_str : "", guest_function);
    if (uses_ymm) {
      YMM2NEON_MISSING(cur.mnemonic);
    } else {
      VEX2NEON_MISSING(cur.mnemonic);
    }
...
X64Emitter::X64Emitter(X64Backend* backend, XbyakAllocator* allocator)
  ...
  // Use xsimd for AVX detection (handles Rosetta 2 properly)
  ...
  if (!avx_ok) {
    if (is_rosetta && cvars::rosetta_force_avx) {
      XELOGW(
          "Rosetta 2: AVX probe failed and rosetta_force_avx=TRUE, but AVX is "
          "not executable.");
      XELOGW(
          "Ignoring rosetta_force_avx to prevent SIGILL; using SSE fallback "
          "path.");
    } else {
      XELOGW(
          "AVX instructions are not executable on this host (probe failed).");
      XELOGW("Falling back to SSE-only encodings for VEX/SIMD ops.");
    }
    use_vex_ = false;
  } else if (is_rosetta) {
...
// AVX/VEX fallback helpers (SSE encodings when AVX isn't executable).
void X64Emitter::vxorps(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    xorps(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vxorps(dest, src);
}

void X64Emitter::vxorps(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
  ...
  Xbyak::CodeGenerator::vxorps(dest, src1, src2);
}

void X64Emitter::vxorps(const Xmm& dest, const Xmm& src1,
                        const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movaps(dest, src1);
    }
...
void X64Emitter::vxorpd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    xorpd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vxorpd(dest, src);
}

void X64Emitter::vxorpd(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movapd(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vxorpd(dest, src1, src2);
}

void X64Emitter::vxorpd(const Xmm& dest, const Xmm& src1,
                        const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movapd(dest, src1);
...
void X64Emitter::vmovups(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movups(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovups(dest, src);
}

void X64Emitter::vmovups(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movups(dest, src);
    return;
  }
...
void X64Emitter::vmovups(const Xbyak::Address& dest, const Xmm& src) {
  if (!use_vex_) {
    movups(dest, src);
    return;
  }
...
void X64Emitter::vmovaps(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movaps(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovaps(dest, src);
}

void X64Emitter::vmovaps(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movaps(dest, src);
    return;
  }
...
void X64Emitter::vmovaps(const Xbyak::Address& dest, const Xmm& src) {
  if (!use_vex_) {
    movaps(dest, src);
    return;
  }
...
void X64Emitter::vpxor(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    pxor(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpxor(dest, src);
}

void X64Emitter::vpxor(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    pxor(dest, src);
    return;
  }
...
void X64Emitter::vpxor(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpxor(dest, src1, src2);
}

void X64Emitter::vpxor(const Xmm& dest, const Xmm& src1,
                       const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
...
void X64Emitter::vpxord(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    pxor(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpxord(dest, dest, src);
}

void X64Emitter::vpxord(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    pxor(dest, src);
    return;
  }
...
void X64Emitter::vpxord(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpxord(dest, src1, src2);
}

void X64Emitter::vpxord(const Xmm& dest, const Xmm& src1,
                        const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
...
void X64Emitter::vpcmpeqb(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    pcmpeqb(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpcmpeqb(dest, src);
}

void X64Emitter::vpcmpeqb(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpcmpeqb(dest, src1, src2);
}

void X64Emitter::vpcmpeqb(const Xmm& dest, const Xmm& src1,
                          const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
...
void X64Emitter::vpcmpeqd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    pcmpeqd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpcmpeqd(dest, src);
}

void X64Emitter::vpcmpeqd(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpcmpeqd(dest, src1, src2);
}

void X64Emitter::vpcmpeqd(const Xmm& dest, const Xmm& src1,
                          const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
...
void X64Emitter::vcmpeqss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cmpeqss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcmpeqss(dest, src);
}

void X64Emitter::vcmpeqpd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cmpeqpd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcmpeqpd(dest, src);
}

// ---- Additional SSE fallbacks for VEX-encoded ops ----

#define VEX_BINOP_SSE_FLOAT(name, movop, sseop)                              \
  void X64Emitter::name(const Xmm& dest, const Xmm& src1, const Xmm& src2) { \
    if (!use_vex_) {                                                         \
      if (dest.getIdx() != src1.getIdx()) {                                  \
        movop(dest, src1);                                                   \
      }                                                                      \
    ...
    Xbyak::CodeGenerator::name(dest, src1, src2);                            \
  }                                                                          \
  void X64Emitter::name(const Xmm& dest, const Xmm& src1,                    \
                        const Xbyak::Address& src2) {                        \
    if (!use_vex_) {                                                         \
      if (dest.getIdx() != src1.getIdx()) {                                  \
        movop(dest, src1);                                                   \
...
#define VEX_BINOP_SSE_INT(name, sseop)                                       \
  void X64Emitter::name(const Xmm& dest, const Xmm& src1, const Xmm& src2) { \
    if (!use_vex_) {                                                         \
      if (dest.getIdx() != src1.getIdx()) {                                  \
        movdqa(dest, src1);                                                  \
...
#define VEX_BINOP_SSE_INT_MEM(name, sseop)                \
  void X64Emitter::name(const Xmm& dest, const Xmm& src1, \
                        const Xbyak::Address& src2) {     \
    if (!use_vex_) {                                      \
      if (dest.getIdx() != src1.getIdx()) {               \
...
#define VEX_UNOP_SSE(name, sseop)                          \
  void X64Emitter::name(const Xmm& dest, const Xmm& src) { \
    if (!use_vex_) {                                       \
      sseop(dest, src);                                    \
      return;                                              \
    }                                                      \
    Xbyak::CodeGenerator::name(dest, src);                 \
  }

// Float arithmetic.
VEX_BINOP_SSE_FLOAT(vaddps, movaps, addps)
VEX_BINOP_SSE_FLOAT(vaddpd, movapd, addpd)
VEX_BINOP_SSE_FLOAT(vaddsd, movapd, addsd)
VEX_BINOP_SSE_FLOAT(vaddss, movaps, addss)

VEX_BINOP_SSE_FLOAT(vsubps, movaps, subps)
VEX_BINOP_SSE_FLOAT(vsubpd, movapd, subpd)
VEX_BINOP_SSE_FLOAT(vsubsd, movapd, subsd)
VEX_BINOP_SSE_FLOAT(vsubss, movaps, subss)

VEX_BINOP_SSE_FLOAT(vmulps, movaps, mulps)
VEX_BINOP_SSE_FLOAT(vmulpd, movapd, mulpd)
VEX_BINOP_SSE_FLOAT(vmulsd, movapd, mulsd)
VEX_BINOP_SSE_FLOAT(vmulss, movaps, mulss)

VEX_BINOP_SSE_FLOAT(vdivps, movaps, divps)
VEX_BINOP_SSE_FLOAT(vdivpd, movapd, divpd)
VEX_BINOP_SSE_FLOAT(vdivsd, movapd, divsd)
VEX_BINOP_SSE_FLOAT(vdivss, movaps, divss)

VEX_BINOP_SSE_FLOAT(vmaxps, movaps, maxps)
VEX_BINOP_SSE_FLOAT(vmaxsd, movapd, maxsd)
VEX_BINOP_SSE_FLOAT(vminps, movaps, minps)
VEX_BINOP_SSE_FLOAT(vminsd, movapd, minsd)

VEX_BINOP_SSE_FLOAT(vandps, movaps, andps)
VEX_BINOP_SSE_FLOAT(vandpd, movapd, andpd)
VEX_BINOP_SSE_FLOAT(vandnps, movaps, andnps)
VEX_BINOP_SSE_FLOAT(vorps, movaps, orps)

void X64Emitter::vorps(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    orps(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vorps(dest, dest, src);
}

void X64Emitter::vorps(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    orps(dest, src);
    return;
  }
...
void X64Emitter::vandps(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    andps(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vandps(dest, dest, src);
}

void X64Emitter::vandps(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    andps(dest, src);
    return;
  }
...
void X64Emitter::vaddss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    addss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vaddss(dest, dest, src);
}

void X64Emitter::vaddss(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    addss(dest, src);
    return;
  }
...
// Compare (imm versions).
void X64Emitter::vcmpps(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                        uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movaps(dest, src1);
  ...
  Xbyak::CodeGenerator::vcmpps(dest, src1, src2, imm);
}

void X64Emitter::vcmpps(const Xmm& dest, const Xmm& src1,
...
void X64Emitter::vcmpps(const Xbyak::Opmask& mask, const Xmm& src1,
                        const Xbyak::Address& src2, uint8_t imm) {
  if (!use_vex_) {
    XELOGW("vcmpps(opmask) invoked without AVX-512; skipping.");
    return;
  }
  Xbyak::CodeGenerator::vcmpps(mask, src1, src2, imm);
}

void X64Emitter::vcmpeqps(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  vcmpps(dest, src1, src2, 0x00);
}

void X64Emitter::vcmpeqps(const Xmm& dest, const Xmm& src1,
                          const Xbyak::Address& src2) {
  vcmpps(dest, src1, src2, 0x00);
}

void X64Emitter::vcmpneqps(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  vcmpps(dest, src1, src2, 0x04);
}

void X64Emitter::vcmpgeps(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  vcmpps(dest, src1, src2, 0x0D);
}

void X64Emitter::vcmpgeps(const Xmm& dest, const Xmm& src1,
                          const Xbyak::Address& src2) {
  vcmpps(dest, src1, src2, 0x0D);
}

void X64Emitter::vcmpgtps(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  vcmpps(dest, src1, src2, 0x0E);
}

void X64Emitter::vcmpunordps(const Xmm& dest, const Xmm& src1,
                             const Xmm& src2) {
  vcmpps(dest, src1, src2, 0x03);
}

// Scalar compares.
void X64Emitter::vcomiss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    comiss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcomiss(dest, src);
}

void X64Emitter::vcomisd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    comisd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcomisd(dest, src);
}

void X64Emitter::vucomisd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    ucomisd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vucomisd(dest, src);
}

// Conversions.
void X64Emitter::vcvtdq2ps(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtdq2ps(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtdq2ps(dest, src);
}

void X64Emitter::vcvttps2dq(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cvttps2dq(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvttps2dq(dest, src);
}

void X64Emitter::vcvtsi2sd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtsi2sd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtsi2sd(dest, src);
}

void X64Emitter::vcvtsi2sd(const Xmm& dest, const Xbyak::Reg64& src) {
  if (!use_vex_) {
    cvtsi2sd(dest, src);
    return;
  }
...
void X64Emitter::vcvtsi2ss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtsi2ss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtsi2ss(dest, src);
}

void X64Emitter::vcvtsd2si(const Xbyak::Reg32& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtsd2si(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtsd2si(dest, src);
}

void X64Emitter::vcvtsd2si(const Xbyak::Reg64& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtsd2si(dest, src);
    return;
  }
...
void X64Emitter::vcvttsd2si(const Xbyak::Reg32& dest, const Xmm& src) {
  if (!use_vex_) {
    cvttsd2si(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvttsd2si(dest, src);
}

void X64Emitter::vcvttsd2si(const Xbyak::Reg64& dest, const Xmm& src) {
  if (!use_vex_) {
    cvttsd2si(dest, src);
    return;
  }
...
void X64Emitter::vcvttss2si(const Xbyak::Reg32& dest, const Xmm& src) {
  if (!use_vex_) {
    cvttss2si(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvttss2si(dest, src);
}

void X64Emitter::vcvttss2si(const Xbyak::Reg64& dest, const Xmm& src) {
  if (!use_vex_) {
    cvttss2si(dest, src);
    return;
  }
...
void X64Emitter::vcvtss2si(const Xbyak::Reg32& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtss2si(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtss2si(dest, src);
}

void X64Emitter::vcvtss2si(const Xbyak::Reg64& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtss2si(dest, src);
    return;
  }
...
void X64Emitter::vcvtsd2ss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    cvtsd2ss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vcvtsd2ss(dest, src);
}

void X64Emitter::vcvttss2si(const Xmm& dest, const Xmm& src) {
  // No direct SSE form that writes to XMM; this overload should not be used.
  XELOGW("vcvttss2si(Xmm,Xmm) invoked - no SSE/AVX encoding available");
  (void)dest;
  (void)src;
}

// Moves.
void X64Emitter::vmovapd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movapd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovapd(dest, src);
}

void X64Emitter::vmovapd(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movapd(dest, src);
    return;
  }
...
void X64Emitter::vmovdqa(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movdqa(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovdqa(dest, src);
}

void X64Emitter::vmovdqa(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movdqa(dest, src);
    return;
  }
...
void X64Emitter::vmovdqa(const Xbyak::Address& dest, const Xmm& src) {
  if (!use_vex_) {
    movdqa(dest, src);
    return;
  }
...
void X64Emitter::vmovdqa(const Xbyak::Address& dest, const Xbyak::Ymm& src) {
  if (!use_vex_) {
    // No YMM stores in SSE-only mode; store low 128 as a fallback.
    movdqa(dest, Xmm(src.getIdx()));
    return;
  }
  Xbyak::CodeGenerator::vmovdqa(dest, src);
}

void X64Emitter::vmovd(const Xmm& dest, const Xbyak::Reg32& src) {
  if (!use_vex_) {
    movd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovd(dest, src);
}

void X64Emitter::vmovd(const Xbyak::Reg32& dest, const Xmm& src) {
  if (!use_vex_) {
    movd(dest, src);
    return;
  }
...
void X64Emitter::vmovq(const Xmm& dest, const Xbyak::Reg64& src) {
  if (!use_vex_) {
    movq(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovq(dest, src);
}

void X64Emitter::vmovq(const Xbyak::Reg64& dest, const Xmm& src) {
  if (!use_vex_) {
    movq(dest, src);
    return;
  }
...
void X64Emitter::vmovsd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movsd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovsd(dest, src);
}

void X64Emitter::vmovsd(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movsd(dest, src);
    return;
  }
...
void X64Emitter::vmovsd(const Xbyak::Address& dest, const Xmm& src) {
  if (!use_vex_) {
    movsd(dest, src);
    return;
  }
...
void X64Emitter::vmovss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    movss(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vmovss(dest, src);
}

void X64Emitter::vmovss(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movss(dest, src);
    return;
  }
...
void X64Emitter::vmovss(const Xbyak::Address& dest, const Xmm& src) {
  if (!use_vex_) {
    movss(dest, src);
    return;
  }
...
// Broadcast.
void X64Emitter::vbroadcastss(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    movss(dest, src);
    shufps(dest, dest, 0x00);
    return;
  }
  Xbyak::CodeGenerator::vbroadcastss(dest, src);
}

void X64Emitter::vbroadcastss(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
...
// Shuffles.
void X64Emitter::vshufps(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                         uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movaps(dest, src1);
    }
    shufps(dest, src2, imm);
    return;
  }
  Xbyak::CodeGenerator::vshufps(dest, src1, src2, imm);
}

void X64Emitter::vshufps(const Xmm& dest, const Xmm& src1,
                         const Xbyak::Address& src2, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movaps(dest, src1);
...
void X64Emitter::vshufps(const Xmm& dest, const Xmm& src, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src.getIdx()) {
      movaps(dest, src);
    }
    shufps(dest, dest, imm);
    return;
  }
  Xbyak::CodeGenerator::vshufps(dest, src, imm);
}

// Blend operations (mask semantics assume compare-style all-0/all-1).
void X64Emitter::vblendvps(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                           const Xmm& mask) {
  if (!use_vex_) {
    auto mask_mem =
        GetBackendCtxPtr(offsetof(X64BackendContext, helper_scratch_xmms[0]));
    auto temp_mem =
        GetBackendCtxPtr(offsetof(X64BackendContext, helper_scratch_xmms[1]));
    movdqu(mask_mem, mask);
    movaps(dest, mask);
    andnps(dest, src1);
    movdqu(temp_mem, dest);
    movdqu(dest, mask_mem);
    andps(dest, src2);
    orps(dest, temp_mem);
    return;
  }
  Xbyak::CodeGenerator::vblendvps(dest, src1, src2, mask);
}

void X64Emitter::vblendvps(const Xmm& dest, const Xmm& src1,
                           const Xbyak::Address& src2, const Xmm& mask) {
  if (!use_vex_) {
    auto mask_mem =
        GetBackendCtxPtr(offsetof(X64BackendContext, helper_scratch_xmms[0]));
    auto temp_mem =
        GetBackendCtxPtr(offsetof(X64BackendContext, helper_scratch_xmms[1]));
    movdqu(mask_mem, mask);
    movups(dest, mask_mem);
    andnps(dest, src1);
    movups(temp_mem, dest);
    movups(dest, src2);
    andps(dest, mask_mem);
    orps(dest, temp_mem);
    return;
...
void X64Emitter::vblendvb(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                          const Xmm& mask) {
  if (!use_vex_) {
    auto mask_mem =
        GetBackendCtxPtr(offsetof(X64BackendContext, helper_scratch_xmms[0]));
  ...
  vblendvb(dest, src1, src2, mask);
}

void X64Emitter::vblendw(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                         uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
    pblendw(dest, src2, imm);
    return;
  }
  vblendw(dest, src1, src2, imm);
}

void X64Emitter::vblendd(const Xmm& dest, const Xmm& src1, const Xmm& src2,
                         uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  vblendd(dest, src1, src2, imm);
}

// Shuffles / unpack / pack.
void X64Emitter::vpshufb(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpshufb(dest, src1, src2);
}

void X64Emitter::vpshufb(const Xmm& dest, const Xmm& src1,
                         const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
...
void X64Emitter::vpshufd(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    pshufd(dest, src1, imm);
    return;
  }
  Xbyak::CodeGenerator::vpshufd(dest, src1, imm);
}

void X64Emitter::vpshufhw(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    pshufhw(dest, src1, imm);
    return;
  }
  Xbyak::CodeGenerator::vpshufhw(dest, src1, imm);
}

void X64Emitter::vpshuflw(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    pshuflw(dest, src1, imm);
    return;
  }
  Xbyak::CodeGenerator::vpshuflw(dest, src1, imm);
}

void X64Emitter::vpunpcklbw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpcklbw(dest, src1, src2);
}

void X64Emitter::vpunpcklbw(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    punpcklbw(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpunpcklbw(dest, dest, src);
}

void X64Emitter::vpunpcklwd(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpcklwd(dest, src1, src2);
}

void X64Emitter::vpunpcklwd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    punpcklwd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpunpcklwd(dest, dest, src);
}

void X64Emitter::vpunpckldq(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpckldq(dest, src1, src2);
}

void X64Emitter::vpunpcklqdq(const Xmm& dest, const Xmm& src1,
                             const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
    punpcklqdq(dest, src2);
    return;
  }
  Xbyak::CodeGenerator::vpunpcklqdq(dest, src1, src2);
}

void X64Emitter::vpunpckhbw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpckhbw(dest, src1, src2);
}

void X64Emitter::vpunpckhwd(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpckhwd(dest, src1, src2);
}

void X64Emitter::vpunpckhdq(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpunpckhdq(dest, src1, src2);
}

void X64Emitter::vpunpckhqdq(const Xmm& dest, const Xmm& src1,
                             const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
    punpckhqdq(dest, src2);
    return;
  }
  Xbyak::CodeGenerator::vpunpckhqdq(dest, src1, src2);
}

void X64Emitter::vpackssdw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpackssdw(dest, src1, src2);
}

void X64Emitter::vpacksswb(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpacksswb(dest, src1, src2);
}

void X64Emitter::vpackusdw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpackusdw(dest, src1, src2);
}

void X64Emitter::vpackuswb(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpackuswb(dest, src1, src2);
}

// Integer arithmetic and logic.
VEX_BINOP_SSE_INT(vpaddb, paddb)
VEX_BINOP_SSE_INT(vpaddw, paddw)
VEX_BINOP_SSE_INT(vpaddd, paddd)
VEX_BINOP_SSE_INT(vpaddsb, paddsb)
VEX_BINOP_SSE_INT(vpaddsw, paddsw)
VEX_BINOP_SSE_INT(vpaddusb, paddusb)
VEX_BINOP_SSE_INT(vpaddusw, paddusw)

void X64Emitter::vpaddb(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    paddb(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpaddb(dest, dest, src);
}

void X64Emitter::vpaddd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    paddd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpaddd(dest, dest, src);
}

void X64Emitter::vpaddd(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    paddd(dest, src);
    return;
  }
...
void X64Emitter::vpaddd(const Xmm& dest, const Xmm& src1,
                        const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  Xbyak::CodeGenerator::vpaddd(dest, src1, src2);
}

VEX_BINOP_SSE_INT(vpsubb, psubb)
VEX_BINOP_SSE_INT(vpsubw, psubw)
VEX_BINOP_SSE_INT(vpsubd, psubd)
VEX_BINOP_SSE_INT(vpsubsb, psubsb)
VEX_BINOP_SSE_INT(vpsubsw, psubsw)
VEX_BINOP_SSE_INT(vpsubusb, psubusb)
VEX_BINOP_SSE_INT(vpsubusw, psubusw)

void X64Emitter::vpsubd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    psubd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpsubd(dest, dest, src);
}

VEX_BINOP_SSE_INT(vpand, pand)
VEX_BINOP_SSE_INT(vpor, por)

void X64Emitter::vpand(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    pand(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpand(dest, dest, src);
}

void X64Emitter::vpand(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    pand(dest, src);
    return;
  }
...
void X64Emitter::vpand(const Xmm& dest, const Xmm& src1,
                       const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  Xbyak::CodeGenerator::vpand(dest, src1, src2);
}

void X64Emitter::vpor(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    por(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpor(dest, dest, src);
}

void X64Emitter::vpor(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    por(dest, src);
    return;
  }
...
void X64Emitter::vpandn(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpandn(dest, src1, src2);
}

VEX_BINOP_SSE_INT(vpcmpeqw, pcmpeqw)
VEX_BINOP_SSE_INT(vpcmpgtb, pcmpgtb)
VEX_BINOP_SSE_INT(vpcmpgtw, pcmpgtw)
VEX_BINOP_SSE_INT(vpcmpgtd, pcmpgtd)

void X64Emitter::vpcmpgtb(const Xmm& dest, const Xmm& src1,
                          const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  Xbyak::CodeGenerator::vpcmpgtb(dest, src1, src2);
}

// Shifts (immediates).
void X64Emitter::vpsllw(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsllw(dest, src1, imm);
}

void X64Emitter::vpsllw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsllw(dest, src1, src2);
}

void X64Emitter::vpslld(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpslld(dest, src1, imm);
}

void X64Emitter::vpslld(const Xmm& dest, uint8_t imm) {
  if (!use_vex_) {
    pslld(dest, imm);
    return;
  }
  Xbyak::CodeGenerator::vpslld(dest, dest, imm);
}

void X64Emitter::vpslld(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpslld(dest, src1, src2);
}

void X64Emitter::vpsrlw(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrlw(dest, src1, imm);
}

void X64Emitter::vpsrlw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrlw(dest, src1, src2);
}

void X64Emitter::vpsrld(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrld(dest, src1, imm);
}

void X64Emitter::vpsrld(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrld(dest, src1, src2);
}

void X64Emitter::vpsraw(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsraw(dest, src1, imm);
}

void X64Emitter::vpsraw(const Xmm& dest, uint8_t imm) {
  if (!use_vex_) {
    psraw(dest, imm);
    return;
  }
  Xbyak::CodeGenerator::vpsraw(dest, imm);
}

void X64Emitter::vpsraw(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsraw(dest, src1, src2);
}

void X64Emitter::vpsrad(const Xmm& dest, const Xmm& src1, uint8_t imm) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrad(dest, src1, imm);
}

void X64Emitter::vpsrad(const Xmm& dest, uint8_t imm) {
  if (!use_vex_) {
    psrad(dest, imm);
    return;
  }
  Xbyak::CodeGenerator::vpsrad(dest, dest, imm);
}

void X64Emitter::vpsrad(const Xmm& dest, const Xmm& src1, const Xmm& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
    }
  ...
  Xbyak::CodeGenerator::vpsrad(dest, src1, src2);
}

// Average/min/max.
VEX_BINOP_SSE_INT(vpavgb, pavgb)
VEX_BINOP_SSE_INT(vpavgw, pavgw)
VEX_BINOP_SSE_INT(vpmaxub, pmaxub)
VEX_BINOP_SSE_INT(vpmaxuw, pmaxuw)
VEX_BINOP_SSE_INT(vpmaxsb, pmaxsb)
VEX_BINOP_SSE_INT(vpmaxsw, pmaxsw)
VEX_BINOP_SSE_INT(vpmaxsd, pmaxsd)
VEX_BINOP_SSE_INT(vpmaxud, pmaxud)
VEX_BINOP_SSE_INT(vpminub, pminub)
VEX_BINOP_SSE_INT(vpminuw, pminuw)
VEX_BINOP_SSE_INT(vpminsb, pminsb)
VEX_BINOP_SSE_INT(vpminsw, pminsw)
VEX_BINOP_SSE_INT(vpminsd, pminsd)
VEX_BINOP_SSE_INT(vpminud, pminud)

void X64Emitter::vpminud(const Xmm& dest, const Xmm& src1,
                         const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  Xbyak::CodeGenerator::vpminud(dest, src1, src2);
}

// Sign.
void X64Emitter::vpsignb(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    psignb(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpsignb(dest, src);
}

void X64Emitter::vpsignw(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    psignw(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpsignw(dest, src);
}

void X64Emitter::vpsignd(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    psignd(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vpsignd(dest, src);
}

// Test.
void X64Emitter::vptest(const Xmm& dest, const Xmm& src) {
  if (!use_vex_) {
    ptest(dest, src);
    return;
  }
  Xbyak::CodeGenerator::vptest(dest, src);
}

void X64Emitter::vptest(const Xmm& dest, const Xbyak::Address& src) {
  if (!use_vex_) {
    ptest(dest, src);
    return;
  }
...
// MXCSR load/store.
void X64Emitter::vldmxcsr(const Xbyak::Address& src) {
  if (!use_vex_) {
    ldmxcsr(src);
    return;
  }
  Xbyak::CodeGenerator::vldmxcsr(src);
}

void X64Emitter::vstmxcsr(const Xbyak::Address& dest) {
  if (!use_vex_) {
    stmxcsr(dest);
    return;
  }
  Xbyak::CodeGenerator::vstmxcsr(dest);
}

#undef VEX_BINOP_SSE_FLOAT
#undef VEX_BINOP_SSE_INT
#undef VEX_BINOP_SSE_INT_MEM
#undef VEX_UNOP_SSE
...
bool X64Emitter::Emit(GuestFunction* function, HIRBuilder* builder,
  ...
  // Start performance monitoring for this compilation
  auto perf_monitor = backend_->GetPerformanceMonitor();
  if (perf_monitor) {
    perf_monitor->BeginFunctionCompilation(function->address());
  }
  ...
  // Central VEX/EVEX emission logging to surface missing translation hooks.
  LogVexTranslationMissing(getCode(), getSize(), function->address());

  // Copy the final code to the cache and relocate it.
  *out_code_size = getSize();
  *out_code_address = Emplace(func_info, function);

  // End performance monitoring for this compilation
  if (perf_monitor) {
    ...
    perf_monitor->EndFunctionCompilation(function->address(), *out_code_size,
                                         instruction_count);
  }
...
void X64Emitter::MarkSourceOffset(const Instr* i) {
  ...
  if (::cvars::track_guest_pc || ::cvars::trace_gpu_bootstrap) {
    mov(dword[GetContextReg() + offsetof(ppc::PPCContext, current_pc)],
        entry->guest_address);
  }
  if (::cvars::guest_pc_watch &&
      entry->guest_address == ::cvars::guest_pc_watch) {
    CallNative(DebugGuestPcWatch, entry->guest_address);
  }
  if (::cvars::guest_pc_watch2 &&
      entry->guest_address == ::cvars::guest_pc_watch2) {
    CallNative(DebugGuestPcWatch, entry->guest_address);
  }
  if (::cvars::guest_pc_watch3 &&
      entry->guest_address == ::cvars::guest_pc_watch3) {
    CallNative(DebugGuestPcWatch, entry->guest_address);
  }
  if (::cvars::guest_pc_watch4 &&
      entry->guest_address == ::cvars::guest_pc_watch4) {
    CallNative(DebugGuestPcWatch, entry->guest_address);
  }
  if (::cvars::log_main_loop_state && (entry->guest_address == 0x82096008 ||
                                       entry->guest_address == 0x82097950)) {
    CallNative(DebugMainLoopPc, entry->guest_address);
  }
...
void X64Emitter::Call(const hir::Instr* instr, GuestFunction* function) {
  ...
  if (fn->machine_code()) {
    if (!(instr->flags & hir::CALL_TAIL)) {
      ...
      call(reinterpret_cast<void*>(fn->machine_code()));
      ...
      PopStackpoint();
      jmp(reinterpret_cast<void*>(fn->machine_code()), T_NEAR);
    }
...
namespace {
constexpr bool IsTrackedBootstrapImportOrdinal(uint16_t ordinal) {
  switch (ordinal) {
    case 0x0194:  // XexCheckExecutablePrivilege
...
uint64_t LogGpuImportThunkCall(void* raw_context, uint64_t function_ptr) {
  auto* function = reinterpret_cast<const GuestFunction*>(function_ptr);
  if (!function) {
    return 0;
  }
  auto* export_data = function->export_data();
  if (!export_data ||
      !IsTrackedBootstrapImportOrdinal(export_data->ordinal)) {
    return 0;
  }
  auto* context = reinterpret_cast<ppc::PPCContext*>(raw_context);
  uint32_t pc = context ? context->current_pc : 0;
  uint32_t lr = context ? static_cast<uint32_t>(context->lr) : 0;
  uint32_t thread_id = (context && context->thread_state)
                           ? context->thread_state->thread_id()
                           : 0;
  kernel::xboxkrnl::RecordGpuBootstrapStaticThunkCall(export_data->ordinal, pc,
                                                      lr, thread_id);
  const bool trace_bootstrap = ::cvars::trace_gpu_bootstrap;
  if (!trace_bootstrap && !::cvars::log_gpu_import_thunks) {
    return 0;
  }
  static std::atomic<uint64_t> call_count{0};
  uint64_t call_id = call_count.fetch_add(1) + 1;
  uint32_t every_n = ::cvars::log_gpu_import_thunks_every_n;
  if (!trace_bootstrap && every_n > 1 && (call_id % every_n) != 0) {
    return 0;
  }
  uint64_t r3 = context ? context->r[3] : 0;
  uint64_t r4 = context ? context->r[4] : 0;
  uint64_t r5 = context ? context->r[5] : 0;
  uint64_t r6 = context ? context->r[6] : 0;
  uint64_t r7 = context ? context->r[7] : 0;
  uint64_t r8 = context ? context->r[8] : 0;
  const char* caller_module_name = "<unknown>";
  if (context && context->thread_state && context->thread_state->processor() &&
      pc) {
    auto* caller_module = context->thread_state->processor()->LookupModule(pc);
    if (caller_module) {
      caller_module_name = caller_module->name().c_str();
    }
  }
  XELOGW(
      "RING BUFFER: bootstrap import thunk dispatch #{}: caller_module={} "
      "pc={:08X} thread_id={} export_module=xboxkrnl ordinal=0x{:03X} name={} "
      "lr={:08X} args[r3..r8]={:016X},{:016X},{:016X},{:016X},{:016X},{:016X}",
      call_id, caller_module_name, pc, thread_id, export_data->ordinal,
      function->name(), lr, r3, r4, r5, r6, r7, r8);
  return 0;
}

uint64_t LogExternThunkCall(void* raw_context, uint64_t function_ptr) {
  if (!::cvars::log_extern_calls) {
    return 0;
  }
  auto* function = reinterpret_cast<const GuestFunction*>(function_ptr);
  if (!function) {
  ...
  static std::atomic<uint64_t> call_count{0};
  uint64_t call_id = call_count.fetch_add(1) + 1;
  uint32_t every_n = ::cvars::log_extern_calls_every_n;
  ...
  uint32_t thread_id = (context && context->thread_state)
                           ? context->thread_state->thread_id()
                           : 0;
  const Export* export_data = function->export_data();
  XELOGW(
      "DEBUG: Extern thunk call #{}: {} export={} ordinal=0x{:03X} pc={:08X} "
      "lr={:08X} r3={:016X} r4={:016X} r5={:016X} thread_id={}",
      call_id, function->name(), export_data ? export_data->name : "<null>",
      export_data ? export_data->ordinal : 0, pc, lr, r3, r4, r5, thread_id);
  return 0;
}
}  // namespace
...
void X64Emitter::CallExtern(const hir::Instr* instr, const Function* function) {
  ...
  if (function->behavior() == Function::Behavior::kBuiltin) {
    ...
    if (::cvars::log_extern_calls) {
      CallNative(LogExternThunkCall,
                 reinterpret_cast<uint64_t>(extern_function));
    }
    if (::cvars::log_gpu_import_thunks || ::cvars::trace_gpu_bootstrap) {
      auto export_data = extern_function->export_data();
      if (export_data &&
          IsTrackedBootstrapImportOrdinal(export_data->ordinal)) {
        CallNative(LogGpuImportThunkCall,
                   reinterpret_cast<uint64_t>(extern_function));
      }
    }
...
uintptr_t X64Emitter::PlaceConstData() {
  XELOGI("X64Emitter::PlaceConstData() starting...");
  XELOGI("X64Emitter::PlaceConstData() target size: {} bytes", kConstDataSize);
  XELOGI("X64Emitter::PlaceConstData() page size: {} bytes",
         memory::page_size());
  XELOGI("X64Emitter::PlaceConstData() rounded size: {} bytes",
         xe::round_up(kConstDataSize, memory::page_size()));

  // Try dynamic allocation first as fallback for macOS
  void* mem = memory::AllocFixed(nullptr, kConstDataSize,
                                 memory::AllocationType::kReserveCommit,
                                 memory::PageAccess::kReadWrite);
  if (!mem) {
    XELOGI(
        "X64Emitter::PlaceConstData() dynamic allocation failed, trying fixed "
        "address allocation...");
    ...
    while (!mem) {
      ...
      if (!mem) {
        XELOGI(
            "X64Emitter::PlaceConstData() fixed allocation failed, moving to "
            "next location...");
      }

      ptr += kConstDataIncrement;

      // Prevent infinite loop - if we've tried too many locations, abort
      if (attempt_count > 10) {
        XELOGE(
            "X64Emitter::PlaceConstData() failed to allocate after 10 attempts "
            "- giving up");
        return 0;
      }
    }
  ...
  if (!mem) {
    XELOGI("X64Emitter::PlaceConstData() all allocation methods failed!");
    return 0;
  }

  XELOGI("X64Emitter::PlaceConstData() allocated at: 0x{:08X}",
         reinterpret_cast<uintptr_t>(mem));

  // The pointer must not be greater than 31 bits.
  if (reinterpret_cast<uintptr_t>(mem) & ~0x7FFFFFFF) {
    XELOGE("X64Emitter::PlaceConstData() allocated pointer exceeds 31 bits!");
    return 0;
  }

  XELOGI("X64Emitter::PlaceConstData() copying {} bytes of constant data...",
         sizeof(xmm_consts));
  std::memcpy(mem, xmm_consts, sizeof(xmm_consts));
  /*
    set each 32-bit element of the constant XMMVRsqrteTableBase to be
    address of start of the constant XMMVRsqrteTableStart this
   */
  ...
  *vrsqrte_table_base = vec128i(ptr_to_vrsqrte_table32);
  ...
  XELOGI(
      "X64Emitter::PlaceConstData() completed successfully, returning: "
      "0x{:08X}",
      reinterpret_cast<uintptr_t>(mem));
...
void X64Emitter::PushStackpoint() {
  ...
  Xbyak::Label& overflowed_stackpoints =
      AddToTail([](X64Emitter& e, Xbyak::Label& our_tail_label) {
        ...
        e.CallNativeSafe(
            reinterpret_cast<void*>(X64Emitter::HandleStackpointOverflowError));
      });
...
void X64Emitter::vpaddb(const Xmm& dest, const Xmm& src1,
                        const Xbyak::Address& src2) {
  if (!use_vex_) {
    if (dest.getIdx() != src1.getIdx()) {
      movdqa(dest, src1);
  ...
  Xbyak::CodeGenerator::vpaddb(dest, src1, src2);
}
}  // namespace x64
}  // namespace backend
}  // namespace cpu
