...
struct CONVERT_I32_F32
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    ...
    if (i.instr->flags == ROUND_TO_ZERO) {
      e.vcvttss2si(i.dest.reg(), src1);
    } else {
      e.vcvtss2si(i.dest.reg(), src1);
    }
  }
};
struct CONVERT_I32_F64
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    ...
    if (i.instr->flags == ROUND_TO_ZERO) {
      e.vcvttsd2si(i.dest.reg(), e.xmm1);
    } else {
      e.vcvtsd2si(i.dest.reg(), e.xmm1);
    }
  }
};
struct CONVERT_I64_F64
  ...
  static void Emit(X64Emitter& e, const EmitArgType& i) {
    ...
    // TODO(benvanik): saturation check? cvtt* (trunc?)
    if (i.instr->flags == ROUND_TO_ZERO) {
      e.vcvttsd2si(i.dest.reg(), src1);
    } else {
      e.vcvtsd2si(i.dest.reg(), src1);
    }
