...
#ifndef XENIA_CPU_BACKEND_X64_X64_BACKEND_MAC_H_
#define XENIA_CPU_BACKEND_X64_X64_BACKEND_MAC_H_
...
DECLARE_bool(enable_performance_monitoring);
DECLARE_int32(perf_monitor_hot_threshold);
DECLARE_bool(perf_monitor_enable_vtune);
DECLARE_bool(perf_monitor_detailed_metrics);
namespace xe {
class Exception;
}  // namespace xe
namespace xe {
...
struct ReserveHelper {
  static constexpr size_t kBlockCount = RESERVE_NUM_ENTRIES / 64;
  uint64_t blocks[kBlockCount] = {};

  ReserveHelper() = default;
  ~ReserveHelper() { SecureClear(); }

 private:
  void SecureClear() {
    volatile uint64_t* blocks_volatile = blocks;
...
class X64Backend : public Backend {
 ...
 private:
  ...
  std::unique_ptr<X64PerformanceMonitor> performance_monitor_;

  // Advanced optimization systems
  ThunkOptimizer* thunk_optimizer_;

  // CPU feature access
  const CPUInfo& GetCPUInfo() const;

  void* reserved_store_32_helper_ = nullptr;
  void* reserved_store_64_helper_ = nullptr;
  void* vrsqrtefp_vector_helper_ = nullptr;
  void* vrsqrtefp_scalar_helper_ = nullptr;
  void* frsqrtefp_helper_ = nullptr;

 public:
  // Performance monitoring access
  X64PerformanceMonitor* GetPerformanceMonitor() const {
    return performance_monitor_.get();
  }
  // Thunk optimizer access
  ThunkOptimizer* GetThunkOptimizer() const { return thunk_optimizer_; }
...
#endif  // XENIA_CPU_BACKEND_X64_X64_BACKEND_MAC_H_
