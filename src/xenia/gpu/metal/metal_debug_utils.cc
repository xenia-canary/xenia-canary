...
MetalDebugUtils::~MetalDebugUtils() { LogStatistics(); }
...
void MetalDebugUtils::BeginProgrammaticCapture() {
  ...
  std::string filename =
      capture_dir
          ? fmt::format("{}/gpu_capture_{:04d}.gputrace", capture_dir,
                        captured_count_++)
...
void MetalDebugUtils::ValidateDrawState(uint32_t draw_index,
                                        RegisterFile* register_file) {
  if (!validation_enabled_) return;

  // Check scissor
  uint32_t scissor_tl =
      register_file->values[XE_GPU_REG_PA_SC_SCREEN_SCISSOR_TL];
  uint32_t scissor_x = scissor_tl & 0x7FFF;
  uint32_t scissor_y = (scissor_tl >> 16) & 0x7FFF;

  uint32_t scissor_br =
      register_file->values[XE_GPU_REG_PA_SC_SCREEN_SCISSOR_BR];
  uint32_t scissor_width = (scissor_br & 0x7FFF) - scissor_x;
  uint32_t scissor_height = ((scissor_br >> 16) & 0x7FFF) - scissor_y;

  if (scissor_width == 0 || scissor_height == 0) {
    XELOGW("Draw {} has zero scissor: {}x{} at ({},{})", draw_index,
           scissor_width, scissor_height, scissor_x, scissor_y);
    zero_scissor_count_++;
  }

  // Check viewport
  float viewport_scale_x = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_XSCALE]);
  float viewport_scale_y = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_YSCALE]);
  float vp_width = std::abs(viewport_scale_x * 2.0f);
  float vp_height = std::abs(viewport_scale_y * 2.0f);

  if (vp_width <= 0 || vp_height <= 0) {
    XELOGW("Draw {} has invalid viewport: {}x{}", draw_index, vp_width,
           vp_height);
  ...
  if (dump_state_) {
    XELOGI("Draw {} state: scissor={}x{}+{}+{}, viewport={}x{}", draw_index,
           scissor_width, scissor_height, scissor_x, scissor_y, vp_width,
           vp_height);
  }
}

void MetalDebugUtils::ValidateDescriptorHeap(MTL::Buffer* heap,
                                             const std::string& name) {
  if (!validation_enabled_ || !heap) return;

  XELOGI("ValidateDescriptorHeap: Checking {} heap at buffer {:p}", name,
         (void*)heap);
  ...
  for (size_t i = 0; i < 32; ++i) {
    if (heap_data[i].gpuVA != 0 || heap_data[i].textureViewID != 0 ||
        heap_data[i].metadata != 0) {
      has_valid_entry = true;
      valid_count++;

      if (dump_state_) {
        XELOGI("{} heap[{}]: gpuVA=0x{:x}, tex=0x{:x}, meta=0x{:x}", name, i,
               heap_data[i].gpuVA, heap_data[i].textureViewID,
...
void MetalDebugUtils::LabelPipeline(MTL::RenderPipelineState* pipeline,
  ...
  std::string label =
      FormatPipelineLabel(vertex_shader_hash, pixel_shader_hash);
...
void MetalDebugUtils::LabelBuffer(MTL::Buffer* buffer,
                                  const std::string& name) {
...
void MetalDebugUtils::DumpTexture(MTL::Texture* texture,
                                  const std::string& name) {
  ...
  std::string filename =
      texture_dir ? fmt::format("{}/xenia_tex_{}_{:04d}_{}x{}.raw", texture_dir,
                                name, captured_count_, width, height)
                  : fmt::format("/tmp/xenia_tex_{}_{:04d}_{}x{}.raw", name,
                                captured_count_, width, height);
...
void MetalDebugUtils::DumpDescriptorHeap(MTL::Buffer* heap,
                                         const std::string& name) {
  ...
  for (size_t i = 0; i < 32; ++i) {
    if (heap_data[i].gpuVA != 0 || heap_data[i].textureViewID != 0 ||
        heap_data[i].metadata != 0) {
      XELOGI("  [{}]: gpuVA=0x{:016x} tex=0x{:016x} meta=0x{:016x}", i,
             heap_data[i].gpuVA, heap_data[i].textureViewID,
             heap_data[i].metadata);
    }
  }
...
void MetalDebugUtils::DumpViewportScissor(RegisterFile* register_file) {
  ...
  // Viewport
  float vp_xscale = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_XSCALE]);
  float vp_yscale = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_YSCALE]);
  float vp_xoffset = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_XOFFSET]);
  float vp_yoffset = *reinterpret_cast<float*>(
      &register_file->values[XE_GPU_REG_PA_CL_VPORT_YOFFSET]);

  XELOGI("Viewport: scale=({}, {}), offset=({}, {})", vp_xscale, vp_yscale,
         vp_xoffset, vp_yoffset);
  ...
  XELOGI("Scissor: ({}, {}) to ({}, {}) = {}x{}", sx, sy, ex, ey, ex - sx,
         ey - sy);
}
...
std::string MetalDebugUtils::FormatPipelineLabel(uint64_t vs_hash,
                                                 uint64_t ps_hash) {
...
bool MetalDebugUtils::CheckTextureIsEmpty(MTL::Texture* texture) {
  ...
  // Check center pixel
  uint32_t pixel[4] = {0};
  texture->getBytes(pixel, 16, MTL::Region::Make2D(width / 2, height / 2, 1, 1),
                    0);
