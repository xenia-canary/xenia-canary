...
class MetalObjectTracker {
 public:
  ...
  void TrackCreation(void* obj, const std::string& type,
    ...
    XELOGI(
        "MetalObjectTracker: Created {} at {} on thread {} [{}] (alive: {}, "
        "total: {})",
        type, obj, info.thread_name,
        std::hash<std::thread::id>{}(info.thread_id), alive_count_.load(),
        total_created_.load());
  }
  ...
  void TrackRelease(void* obj) {
    ...
    XELOGI(
        "MetalObjectTracker: Released {} at {} on thread {} (lifetime: {}ms, "
        "alive: {})",
    ...
    if (info.thread_id != std::this_thread::get_id()) {
      XELOGW(
          "MetalObjectTracker: {} created on thread {} but released on thread "
          "{}",
...
class TrackedMetalObject {
 public:
  TrackedMetalObject(T* obj, const std::string& type,
                     const std::string& location = "")
      : obj_(obj), type_(type) {
    if (obj_) {
