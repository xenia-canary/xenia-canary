...
void LogMetalRenderTargetTopLeftPixels(
  ...
  if (is_depth_stencil) {
    ...
    if (!depth_pipeline || !depth_msaa_pipeline) {
      ...
      if (!lib) {
        XELOGE("{}: failed to compile depth readback MSL: {}", tag,
      ...
      if (depth_ms_fn) {
        depth_msaa_pipeline =
            device->newComputePipelineState(depth_ms_fn, &error);
        depth_ms_fn->release();
      }
      lib->release();
      if (!depth_pipeline || !depth_msaa_pipeline) {
    ...
    encoder->setComputePipelineState(is_msaa ? depth_msaa_pipeline
                                             : depth_pipeline);
...
void ScheduleEdramDumpColorSamples(MTL::CommandBuffer* cmd, MTL::Texture* tex,
  ...
  MTL::Buffer* buffer =
      device->newBuffer(bytes_per_pixel * uint32_t(sample_coords.size()),
  ...
  for (size_t i = 0; i < sample_coords.size(); ++i) {
    blit->copyFromTexture(
        ...
        MTL::Size::Make(1, 1, 1), buffer, uint32_t(i) * bytes_per_pixel,
        bytes_per_pixel, 0);
  }
  blit->endEncoding();
  ...
  cmd->addCompletedHandler([buffer, bytes_per_pixel, format, dump_format,
                            rt_key_value, sample_coords,
                            sample_labels](MTL::CommandBuffer*) {
