...
void ShaderInterpreter::ExecuteAluInstruction(ucode::AluInstruction instr) {
  ...
  if (vector_result_write_mask || vector_opcode_info.changed_state) {
    ...
    for (uint32_t i = 0; i < 3; ++i) {
      ...
      for (uint32_t j = 0; j < 4; ++j) {
        ...
        uint32_t vector_src_component_bits;
        std::memcpy(&vector_src_component_bits, &vector_src_component,
                    sizeof(vector_src_component_bits));
        vector_src_component_bits =
            (vector_src_component_bits & vector_src_absolute_mask) ^
            vector_src_negate_bit;
        std::memcpy(&vector_src_component, &vector_src_component_bits,
                    sizeof(vector_src_component));
  ...
  if (scalar_operand_component_count) {
    ...
    for (uint32_t i = 0; i < scalar_operand_component_count; ++i) {
      ...
      uint32_t scalar_operand_bits;
      std::memcpy(&scalar_operand_bits, &scalar_operand,
                  sizeof(scalar_operand_bits));
      scalar_operand_bits =
          (scalar_operand_bits & scalar_src_absolute_mask) ^
          scalar_src_negate_bit;
      std::memcpy(&scalar_operand, &scalar_operand_bits,
                  sizeof(scalar_operand));
  ...
  if (needed_dwords) {
    ...
    switch (instr.data_format()) {
      ...
      case xenos::VertexFormat::k_32_32_32_FLOAT: {
        for (uint32_t i = 0; i < 4; ++i) {
          std::memcpy(&result[i], &data[i], sizeof(result[i]));
        }
      } break;
      default:
