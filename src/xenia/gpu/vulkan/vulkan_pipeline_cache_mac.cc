...
VulkanShader* VulkanPipelineCache::LoadShader(xenos::ShaderType shader_type,
  ...
  static std::atomic<uint64_t> shader_load_count{0};
  uint64_t shader_id = shader_load_count.fetch_add(1) + 1;
  if (shader_id <= 10 || (shader_id % 200) == 0) {
    ...
    XELOGI(
        "DEBUG: VulkanPipelineCache: loaded {} shader #{} hash={:016X} dwords={}",
        shader_type_name, shader_id, data_hash, dword_count);
  }
  return shader;
}
...
bool VulkanPipelineCache::TranslateAnalyzedShader(
  ...
  static std::atomic<uint64_t> shader_translate_count{0};
  uint64_t shader_translate_id = shader_translate_count.fetch_add(1) + 1;
  if (shader_translate_id <= 10 || (shader_translate_id % 200) == 0) {
    const char* shader_type_name = "Unknown";
    switch (shader.type()) {
      case xenos::ShaderType::kVertex:
        shader_type_name = "VS";
    ...
    XELOGI(
        "DEBUG: VulkanPipelineCache: translated {} shader #{} hash={:016X} "
        "valid={}",
        shader_type_name, shader_translate_id, shader.ucode_data_hash(),
        translation.is_valid() ? "YES" : "NO");
  }
...
bool VulkanPipelineCache::EnsurePipelineCreated(
  ...
  dynamic_state.dynamicStateCount =
      edram_fragment_shader_interlock ? 2 : uint32_t(dynamic_states.size());
  ...
  static std::atomic<uint64_t> pipeline_create_count{0};
  uint64_t pipeline_id = pipeline_create_count.fetch_add(1) + 1;
  if (pipeline_id <= 10 || (pipeline_id % 500) == 0) {
    if (creation_arguments.pixel_shader) {
      XELOGI(
          "DEBUG: VulkanPipelineCache: pipeline created #{} VS={:016X} PS={:016X} "
          "render_pass=0x{:016X}",
          pipeline_id,
          creation_arguments.vertex_shader->shader().ucode_data_hash(),
          creation_arguments.pixel_shader->shader().ucode_data_hash(),
          reinterpret_cast<uint64_t>(creation_arguments.render_pass));
    } else {
      XELOGI(
          "DEBUG: VulkanPipelineCache: pipeline created #{} VS={:016X} "
          "render_pass=0x{:016X}",
          pipeline_id,
          creation_arguments.vertex_shader->shader().ucode_data_hash(),
          reinterpret_cast<uint64_t>(creation_arguments.render_pass));
