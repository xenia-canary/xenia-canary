...
DEFINE_bool(
    vulkan_log_queue_submit_verbose, true,
    "Log detailed vkQueueSubmit information for Vulkan command processor.",
    "Vulkan");
DEFINE_int32(
    vulkan_log_queue_submit_every_n, 1,
    "Log every N Vulkan queue submissions when verbose logging is enabled.",
    "Vulkan");
DEFINE_bool(
    vulkan_log_issue_swap_verbose, true,
    "Log detailed IssueSwap activity in the Vulkan command processor.",
    "Vulkan");
DEFINE_int32(
    vulkan_log_issue_swap_every_n, 1,
    "Log every N IssueSwap calls when verbose logging is enabled.",
    "Vulkan");
DEFINE_bool(
    vulkan_log_issue_swap_refresh_verbose, false,
    "Log detailed RefreshGuestOutput activity during IssueSwap.",
    "Vulkan");
DEFINE_int32(
    vulkan_log_issue_swap_refresh_every_n, 1,
    "Log every N RefreshGuestOutput calls when verbose logging is enabled.",
    "Vulkan");
...
void VulkanCommandProcessor::DebugIssueSwapFromHost(uint32_t width,
                                                    uint32_t height) {
  static std::atomic<uint64_t> debug_swap_count{0};
  uint64_t debug_id = debug_swap_count.fetch_add(1) + 1;
  if (!memory_ || !kernel_state_) {
    XELOGW("DEBUG: DebugIssueSwapFromHost #{} skipped (memory not ready)", debug_id);
    return;
  }
  if (width == 0 || height == 0) {
    XELOGW("DEBUG: DebugIssueSwapFromHost #{} skipped (invalid size {}x{})",
           debug_id, width, height);
    return;
  }

  constexpr uint32_t kBytesPerPixel = 4;
  constexpr uint32_t kPitchAlignmentBytes =
      xe::gpu::xenos::kTextureLinearRowAlignmentBytes;
  uint32_t pitch_pixels =
      xe::align(width * kBytesPerPixel, kPitchAlignmentBytes) /
      kBytesPerPixel;
  uint32_t size_bytes = pitch_pixels * height * kBytesPerPixel;
  size_bytes = xe::align(size_bytes, 0x1000u);

  bool needs_alloc = debug_swap_frontbuffer_physical_ == 0 ||
                     width != debug_swap_frontbuffer_width_ ||
                     height != debug_swap_frontbuffer_height_ ||
                     size_bytes > debug_swap_frontbuffer_size_;
  if (needs_alloc) {
    if (debug_swap_frontbuffer_virtual_ != 0) {
      memory_->SystemHeapFree(debug_swap_frontbuffer_virtual_);
    }
    uint32_t frontbuffer_virtual =
        memory_->SystemHeapAlloc(size_bytes, 0x1000, kSystemHeapPhysical);
    if (!frontbuffer_virtual) {
      XELOGW(
          "DEBUG: DebugIssueSwapFromHost #{} failed to allocate {} bytes for "
          "frontbuffer",
          debug_id, size_bytes);
      return;
    }
    uint32_t frontbuffer_physical =
        memory_->GetPhysicalAddress(frontbuffer_virtual);
    if (frontbuffer_physical == UINT32_MAX) {
      XELOGW(
          "DEBUG: DebugIssueSwapFromHost #{} invalid physical address for "
          "frontbuffer (virtual={:08X})",
          debug_id, frontbuffer_virtual);
      return;
    }
    debug_swap_frontbuffer_virtual_ = frontbuffer_virtual;
    debug_swap_frontbuffer_physical_ = frontbuffer_physical;
    debug_swap_frontbuffer_size_ = size_bytes;
    debug_swap_frontbuffer_width_ = width;
    debug_swap_frontbuffer_height_ = height;
    debug_swap_frontbuffer_pitch_pixels_ = pitch_pixels;
    XELOGW(
        "DEBUG: DebugIssueSwapFromHost #{} allocated frontbuffer: "
        "virt={:08X} phys={:08X} size={} pitch_pixels={} ({}x{})",
        debug_id, frontbuffer_virtual, frontbuffer_physical, size_bytes,
        pitch_pixels, width, height);
  }

  uint8_t* frontbuffer_host =
      memory_->TranslatePhysical(debug_swap_frontbuffer_physical_);
  if (!frontbuffer_host) {
    XELOGW(
        "DEBUG: DebugIssueSwapFromHost #{} failed to map frontbuffer phys={:08X}",
        debug_id, debug_swap_frontbuffer_physical_);
    return;
  }

  uint64_t frame = ++debug_swap_frame_;
  uint8_t r = 0, g = 0, b = 0;
  switch (frame % 3) {
    case 0:
      r = 0xFF;
  ...
  uint32_t pixel = (0xFFu << 24) | (uint32_t(r) << 16) |
                   (uint32_t(g) << 8) | uint32_t(b);
  uint32_t* dst = reinterpret_cast<uint32_t*>(frontbuffer_host);
  uint32_t pitch = debug_swap_frontbuffer_pitch_pixels_;
  for (uint32_t y = 0; y < height; ++y) {
    uint32_t row_start = y * pitch;
    for (uint32_t x = 0; x < pitch; ++x) {
      dst[row_start + x] = pixel;
    }
  }
  ...
  fetch.base_address = debug_swap_frontbuffer_physical_ >> 12;
  ...
  bool ring_ready =
      ring_buffer_initialized_.load() && reader_.buffer() &&
      primary_buffer_size_ != 0 && read_ptr_index_ == write_ptr_index_.load();
  if (!ring_ready) {
    XELOGW(
        "DEBUG: DebugIssueSwapFromHost #{} ring not idle/ready "
        "(init={} read={:08X} write={:08X} size={:08X}) - issuing direct swap",
        debug_id, ring_buffer_initialized_.load(), read_ptr_index_,
        write_ptr_index_.load(), primary_buffer_size_);
    std::memcpy(
        &register_file_->values[XE_GPU_REG_SHADER_CONSTANT_FETCH_00_0], &fetch,
        sizeof(fetch));
    RecordDebugHostSwapInjection(debug_swap_frontbuffer_physical_, width, height,
                                 false);
    NotifySwapPacket(debug_swap_frontbuffer_physical_, width, height);
    IssueSwap(debug_swap_frontbuffer_physical_, width, height);
    return;
  }

  constexpr uint32_t kFetchDwords = 6;
  constexpr uint32_t kSwapPacketPayloadDwords = 4;
  constexpr uint32_t kSwapPacketDwords =
      1 + kFetchDwords + 1 + kSwapPacketPayloadDwords;
  uint32_t packet[kSwapPacketDwords] = {};
  uint32_t offset = 0;
  packet[offset++] = xenos::MakePacketType0(
      XE_GPU_REG_SHADER_CONSTANT_FETCH_00_0, kFetchDwords);
  ...
  packet[offset++] =
      xenos::MakePacketType3(xenos::PM4_XE_SWAP, kSwapPacketPayloadDwords);
  packet[offset++] = xenos::kSwapSignature;
  packet[offset++] = debug_swap_frontbuffer_physical_;
  packet[offset++] = width;
  packet[offset++] = height;

  uint8_t* ring_host = reader_.buffer();
  ...
  for (uint32_t i = 0; i < kSwapPacketDwords; ++i) {
    uint32_t idx = (write_ptr + i) % ring_dwords;
    xe::store_and_swap<uint32_t>(ring_host + idx * sizeof(uint32_t), packet[i]);
  }
  uint32_t new_wptr = (write_ptr + kSwapPacketDwords) % ring_dwords;
  if (graphics_system_) {
    graphics_system_->RecordWritePointerUpdate(new_wptr, "DebugSwapInject");
  }
  XELOGW(
      "DEBUG: DebugIssueSwapFromHost #{} injected swap packet to ring buffer "
      "(frontbuffer={:08X}, {}x{}, wptr {:08X}->{:08X})",
      debug_id, debug_swap_frontbuffer_physical_, width, height, write_ptr,
      new_wptr);
  RecordDebugHostSwapInjection(debug_swap_frontbuffer_physical_, width, height,
                               true);
  UpdateWritePointer(new_wptr, "VULKAN_DEBUG_SWAP_INJECT");
}
...
bool VulkanCommandProcessor::SetupContext() {
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: SetupContext ENTRY");
  if (!CommandProcessor::SetupContext()) {
    XELOGE("Failed to initialize base command processor context");
    return false;
  }
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: Base context initialized");
  ...
  // Verify submodules
  VerifyVulkanSubmodules(vulkan_device);
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: Submodules verified");
  ...
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: Empty descriptor set layout created");
  ...
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: Constant descriptor set layout created");
  ...
  XELOGI("DEBUG: VULKAN_COMMAND_PROCESSOR: SetupContext SUCCESS");
  return true;
}
...
void VulkanCommandProcessor::IssueSwap(uint32_t frontbuffer_ptr,
  ...
  uint64_t swap_id = swap_count.fetch_add(1) + 1;
  int log_every = cvars::vulkan_log_issue_swap_every_n;
  bool log_swap =
      cvars::vulkan_log_issue_swap_verbose &&
      (swap_id <= 5 || (log_every > 0 && (swap_id % log_every) == 0));
  int refresh_every = cvars::vulkan_log_issue_swap_refresh_every_n;
  bool log_refresh =
      cvars::vulkan_log_issue_swap_refresh_verbose &&
      (swap_id <= 5 ||
       (refresh_every > 0 && (swap_id % refresh_every) == 0));
  if (log_swap) {
    XELOGI(
        "DEBUG: VULKAN_COMMAND_PROCESSOR: IssueSwap #{} (ptr: 0x{:08X}, {}x{})",
        swap_id, frontbuffer_ptr, frontbuffer_width, frontbuffer_height);
  }
  ...
  if (!presenter) {
    if (log_swap) {
      XELOGW("DEBUG: IssueSwap #{}: presenter is null, dropping swap", swap_id);
    }
    return;
  }
  ...
  if (!first_frame_rendered_) {
    ...
    XELOGI("DEBUG: FIRST FRAME SWAP REQUESTED");
    XELOGI("   Frontbuffer address: {:08X}", frontbuffer_ptr);
    XELOGI("   Resolution: {}x{}", frontbuffer_width, frontbuffer_height);
    XELOGI("   Frame counter: {}", counter_);
    XELOGI("   Graphics pipeline is now actively rendering!");
  }

  // In case the swap command is the only one in the frame.
  if (!BeginSubmission(true)) {
    if (log_swap) {
      XELOGW("DEBUG: IssueSwap #{}: BeginSubmission failed", swap_id);
    }
    return;
  }
  ...
  if (swap_texture_view == VK_NULL_HANDLE) {
    if (log_swap) {
      XELOGW(
          "DEBUG: IssueSwap #{}: RequestSwapTexture returned null (scaled={}x{}, "
          "format={})",
          swap_id, frontbuffer_width_scaled, frontbuffer_height_scaled,
  ...
  if (log_swap) {
    XELOGI(
        "DEBUG: IssueSwap #{}: swap texture acquired (scaled={}x{}, format={})",
        swap_id, frontbuffer_width_scaled, frontbuffer_height_scaled,
        static_cast<uint32_t>(frontbuffer_format));
  }

  auto aspect = graphics_system_->GetScaledAspectRatio();
  uint64_t refresh_success_before =
      presenter->GetGuestOutputRefreshSuccessCount();
  if (log_swap) {
    XELOGI(
        "DEBUG: IssueSwap #{}: refresh pre-state success_count={} "
        "frontbuffer_scaled={}x{} aspect={}x{}",
        swap_id, refresh_success_before, frontbuffer_width_scaled,
        frontbuffer_height_scaled, aspect.first, aspect.second);
  }

  bool refresh_active = presenter->RefreshGuestOutput(
      frontbuffer_width_scaled, frontbuffer_height_scaled, aspect.first,
      aspect.second,
      [this, frontbuffer_width_scaled, frontbuffer_height_scaled,
       frontbuffer_format, swap_texture_view, swap_id, log_refresh,
       frontbuffer_ptr](
          ui::Presenter::GuestOutputRefreshContext& context) -> bool {
        if (log_refresh) {
          XELOGI(
              "DEBUG: IssueSwap #{}: RefreshGuestOutput entry (scaled={}x{} "
              "format={} swap_texture_view=0x{:X} frontbuffer_ptr=0x{:08X})",
              swap_id, frontbuffer_width_scaled, frontbuffer_height_scaled,
              static_cast<uint32_t>(frontbuffer_format),
              static_cast<uint64_t>(reinterpret_cast<uintptr_t>(swap_texture_view)),
              frontbuffer_ptr);
        }
        // In case the swap command is the only one in the frame.
        if (!BeginSubmission(true)) {
          if (log_refresh) {
            XELOGW("DEBUG: IssueSwap #{}: RefreshGuestOutput BeginSubmission failed",
                   swap_id);
          }
          return false;
        }
        ...
        if (log_refresh) {
          XELOGI(
              "DEBUG: IssueSwap #{}: guest_output_image_version={} "
              "swap_frame_index={} image_view=0x{:X} image=0x{:X}",
              swap_id, guest_output_image_version, swap_frame_index,
              static_cast<uint64_t>(reinterpret_cast<uintptr_t>(vulkan_context.image_view())),
              static_cast<uint64_t>(reinterpret_cast<uintptr_t>(vulkan_context.image())));
        }
        ...
        if (log_refresh) {
          XELOGI("DEBUG: IssueSwap #{}: use_pwl_gamma_ramp={}",
                 swap_id, use_pwl_gamma_ramp ? "YES" : "NO");
        }
        ...
        if (gamma_ramp_frame_index_ref == UINT32_MAX) {
          ...
          if (log_refresh) {
            XELOGI(
                "DEBUG: IssueSwap #{}: uploading gamma ramp (size={}, offset={}, "
                "use_pwl={})",
                swap_id, gamma_ramp_size, gamma_ramp_upload_offset,
                use_pwl_gamma_ramp ? "YES" : "NO");
          }
          ...
          if (log_refresh) {
            XELOGI(
                "DEBUG: IssueSwap #{}: gamma ramp upload buffer present={} "
                "frame_index_ref(before)={}",
                swap_id, gamma_ramp_has_upload_buffer ? "YES" : "NO",
          ...
          if (log_refresh) {
            XELOGI("DEBUG: IssueSwap #{}: gamma_ramp_frame_index_ref now {}",
                   swap_id, gamma_ramp_frame_index_ref);
          }
        }
        ...
        if (swap_framebuffer_index == SIZE_MAX) {
          ...
          if (log_refresh) {
            XELOGI(
                "DEBUG: IssueSwap #{}: created swap framebuffer index={} "
                "version={}",
                swap_id, swap_framebuffer_index, guest_output_image_version);
          }
        }
        ...
        if (vulkan_context.image_ever_written_previously()) {
          if (log_refresh) {
            XELOGI(
                "DEBUG: IssueSwap #{}: guest output image previously written, "
                "inserting layout barrier",
                swap_id);
          }
        ...
        if (log_refresh) {
          XELOGI("DEBUG: IssueSwap #{}: render pass begun", swap_id);
        }
        ...
        if (log_refresh) {
          XELOGI("DEBUG: IssueSwap #{}: issued fullscreen draw", swap_id);
        }

        deferred_command_buffer_.CmdVkEndRenderPass();
        if (log_refresh) {
          XELOGI("DEBUG: IssueSwap #{}: render pass ended", swap_id);
        }
        ...
        EndSubmission(true);
        if (log_refresh) {
          XELOGI("DEBUG: IssueSwap #{}: RefreshGuestOutput completed", swap_id);
        }
        return true;
      });
  uint64_t refresh_success_after =
      presenter->GetGuestOutputRefreshSuccessCount();
  if (log_swap || (!refresh_active && refresh_success_after == 0)) {
    XELOGI(
        "DEBUG: IssueSwap #{}: RefreshGuestOutput result={} success_count={} "
        "delta={} frontbuffer_ptr=0x{:08X} scaled={}x{} aspect={}x{}",
        swap_id, refresh_active ? "ACTIVE" : "INACTIVE", refresh_success_after,
        refresh_success_after - refresh_success_before, frontbuffer_ptr,
        frontbuffer_width_scaled, frontbuffer_height_scaled, aspect.first,
        aspect.second);
  }
  if (!refresh_active && refresh_success_after == 0) {
    XELOGW(
        "DEBUG: IssueSwap #{}: refresh path did not activate guest output; "
        "presenter will remain force_clear_only until at least one refresh "
        "succeeds.",
        swap_id);
  }
  if (log_swap) {
    XELOGI("DEBUG: IssueSwap #{}: RefreshGuestOutput active={}",
           swap_id, refresh_active ? "YES" : "NO");
  }
...
bool VulkanCommandProcessor::SubmitBarriers(bool force_end_render_pass) {
  ...
  static std::atomic<uint64_t> barrier_submit_count{0};
  uint64_t barrier_id = barrier_submit_count.fetch_add(1) + 1;
  if (barrier_id <= 5 || (barrier_id % 500) == 0) {
    XELOGI(
        "DEBUG: SubmitBarriers #{} batches={} buffer_barriers={} image_barriers={} force_end_render_pass={}",
        barrier_id, pending_barriers_.size(),
        pending_barriers_buffer_memory_barriers_.size(),
        pending_barriers_image_memory_barriers_.size(),
        force_end_render_pass ? "YES" : "NO");
  }
  EndRenderPass();
...
void VulkanCommandProcessor::SubmitBarriersAndEnterRenderTargetCacheRenderPass(
  ...
  static std::atomic<uint64_t> render_pass_begin_count{0};
  uint64_t render_pass_id = render_pass_begin_count.fetch_add(1) + 1;
  if (render_pass_id <= 5 || (render_pass_id % 500) == 0) {
    XELOGI("DEBUG: BeginRenderPass #{} render_pass=0x{:016X} framebuffer=0x{:016X} extent={}x{}",
           render_pass_id,
           reinterpret_cast<uint64_t>(render_pass),
           reinterpret_cast<uint64_t>(framebuffer ? framebuffer->framebuffer
                                                  : VK_NULL_HANDLE),
           framebuffer ? framebuffer->host_extent.width : 0,
           framebuffer ? framebuffer->host_extent.height : 0);
  }
...
void VulkanCommandProcessor::EndRenderPass() {
  ...
  static std::atomic<uint64_t> render_pass_end_count{0};
  uint64_t render_pass_end_id = render_pass_end_count.fetch_add(1) + 1;
  if (render_pass_end_id <= 5 || (render_pass_end_id % 500) == 0) {
    XELOGI("DEBUG: EndRenderPass #{} current_render_pass=0x{:016X}",
           render_pass_end_id,
           reinterpret_cast<uint64_t>(current_render_pass_));
  }
  deferred_command_buffer_.CmdVkEndRenderPass();
...
bool VulkanCommandProcessor::IssueDraw(xenos::PrimitiveType prim_type,
  ...
  static std::atomic<uint64_t> draw_count{0};
  uint64_t draw_id = draw_count.fetch_add(1) + 1;
  if (draw_id <= 5 || (draw_id % 1000) == 0) {
    XELOGI("DEBUG: IssueDraw #{} prim={} index_count={} major_mode_explicit={}",
           draw_id, xenos::GetPrimitiveTypeEnglishDescription(prim_type),
           index_count, major_mode_explicit ? "YES" : "NO");
  }
  ...
  if (edram_mode == xenos::EdramMode::kCopy) {
    ...
    static std::atomic<bool> logged_copy_mode{false};
    if (!logged_copy_mode.exchange(true)) {
      XELOGI("DEBUG: IssueDraw: EDRAM copy mode active, issuing copy instead of draw");
    }
    return IssueCopy();
  }
  ...
  if (!vertex_shader) {
    ...
    static std::atomic<bool> logged_missing_vs{false};
    if (!logged_missing_vs.exchange(true)) {
      XELOGW("DEBUG: IssueDraw skipped: no active vertex shader bound");
    }
    return false;
  }
  ...
  if (is_rasterization_done) {
    ...
    if (memexport_ranges_.empty()) {
      ...
      static std::atomic<bool> logged_no_effect{false};
      if (!logged_no_effect.exchange(true)) {
        XELOGI("DEBUG: IssueDraw skipped: no rasterization and no memexport ranges");
      }
      return true;
    }
  }
...
bool VulkanCommandProcessor::BeginSubmission(bool is_guest_command) {
  ...
  if (device_lost_) {
    static std::atomic<bool> logged_device_lost{false};
    if (!logged_device_lost.exchange(true)) {
      XELOGE("DEBUG: VulkanCommandProcessor::BeginSubmission called after device loss");
    }
    return false;
  }
  ...
  uint64_t begin_id = begin_count.fetch_add(1) + 1;
  if (begin_id <= 3 || (begin_id % 300) == 0) {
    XELOGI("DEBUG: BeginSubmission #{} guest={} opening_frame={} submission_open={} frame_open={}",
           begin_id, is_guest_command ? "YES" : "NO",
           is_opening_frame ? "YES" : "NO",
           submission_open_ ? "YES" : "NO", frame_open_ ? "YES" : "NO");
  }
  ...
  if (is_opening_frame) {
    ...
    if (!logged_first_frame.exchange(true)) {
      XELOGI("DEBUG: VulkanCommandProcessor: first frame opened");
    }
...
bool VulkanCommandProcessor::EndSubmission(bool is_swap) {
  ...
  static std::atomic<uint64_t> end_submission_count{0};
  uint64_t end_id = end_submission_count.fetch_add(1) + 1;
  if (end_id <= 5 || (end_id % 300) == 0) {
    XELOGI(
        "DEBUG: EndSubmission #{} swap={} submission_open={} frame_open={} frame_current={} submission_current={}",
        end_id, is_swap ? "YES" : "NO", submission_open_ ? "YES" : "NO",
        frame_open_ ? "YES" : "NO",
        static_cast<unsigned long long>(frame_current_),
        static_cast<unsigned long long>(GetCurrentSubmission()));
  }
  ...
  if (submission_open_) {
    ...
    static std::atomic<uint64_t> vk_submit_count{0};
    uint64_t vk_submit_id = vk_submit_count.fetch_add(1) + 1;
    if (vk_submit_id <= 5 || (vk_submit_id % 500) == 0) {
      XELOGI("DEBUG: vkQueueSubmit #{} wait_semaphores={} command_buffers=1",
             vk_submit_id,
             current_submission_wait_semaphores_.size());
    }
    if (cvars::vulkan_log_queue_submit_verbose) {
      int log_every = cvars::vulkan_log_queue_submit_every_n;
      bool log_submit =
          (vk_submit_id <= 5) ||
          (log_every > 0 && (vk_submit_id % log_every) == 0);
      if (log_submit) {
        std::ostringstream wait_masks;
        for (size_t i = 0; i < current_submission_wait_stage_masks_.size();
             ++i) {
          if (i) {
            wait_masks << ",";
          }
          wait_masks << "0x" << std::hex << current_submission_wait_stage_masks_[i];
        }
        XELOGI(
            "DEBUG: vkQueueSubmit detail #{} submission={} fence={} cmd={} waits={} "
            "wait_masks=[{}] semaphores_in_flight={} fences_free={}",
            vk_submit_id, GetCurrentSubmission(),
            static_cast<void*>(fence),
            static_cast<void*>(command_buffer.buffer),
            current_submission_wait_semaphores_.size(), wait_masks.str(),
            submissions_in_flight_semaphores_.size(), fences_free_.size());
      }
    }
    uint64_t submission_current = GetCurrentSubmission();
    current_submission_wait_stage_masks_.clear();
