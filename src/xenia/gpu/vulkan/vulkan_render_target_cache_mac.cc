...
DEFINE_string(
    ...
    "GPU");
DEFINE_bool(
    vulkan_log_rt_resolve_verbose, true,
    "Log detailed resolve operations in Vulkan render target cache.",
    "GPU");
DEFINE_int32(
    vulkan_log_rt_resolve_every_n, 1,
    "Log every N resolve operations when verbose resolve logging is enabled.",
    "GPU");
DEFINE_bool(
    vulkan_log_rt_transfer_verbose, true,
    "Log detailed render target transfer and resolve clear operations.",
    "GPU");
DEFINE_int32(
    vulkan_log_rt_transfer_every_n, 1,
    "Log every N transfer/resolve-clear batches when verbose logging is enabled.",
    "GPU");
...
bool VulkanRenderTargetCache::Initialize(uint32_t shared_memory_binding_count) {
  ...
  bool requested_fsi = (cvars::render_target_path_vulkan == "fsi");
  if (requested_fsi) {
  ...
  const char* requested_name =
      cvars::render_target_path_vulkan.empty()
          ? "auto"
          : cvars::render_target_path_vulkan.c_str();
  XELOGI("Vulkan render target path: {} (requested='{}')", path_name,
         requested_name);
  if (requested_fsi && path_ != Path::kPixelShaderInterlock) {
    XELOGW(
        "Vulkan render target path: FSI requested but not supported; "
        "falling back to FBO");
  }
...
bool VulkanRenderTargetCache::Resolve(const Memory& memory,
  ...
  uint64_t resolve_id = resolve_count.fetch_add(1) + 1;
  int log_every = cvars::vulkan_log_rt_resolve_every_n;
  bool log_resolve =
      cvars::vulkan_log_rt_resolve_verbose &&
      (resolve_id <= 5 || (log_every > 0 && (resolve_id % log_every) == 0));
  ...
  if (!draw_util::GetResolveInfo(
    ...
    if (log_resolve) {
      XELOGW("DEBUG: Resolve #{} dropped: GetResolveInfo failed", resolve_id);
    }
    return false;
  }
  if (log_resolve) {
    XELOGI("DEBUG: Resolve #{} path={} scaled={}x{}", resolve_id,
           GetPath() == Path::kPixelShaderInterlock ? "FSI" : "FBO",
           draw_resolution_scale_x(), draw_resolution_scale_y());
    XELOGI(
        "DEBUG: Resolve #{} copy_control=0x{:08X} copy_src_select={} copy_command={} "
        "color_clear={} depth_clear={}",
        resolve_id, resolve_info.rb_copy_control.value,
        resolve_info.rb_copy_control.copy_src_select,
        static_cast<uint32_t>(resolve_info.rb_copy_control.copy_command),
        resolve_info.rb_copy_control.color_clear_enable ? "YES" : "NO",
        resolve_info.rb_copy_control.depth_clear_enable ? "YES" : "NO");
    XELOGI(
        "DEBUG: Resolve #{} coord edram_off=({}, {}) width_div_8={} height_div_8={} "
        "scale={}x{}",
        resolve_id, resolve_info.coordinate_info.edram_offset_x_div_8,
    ...
    XELOGI(
        "DEBUG: Resolve #{} dest_info=0x{:08X} dest_base=0x{:X} dest_extent=[0x{:X}, "
        "len=0x{:X}] copy_dest_format={} number={} endian={} swap={} array={} "
        "slice={}",
        resolve_id, resolve_info.copy_dest_info.value,
        ...
        static_cast<uint32_t>(resolve_info.copy_dest_info.copy_dest_number),
        static_cast<uint32_t>(resolve_info.copy_dest_info.copy_dest_endian),
        resolve_info.copy_dest_info.copy_dest_swap ? 1 : 0,
        resolve_info.copy_dest_info.copy_dest_array ? 1 : 0,
  ...
  if (!resolve_info.coordinate_info.width_div_8 || !resolve_info.height_div_8) {
    if (log_resolve) {
      XELOGI(
          "DEBUG: Resolve #{} no-op: width_div_8={} height_div_8={}",
          resolve_id, resolve_info.coordinate_info.width_div_8,
  ...
  if (resolve_info.copy_dest_extent_length) {
    ...
    if (log_resolve) {
      XELOGI("DEBUG: Resolve #{} copy_shader={} groups={}x{}",
             resolve_id, static_cast<uint32_t>(copy_shader),
    ...
    switch (mode.output) {
      ...
      case TransferOutput::kDepth: {
        if (packed) {
          ...
          #ifdef __APPLE__
          (void)depth24_to_depth32_convert_id;
          #endif
...
void VulkanRenderTargetCache::PerformTransfersAndResolveClears(
  ...
  static std::atomic<uint64_t> transfer_batch_count{0};
  uint64_t transfer_id = transfer_batch_count.fetch_add(1) + 1;
  int log_every = cvars::vulkan_log_rt_transfer_every_n;
  bool log_transfer =
      cvars::vulkan_log_rt_transfer_verbose &&
      (transfer_id <= 5 || (log_every > 0 && (transfer_id % log_every) == 0));
  if (log_transfer) {
    XELOGI(
        "DEBUG: RT transfers #{} submission={} render_targets={} resolve_clear={}",
        transfer_id, current_submission, render_target_count,
        resolve_clear_needed ? "YES" : "NO");
    for (uint32_t i = 0; i < render_target_count; ++i) {
      RenderTarget* dest_rt = render_targets[i];
      if (!dest_rt) {
        XELOGI("DEBUG: RT transfers #{} rt[{}]=null", transfer_id, i);
        continue;
      }
      auto& dest_vulkan_rt = *static_cast<VulkanRenderTarget*>(dest_rt);
      RenderTargetKey dest_key = dest_vulkan_rt.key();
      size_t transfer_count = render_target_transfers ? render_target_transfers[i].size() : 0;
      XELOGI(
          "DEBUG: RT transfers #{} rt[{}] key=0x{:08X} base_tiles={} pitch_tiles={} "
          "msaa={} depth={} fmt={} transfers={}",
          transfer_id, i, dest_key.key, dest_key.base_tiles,
          dest_key.pitch_tiles_at_32bpp,
          static_cast<uint32_t>(dest_key.msaa_samples),
          dest_key.is_depth ? "YES" : "NO",
          static_cast<uint32_t>(dest_key.resource_format), transfer_count);
      if (render_target_transfers && transfer_count) {
        for (size_t t = 0; t < transfer_count; ++t) {
          const Transfer& transfer = render_target_transfers[i][t];
          RenderTargetKey source_key{};
          if (transfer.source) {
            auto& source_rt = *static_cast<VulkanRenderTarget*>(transfer.source);
            source_key = source_rt.key();
          }
          XELOGI(
              "DEBUG: RT transfers #{} rt[{}] transfer[{}] tiles=[{}, {}) source_key=0x{:08X} host_depth_source={}",
              transfer_id, i, t, transfer.start_tiles, transfer.end_tiles,
              source_key.key, transfer.host_depth_source ? "YES" : "NO");
        }
      }
    }
  }
...
#ifdef __APPLE__
builder.addDecoration(id_vector_temp.back(), spv::DecorationArrayStride,
                      static_cast<int>(sizeof(uint32_t) << uint32_t(format_is_64bpp)));
#else
builder.addDecoration(id_vector_temp.back(), spv::DecorationArrayStride,
