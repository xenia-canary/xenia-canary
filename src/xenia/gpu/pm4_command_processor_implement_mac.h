    ...
    do {
      if (COMMAND_PROCESSOR::ExecutePacket()) {
        ...
        XELOGE(
            ...
            "aborting current indirect buffer execution.");
        break;
      }
...
XE_NOINLINE
static void DumpIndirectBufferDwords(const char* label, const uint8_t* data,
                                     uint32_t dword_count,
                                     uint32_t max_dump_dwords) {
  if (!data || dword_count == 0 || max_dump_dwords == 0) {
    XELOGI("{}: <empty>", label ? label : "indirect");
    return;
  }
  uint32_t dump_count = std::min<uint32_t>(dword_count, max_dump_dwords);
  XELOGI("{} ({} dwords, dumping {}):", label ? label : "indirect",
         dword_count, dump_count);
  for (uint32_t i = 0; i < dump_count; i += 8) {
    uint32_t end = std::min<uint32_t>(i + 7, dump_count - 1);
    uint32_t values[8] = {};
    for (uint32_t j = 0; j < 8; ++j) {
      uint32_t index = i + j;
      if (index < dump_count) {
        values[j] =
            xe::load_and_swap<uint32_t>(data + index * sizeof(uint32_t));
      }
    }
    XELOGI("[{:02X}-{:02X}] {:08X} {:08X} {:08X} {:08X} {:08X} {:08X} {:08X} {:08X}",
           i, end,
...
std::string GenerateRegnameForPm4Print(uint32_t reg) {
  auto reg_info = gpu::RegisterFile::GetRegisterInfo(reg);
...
#endif
  ring_size_t read_offset_before = reader_.read_offset();
  const uint32_t packet = reader_.ReadAndSwap<uint32_t>();
  const uint32_t packet_type = packet >> 30;
  static std::atomic<uint64_t> packet_count{0};
  uint64_t packet_id = packet_count.fetch_add(1) + 1;
  // debug: log all packets to find the hang
  if (true || packet_id <= 20 || (packet_id % 2000) == 0) {
    XELOGI("DEBUG: PM4 packet #{} type={} raw=0x{:08X} read_off={:08X} read_count={} write_off={:08X}",
           packet_id, packet_type, packet,
           static_cast<uint32_t>(read_offset_before),
           reader_.read_count(),
           static_cast<uint32_t>(reader_.write_offset()));
  }
...
bool COMMAND_PROCESSOR::ExecutePacketType3(uint32_t packet) XE_RESTRICT {
  ...
  static std::atomic<bool> logged_first_type3{false};
  if (!logged_first_type3.exchange(true)) {
    XELOGI("DEBUG: First PM4 Type3 packet: opcode=0x{:02X} count={} read_offset={:08X}",
  ...
  if (COMMAND_PROCESSOR::GetCurrentRingReadCount() >=
    ...
    switch (opcode) {
      ...
      case PM4_DRAW_INDX:
        {
          static std::atomic<bool> logged_first_draw{false};
          if (!logged_first_draw.exchange(true)) {
            XELOGI("DEBUG: First DRAW packet: PM4_DRAW_INDX count={} read_offset={:08X}",
                   count, data_start_offset);
          }
        }
      ...
      case PM4_DRAW_INDX_2:
        {
          static std::atomic<bool> logged_first_draw2{false};
          if (!logged_first_draw2.exchange(true)) {
            XELOGI("DEBUG: First DRAW packet: PM4_DRAW_INDX_2 count={} read_offset={:08X}",
                   count, data_start_offset);
          }
        }
        result =
...
bool COMMAND_PROCESSOR::HitUnimplementedOpcode(uint32_t opcode,
  ...
  if (::cvars::gpu_ignore_unimplemented_opcode) {
    reader_.AdvanceRead(count * sizeof(uint32_t));
    trace_writer_.WritePacketEnd();
    return true;
  }
...
bool COMMAND_PROCESSOR::ExecutePacketType3_XE_SWAP(uint32_t packet,
  ...
  uint32_t packet_start_offset = reader_.read_offset();
  ...
  NotifySwapPacket(frontbuffer_ptr, frontbuffer_width, frontbuffer_height);

  static std::atomic<uint64_t> swap_packet_count{0};
  uint64_t swap_packet_id = swap_packet_count.fetch_add(1) + 1;
  bool log_swap =
      cvars::gpu_log_pm4_swap_verbose &&
      (swap_packet_id <= 5 ||
       (cvars::gpu_log_pm4_swap_every_n > 0 &&
        (swap_packet_id % cvars::gpu_log_pm4_swap_every_n) == 0));
  if (log_swap) {
    XELOGI(
        "DEBUG: PM4_XE_SWAP #{} packet=0x{:08X} count={} read_offset=0x{:X}",
        swap_packet_id, packet, count, packet_start_offset);
    XELOGI("magic=0x{:08X} expected=0x{:08X}",
           magic, static_cast<uint32_t>(kSwapSignature));
    XELOGI("frontbuffer={:08X} size={}x{}", frontbuffer_ptr,
           frontbuffer_width, frontbuffer_height);
    XELOGI("reader offset now=0x{:X} remaining_dwords={}",
           reader_.read_offset(), reader_.read_count() / sizeof(uint32_t));
  } else if (swap_packet_id <= 5 || (swap_packet_id % 300) == 0) {
    XELOGI("DEBUG: PM4_XE_SWAP #{} frontbuffer={:08X} {}x{}",
           swap_packet_id, frontbuffer_ptr, frontbuffer_width,
           frontbuffer_height);
  }
...
bool COMMAND_PROCESSOR::ExecutePacketType3_INDIRECT_BUFFER(
  ...
  list_length &= 0xFFFFF;
  static std::atomic<uint64_t> ib_count{0};
  uint64_t ib_id = ib_count.fetch_add(1) + 1;
  bool log_ib =
      cvars::gpu_log_pm4_indirect_verbose &&
      (ib_id <= 5 ||
       (cvars::gpu_log_pm4_indirect_every_n > 0 &&
        (ib_id % cvars::gpu_log_pm4_indirect_every_n) == 0));
  if (log_ib) {
    XELOGI(
        "DEBUG: PM4 indirect buffer #{} packet=0x{:08X} count={} list_ptr={:08X} "
        "length_dwords={}",
        ib_id, packet, count, list_ptr, list_length);
    if (list_length > 0) {
      const uint8_t* ib_data =
          memory_ ? memory_->TranslatePhysical(list_ptr) : nullptr;
      DumpIndirectBufferDwords("indirect buffer dump", ib_data, list_length,
                               static_cast<uint32_t>(
                                   std::max(0, cvars::gpu_log_pm4_indirect_dump_dwords)));
    }
  } else if (ib_id <= 5 || (ib_id % 200) == 0) {
    XELOGI("DEBUG: PM4 indirect buffer #{} list_ptr={:08X} length_dwords={}",
           ib_id, list_ptr, list_length);
  }
    ...
    if (!matched) {
      ...
      if ((spin_iterations & 0xFFFF) == 0) {
        ...
        if (elapsed_ms >= 10 && (elapsed_ms - last_log_ms) >= 100) {
          XELOGI(
              "DEBUG: WAIT_REG_MEM still waiting: addr={:08X} value={:08X} ref={:08X} mask={:08X} wait_info=0x{:08X} elapsed={}ms",
              poll_reg_addr, value, ref, mask, wait_info,
  ...
  if (spin_iterations) {
    uint64_t elapsed_ms = static_cast<uint64_t>(
        std::chrono::duration_cast<std::chrono::milliseconds>(Clock::now() -
                                                              wait_start)
            .count());
    XELOGI("DEBUG: WAIT_REG_MEM satisfied after {} ms (iters={})",
           static_cast<unsigned long long>(elapsed_ms),
           static_cast<unsigned long long>(spin_iterations));
  }
...
bool COMMAND_PROCESSOR::ExecutePacketType3Draw(
  ...
  uint32_t vgt_dma_base = 0;
  reg::VGT_DMA_SIZE vgt_dma_size;
  vgt_dma_size.value = 0;
  ...
  switch (vgt_draw_initiator.source_select) {
    case xenos::SourceSelect::kDMA: {
      ...
      vgt_dma_base = reader_.ReadAndSwap<uint32_t>();
      --count_remaining;
  ...
  uint64_t draw_packet_id = draw_packet_count.fetch_add(1) + 1;
  if (draw_packet_id <= 10 || (draw_packet_id % 500) == 0) {
    const char* source_select_name = "Unknown";
    switch (vgt_draw_initiator.source_select) {
      case xenos::SourceSelect::kDMA:
        source_select_name = "DMA";
        break;
      case xenos::SourceSelect::kImmediate:
        source_select_name = "Immediate";
        break;
      case xenos::SourceSelect::kAutoIndex:
        source_select_name = "AutoIndex";
        break;
      default:
        break;
    }
    XELOGI(
        "DEBUG: {} draw packet #{}: prim={} num_indices={} source={} indexed={} index_size={} dma_base={:08X} dma_words={} viz_query=0x{:08X}",
        opcode_name, draw_packet_id,
        GetPrimitiveTypeEnglishDescription(vgt_draw_initiator.prim_type),
        vgt_draw_initiator.num_indices, source_select_name,
        is_indexed ? "YES" : "NO", index_size_bytes, vgt_dma_base,
        vgt_dma_size.num_words, viz_query_condition);
  }
...
uint32_t COMMAND_PROCESSOR::ExecutePrimaryBuffer(uint32_t read_index,
  ...
  SCOPE_profile_cpu_f("gpu");
  XELOGI("DEBUG: BREADCRUMB: ExecutePrimaryBuffer read={:08X} write={:08X}", read_index, write_index);
#if XE_ENABLE_TRACE_WRITER_INSTRUMENTATION == 1
...
#endif
  // Validate ring buffer before usage
  if (!primary_buffer_ptr_ || !ring_buffer_initialized_.load()) {
    XELOGE("DEBUG: CRITICAL: Attempting to use uninitialized ring buffer!");
    XELOGE("primary_buffer_ptr_ = {:08X}", primary_buffer_ptr_);
    return read_index;
  }
  ...
  // Validate memory translation before creating ring buffer
  auto* buffer_ptr = memory_->TranslatePhysical(primary_buffer_ptr_);
  if (!buffer_ptr) {
    XELOGE("DEBUG: CRITICAL: Failed to translate primary buffer address: {:08X}",
           primary_buffer_ptr_);
    return read_index;
  }
  
  new (&reader_) RingBuffer(buffer_ptr, primary_buffer_size_);

  reader_.set_read_offset(read_index * sizeof(uint32_t));
  reader_.set_write_offset(write_index * sizeof(uint32_t));
  static std::atomic<bool> logged_first_pm4{false};
  if (!logged_first_pm4.exchange(true)) {
    XELOGI("DEBUG: First PM4 execution: read={:08X} write={:08X} buffer_ptr={:08X} size_bytes={:08X}",
  ...
  do {
    if (!COMMAND_PROCESSOR::ExecutePacket()) {
      ...
      XELOGE(
          ...
          "resynchronizing to write pointer (read={:08X} write={:08X} "
...
void COMMAND_PROCESSOR::ExecutePacket(uint32_t ptr, uint32_t count) {
  ...
  do {
    if (!COMMAND_PROCESSOR::ExecutePacket()) {
      XELOGE(
          "**** ExecutePacket: Failed to execute packet in explicit range "
          "(ptr={:08X} count={:08X}); aborting this range.",
          ptr, count);
      break;
    }
  } while (reader_.read_count());
