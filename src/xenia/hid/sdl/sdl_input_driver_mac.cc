...
X_STATUS SDLInputDriver::Setup() {
  ...
  window()->app_context().CallInUIThread([this]() {
    fprintf(stderr, "Attempting to initialize SDL\n");
#else
  window()->app_context().CallInUIThreadSynchronous([this]() {
#endif
...
#if defined(__APPLE__)
  // On macOS, wait a bit for async initialization to complete
  // This is a simple fix - ideally we'd use proper synchronization
  for (int i = 0; i < 100; ++i) {
...
void SDLInputDriver::LoadGameControllerDB() {
  ...
  for (size_t i = 0; i < mappings.GetRowCount(); i++) {
    ...
    auto format = [](const std::string& ss, const std::string& s) {
