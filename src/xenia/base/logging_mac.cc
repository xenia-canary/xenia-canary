...
class AndroidLogSink final : public LogSink {
 public:
  explicit AndroidLogSink(const std::string_view tag)
      : tag_(tag), line_buffer_{} {}
...
class Logger {
 ...
 private:
  static constexpr size_t kBufferSize = 32_MiB;
  ...
  std::atomic<uint32_t> dropped_lines_{0};
 ...
 public:
  void AppendLine(uint32_t thread_id, const char prefix_char,
    ...
    if (!claim_strategy_.try_claim(count, range)) {
      uint32_t dropped = dropped_lines_.fetch_add(1, std::memory_order_relaxed) + 1;
      // Log occasionally to stderr to avoid feedback through the logger.
      if ((dropped & (dropped - 1)) == 0) {
        std::fprintf(stderr,
                     "[logging] Dropped %u log lines (logger buffer full)\n",
                     dropped);
      }
      return;
    }
...
void InitializeLogging(const std::string_view app_name) {
  ...
  if (cvars::log_file.empty()) {
    ...
    // On macOS, writing into the application bundle invalidates the code signature.
    // Write to the user data folder instead.
    auto file_path = xe::filesystem::GetUserFolder() / file_name;
#else
...
// Explicit template instantiations for logging functions at global scope
// These are needed to export symbols from the static library
template void XELOGI<char const*>(std::string_view, char const* const&);
template void XELOGW<>(std::string_view);
template void XELOGE<>(std::string_view);
template void XELOGD<>(std::string_view);

// Explicit instantiation for AppendLogLineFormat_Impl
template void xe::logging::AppendLogLineFormat_Impl<char const*>(
    xe::LogLevel, const char, std::string_view, char const* const&);
