...
static void ExceptionHandlerCallback(int signal_number, siginfo_t* signal_info,
  ...
  switch (signal_number) {
    ...
    case SIGSEGV: {
      ...
      // memory location. Darwin exposes ESR via the exception state.
      if (((mcontext->__es.__esr >> 26) & 0b111110) == 0b100100) {
        ...
        // Fall back to decoding the faulting instruction. On AArch64 (unlike
        // on AArch32), the program counter is the address of the currently
        ...
        if (IsArm64LoadPrefetchStore(
          ...
          assert_always(
              "ESR is not a Data Abort, and the faulting instruction is not "
