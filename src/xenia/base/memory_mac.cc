...
#if XE_ARCH_AMD64
#include "third_party/simde/simde/x86/avx.h"
#include "third_party/simde/simde/x86/avx2.h"
#endif
...
static void first_vastcpy(CacheLine* XE_RESTRICT physaddr,
  ...
  if (amd64::GetFeatureFlags() & amd64::kX64EmitMovdir64M) {
    ...
    XELOGW("AVX probe failed - falling back to rep movs vastcpy.");
    dispatch_to_use = vastcpy_impl_repmovs;
  }
...
void copy_and_swap_16_aligned(void* dest_ptr, const void* src_ptr,
  ...
  simde__m128i shufmask =
      simde_mm_set_epi8(0x0E, 0x0F, 0x0C, 0x0D, 0x0A, 0x0B, 0x08, 0x09, 0x06, 0x07,
                       0x04, 0x05, 0x02, 0x03, 0x00, 0x01);

  size_t i = 0;
  for (i = 0; i + 8 <= count; i += 8) {
    simde__m128i input = simde_mm_load_si128(reinterpret_cast<const simde__m128i*>(&src[i]));
    simde__m128i output = simde_mm_shuffle_epi8(input, shufmask);
    simde_mm_store_si128(reinterpret_cast<simde__m128i*>(&dest[i]), output);
  }
...
void copy_and_swap_16_unaligned(void* dest_ptr, const void* src_ptr,
  ...
  size_t i;
  for (i = 0; i + 8 <= count; i += 8) {
    simde__m128i input = simde_mm_loadu_si128(reinterpret_cast<const simde__m128i*>(&src[i]));
    simde__m128i output = simde_mm_shuffle_epi8(input, shufmask);
    simde_mm_storeu_si128(reinterpret_cast<simde__m128i*>(&dest[i]), output);
  }
  XE_WORKAROUND_CONSTANT_RETURN_IF(count % 8 == 0);
  for (; i < count; ++i) {  // handle residual elements
    dest[i] = byte_swap(src[i]);
...
void copy_and_swap_32_aligned(void* dest_ptr, const void* src_ptr,
  ...
  simde__m128i shufmask =
      simde_mm_set_epi8(0x0C, 0x0D, 0x0E, 0x0F, 0x08, 0x09, 0x0A, 0x0B, 0x04, 0x05,
...
// Ambitious SIMD compatibility via SIMDe
void copy_and_swap_32_unaligned(void* dest_ptr, const void* src_ptr,
  ...
  if (amd64::GetFeatureFlags() & amd64::kX64EmitAVX2) {
    simde__m256i shufmask = simde_mm256_set_epi8(
    ...
    for (i = 0; i + 16 <= count; i += 16) {
      simde__m256i input1 =
          simde_mm256_loadu_si256(reinterpret_cast<const simde__m256i*>(&src[i]));
      simde__m256i input2 =
          simde_mm256_loadu_si256(reinterpret_cast<const simde__m256i*>(&src[i + 8]));

      simde__m256i output1 = simde_mm256_shuffle_epi8(input1, shufmask);
      simde__m256i output2 = simde_mm256_shuffle_epi8(input2, shufmask);

      simde_mm256_storeu_si256(reinterpret_cast<simde__m256i*>(&dest[i]), output1);
      simde_mm256_storeu_si256(reinterpret_cast<simde__m256i*>(&dest[i + 8]), output2);
    }
    if (i + 8 <= count) {
      simde__m256i input =
          simde_mm256_loadu_si256(reinterpret_cast<const simde__m256i*>(&src[i]));
      simde__m256i output = simde_mm256_shuffle_epi8(input, shufmask);
      simde_mm256_storeu_si256(reinterpret_cast<simde__m256i*>(&dest[i]), output);
      i += 8;
    }
    if (i + 4 <= count) {
      simde__m128i input =
          simde_mm_loadu_si128(reinterpret_cast<const simde__m128i*>(&src[i]));
      simde__m128i output =
          simde_mm_shuffle_epi8(input, simde_mm256_castsi256_si128(shufmask));
      simde_mm_storeu_si128(reinterpret_cast<simde__m128i*>(&dest[i]), output);
      i += 4;
    }
  } else {
...
void copy_and_swap_64_aligned(void* dest_ptr, const void* src_ptr,
  ...
  simde__m128i shufmask =
      simde_mm_set_epi8(0x08, 0x09, 0x0A, 0x0B, 0x0C, 0x0D, 0x0E, 0x0F, 0x00, 0x01,
...
void copy_and_swap_16_in_32_aligned(void* dest_ptr, const void* src_ptr,
  ...
  for (i = 0; i + 4 <= count; i += 4) {
    simde__m128i input = simde_mm_load_si128(reinterpret_cast<const simde__m128i*>(&src[i]));
    simde__m128i output =
        simde_mm_or_si128(simde_mm_slli_epi32(input, 16), simde_mm_srli_epi32(input, 16));
    simde_mm_store_si128(reinterpret_cast<simde__m128i*>(&dest[i]), output);
  }
  XE_WORKAROUND_CONSTANT_RETURN_IF(count % 4 == 0);
  for (; i < count; ++i) {  // handle residual elements
