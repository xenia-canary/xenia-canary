...
// Simple debug method that returns state as string - no logging, just formatting
const char* RingBuffer::get_debug_state() const {
  static char buffer[256];
  ring_size_t read_count_val = read_offset_ <= write_offset_ ? 
                               write_offset_ - read_offset_ : 
                               (capacity_ - read_offset_) + write_offset_;
  snprintf(buffer, sizeof(buffer), 
           "RingBuffer[%p]: cap=%u, rd=%u, wr=%u, avail=%u, empty=%c",
           static_cast<void*>(buffer_), capacity_, read_offset_, write_offset_, 
           read_count_val, (read_offset_ == write_offset_) ? 'Y' : 'N');
  return buffer;
}

// Method to check if ring buffer is in a valid state
bool RingBuffer::is_valid() const {
  return buffer_ != nullptr && 
         capacity_ > 0 && 
         read_offset_ < capacity_ && 
