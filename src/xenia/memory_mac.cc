...
DEFINE_int32(scribble_heap_value, 0,
             ...
             "Memory");
DEFINE_bool(
    memory_log_physical_callback_mprotect, false,
    "Debug: Log host protection ranges used by physical memory access "
    "callbacks (watch arm/unprotect).",
    "Memory");
...
const char* PageAccessName(xe::memory::PageAccess access) {
  switch (access) {
...
bool Memory::Initialize() {
  fprintf(stderr, "[Memory] Initialize starting...\n");
  file_name_ = fmt::format("xe_mem_{}", Clock::QueryHostTickCount());

  // Create main page file-backed mapping. This is all reserved but
  // uncommitted (so it shouldn't expand page file).
  fprintf(stderr, "[Memory] Creating file mapping handle: %s\n", file_name_.string().c_str());
  mapping_ = xe::memory::CreateFileMappingHandle(
      file_name_,
      // entire 4gb space + 512mb physical:
      // Note: Physical view 7 uses an offset of 0x100001000 and length 0x20000000,
      // which requires at least 0x120001000 bytes. We use 64k alignment (0x120010000).
      0x120010000, xe::memory::PageAccess::kReadWrite, false);
  if (mapping_ == xe::memory::kFileMappingHandleInvalid) {
    XELOGE("Unable to reserve the 4gb guest address space.");
    fprintf(stderr, "[Memory] ERROR: Unable to reserve 4gb guest address space.\n");
    assert_always();
    return false;
  }
  fprintf(stderr, "[Memory] Mapping handle created: %d\n", mapping_);

  mapping_base_ = 0;
#ifdef __APPLE__
  // On macOS (especially Rosetta 2), let the kernel pick a large continuous block.
  // This avoids clashing with the executable's fixed load address (0x100000000).
  // Apple Silicon requires 16k (we use 64k for safety) alignment for mprotect.
  fprintf(stderr, "[Memory] Probing for 4.5GB continuous block (64k aligned)...\n");
  size_t probe_len = 0x120010000ull; // Match the mapping handle length
  size_t alignment = 64 * 1024;
  void* temp_base = mmap(NULL, probe_len + alignment, PROT_NONE, MAP_PRIVATE | MAP_ANONYMOUS, -1, 0);
  if (temp_base != MAP_FAILED) {
    munmap(temp_base, probe_len + alignment);
    uintptr_t unaligned_addr = reinterpret_cast<uintptr_t>(temp_base);
    uintptr_t aligned_addr = (unaligned_addr + (alignment - 1)) & ~(alignment - 1);
    uint8_t* probed_base = reinterpret_cast<uint8_t*>(aligned_addr);
    fprintf(stderr, "[Memory] Probed mapping base (raw): %p, (aligned): %p\n", temp_base, probed_base);
    if (!MapViews(probed_base)) {
      mapping_base_ = probed_base;
    }
  }
#endif

  if (!mapping_base_) {
    // Attempt to create our views by brute force. This may fail at the first address
    // we pick, so try a few times.
    fprintf(stderr, "[Memory] Mapping views (search mode)...\n");
    for (size_t n = 32; n < 64; n++) {
      auto mapping_base = reinterpret_cast<uint8_t*>(1ull << n);
      fprintf(stderr, "[Memory] Attempting mapping at %p (bit %zu)...\n", mapping_base, n);
      if (!MapViews(mapping_base)) {
        mapping_base_ = mapping_base;
        fprintf(stderr, "[Memory] Successfully mapped views at %p\n", mapping_base_);
        break;
      }
    }
  }
  ...
  if (!mapping_base_) {
    ...
    fprintf(stderr, "[Memory] ERROR: Unable to find continuous block.\n");
    assert_always();
    return false;
  }
  ...
  fprintf(stderr, "[Memory] Initializing virtual heaps...\n");
  ...
  fprintf(stderr, "[Memory] Initializing physical heaps...\n");
  ...
  fprintf(stderr, "[Memory] Protecting initial/final pages...\n");
  ...
  fprintf(stderr, "[Memory] Allocating GPU writeback range...\n");
  ...
  fprintf(stderr, "[Memory] Committing physical memory blocks...\n");
  ...
  fprintf(stderr, "[Memory] Installing MMIO handler...\n");
  ...
  if (!mmio_handler_) {
    ...
    fprintf(stderr, "[Memory] ERROR: Unable to install MMIO handler.\n");
    assert_always();
    return false;
  }

  // ?
  fprintf(stderr, "[Memory] Finalizing setup...\n");
  ...
  fprintf(stderr, "[Memory] Initialize complete.\n");
  return true;
}
...
int Memory::MapViews(uint8_t* mapping_base) {
  ...
  for (size_t n = 0; n < xe::countof(map_info); n++) {
    fprintf(stderr, "[Memory] Mapping view %zu: 0x%016llX - 0x%016llX -> target 0x%016llX\n",
            n, (unsigned long long)(mapping_base + map_info[n].virtual_address_start),
            (unsigned long long)(mapping_base + map_info[n].virtual_address_end),
            (unsigned long long)map_info[n].target_address);
    ...
    if (!views_.all_views[n]) {
      fprintf(stderr, "[Memory] View %zu mapping FAILED.\n", n);
      // Failed, so bail and try again.
      UnmapViews();
      return 1;
    }
    fprintf(stderr, "[Memory] View %zu mapped successfully at 0x%p\n", n, views_.all_views[n]);
  }
  return 0;
}

void Memory::UnmapViews() {
  fprintf(stderr, "[Memory] UnmapViews: Unmapping all 9 views...\n");
  for (size_t n = 0; n < xe::countof(views_.all_views); n++) {
    if (views_.all_views[n]) {
      ...
      fprintf(stderr, "[Memory] Unmapping view %zu at %p (len 0x%zx)...\n", n, views_.all_views[n], length);
...
bool Memory::AddVirtualMappedRange(uint32_t virtual_address, uint32_t mask,
  ...
  uint8_t* host_address = TranslateVirtual(virtual_address);
  XELOGI(
      "DEBUG: BREADCRUMB: AddVirtualMappedRange vaddr=0x{:08X} mask=0x{:08X} "
      "size=0x{:08X} host={} ctx={} read_cb={} write_cb={}",
      virtual_address, mask, size, static_cast<void*>(host_address), context,
      reinterpret_cast<void*>(read_callback),
      reinterpret_cast<void*>(write_callback));
  if (!xe::memory::AllocFixed(host_address, size,
                              xe::memory::AllocationType::kCommit,
                              xe::memory::PageAccess::kNoAccess)) {
    XELOGE("Unable to map range; commit/protect failed");
    return false;
  }
  XELOGI("DEBUG: BREADCRUMB: AddVirtualMappedRange commit OK");
  bool registered =
      mmio_handler_->RegisterRange(virtual_address, mask, size, context,
                                   read_callback, write_callback);
  XELOGI("DEBUG: BREADCRUMB: AddVirtualMappedRange register_range={}",
         registered ? "OK" : "FAIL");
  return registered;
}
...
#ifdef __APPLE__
  const uint64_t alloc_start_ticks = Clock::QueryHostTickCount();
  const uint64_t tick_freq = Clock::QueryHostTickFrequency();
  XELOGI("[Memory] SystemHeapAlloc start: size={} align={} flags=0x{:X} physical={} heap_base={:08X} heap_size={:08X} page_size={}",
         size, alignment, system_heap_flags, is_physical ? "yes" : "no",
         heap ? heap->heap_base() : 0, heap ? heap->heap_size() : 0,
         heap ? heap->page_size() : 0);
#endif
  uint32_t address;
...
#ifdef __APPLE__
    XELOGE("[Memory] SystemHeapAlloc FAILED");
#endif
    return 0;
  }
  Zero(address, size);
#ifdef __APPLE__
  const uint64_t alloc_end_ticks = Clock::QueryHostTickCount();
  if (tick_freq) {
    double duration_ms =
        (alloc_end_ticks - alloc_start_ticks) * 1000.0 / tick_freq;
    if (duration_ms > 2.0) {
      XELOGI("[Memory] SystemHeapAlloc done: addr={:08X} size={} (took {:.2f} ms)",
             address, size, duration_ms);
    }
  }
#endif
  return address;
...
bool BaseHeap::AllocFixed(uint32_t base_address, uint32_t size,
                          ...
                          uint32_t protect) {
  XELOGI("DEBUG: BaseHeap::AllocFixed begin base={:08X} size={} alignment={} alloc_type=0x{:X} protect=0x{:X}",
  ...
  if (allocation_type == kMemoryAllocationReserve) {
    ...
    XELOGI("DEBUG: BaseHeap::AllocFixed host alloc returned ptr={} page_count={} page_size={}",
           result, page_count, page_size_);
  ...
  XELOGI("DEBUG: BaseHeap::AllocFixed end base={:08X} size={} page_count={} success=YES",
...
bool BaseHeap::AllocRange(uint32_t low_address, uint32_t high_address,
  ...
  *out_address = 0;
...
#ifdef __APPLE__
  const uint64_t scan_start_ticks = Clock::QueryHostTickCount();
  const uint64_t tick_freq = Clock::QueryHostTickFrequency();
  uint64_t last_log_ticks = scan_start_ticks;
  ...
  if (top_down) {
...
#ifdef __APPLE__
      if (++scan_steps % 4096 == 0) {
        const uint64_t now_ticks = Clock::QueryHostTickCount();
        if (tick_freq && (now_ticks - last_log_ticks) > (tick_freq / 2)) {
          XELOGW("BaseHeap::AllocRange scanning (top-down): heap_base={:08X} size={} pages={} low={:08X} high={:08X}",
                 heap_base_, size, page_count, low_address, high_address);
    ...
    for (uint32_t base_page_number = low_page_number;
      ...
      if (++scan_steps % 4096 == 0) {
        const uint64_t now_ticks = Clock::QueryHostTickCount();
        if (tick_freq && (now_ticks - last_log_ticks) > (tick_freq / 2)) {
          XELOGW("BaseHeap::AllocRange scanning (bottom-up): heap_base={:08X} size={} pages={} low={:08X} high={:08X}",
                 heap_base_, size, page_count, low_address, high_address);
          last_log_ticks = now_ticks;
        }
      }
  ...
  if (allocation_type == kMemoryAllocationReserve) {
    ...
    auto alloc_type = (allocation_type & kMemoryAllocationCommit)
                          ...
                          : xe::memory::AllocationType::kReserve;
#ifdef __APPLE__
    XELOGI("BaseHeap::AllocRange host alloc: addr={:08X} size={} pages={} protect=0x{:X}",
           heap_base_ + (start_page_number << page_size_shift_),
           page_count << page_size_shift_, page_count, protect);
#endif
    void* result = xe::memory::AllocFixed(
  ...
  XELOGI("DEBUG: BaseHeap::AllocRange host alloc returned ptr={} start_page={} end_page={}",
       result, start_page_number, end_page_number);
  ...
  *out_address = heap_base_ + (start_page_number << page_size_shift_);
  XELOGI("DEBUG: BaseHeap::AllocRange end out_address={:08X} size={} page_count={}",
...
bool BaseHeap::Protect(uint32_t address, uint32_t size, uint32_t protect,
  ...
  if (end_page_number < start_page_number) {
    uint64_t addr_end = uint64_t(address) + size - 1;
    uint64_t addr_delta = uint64_t(address) - heap_base_;
    XELOGE("BaseHeap::Protect failed due to invalid page range: start={} end={} "
           "(addr={:08X} size={:08X} addr_end={:08X} addr_delta={:08X} "
           "heap_base={:08X} heap_size={:08X} page_size={} shift={})",
           start_page_number, end_page_number, address, size,
           static_cast<uint32_t>(addr_end),
           static_cast<uint32_t>(addr_delta),
           heap_base_, heap_size_, page_size_, page_size_shift_);
    return false;
  }
  ...
  uint32_t protection_size_bytes = page_count << page_size_shift_;
  
  // Pre-check for unreasonably large protection ranges that will fail on macOS
  // Prevent attempting to protect >512MB ranges in single call
  const uint32_t MAX_REASONABLE_PROTECTION_SIZE = 512 * 1024 * 1024; // 512MB
  if (protection_size_bytes > MAX_REASONABLE_PROTECTION_SIZE) {
    printf("DEBUG: BREADCRUMB: BaseHeap::Protect LARGE REQUEST: addr=%08X, size=%08X, pages=%u, protection_size=%08X\n", 
           address, size, page_count, (uint32_t)protection_size_bytes);
    XELOGW("BaseHeap::Protect: Rejecting large protection request ({} bytes > {} max) - will use chunking fallback", 
             protection_size_bytes, MAX_REASONABLE_PROTECTION_SIZE);
    return false;
  } else if (page_size_ == xe_page_size ||
      ((((protection_size_bytes) & page_size_mask) == 0) &&
       (((start_page_number << page_size_shift_) & page_size_mask) == 0))) {
    memory::PageAccess old_protect_access;
    bool protect_result = xe::memory::Protect(
            TranslateRelative(start_page_number << page_size_shift_),
            protection_size_bytes, ToPageAccess(protect),
            old_protect ? &old_protect_access : nullptr);
            
#ifdef __APPLE__
    // On macOS, mprotect can fail with ENOMEM for very large ranges even when
    // chunked. For NoAccess protection (guard pages), this is non-fatal - we
    // can continue without host-level protection since the guest page table
    // will still track the state.
    if (!protect_result && ToPageAccess(protect) == memory::PageAccess::kNoAccess) {
      XELOGW("BaseHeap::Protect: macOS host protection failed for guard pages (size 0x{:X}), continuing with guest-only protection",
             page_count << page_size_shift_);
      // Continue - the guest page table protection below will still work  
    } else
#endif
    if (!protect_result) {
...
#ifdef __APPLE__
      system_page_count_ = static_cast<uint32_t>((size_t(heap_size_) + host_address_offset + (system_page_size_ - 1)) / system_page_size_);
#else
      system_page_count_ = (size_t(heap_size_) + host_address_offset + (system_page_size_ - 1)) / system_page_size_;
#endif

  // Initialize system_page_flags_ vector with the calculated size
...
XE_NOINLINE void PhysicalHeap::EnableAccessCallbacksInner(
  ...
  for (; i <= system_page_last; ++i) {
    ...
    if (protect_system_page) {
      ...
      if (protect_system_page_first != UINT32_MAX) {
        uint32_t protect_page_count = i - protect_system_page_first;
        uint32_t protect_size = protect_page_count << system_page_shift_;
        uint32_t protect_virtual_start =
            xe::sat_sub(protect_system_page_first << system_page_shift_,
                        host_address_offset()) +
            heap_base_;
        uint32_t protect_physical_start = GetPhysicalAddress(protect_virtual_start);
        if (cvars::memory_log_physical_callback_mprotect) {
          XELOGI(
              "MEMORY WATCH: arm mprotect access={} host={} size={:08X} "
              "system_pages=[{}..{}) virtual_start={:08X} physical_start={:08X}",
              PageAccessName(protect_access),
              static_cast<const void*>(
                  protect_base +
                  (protect_system_page_first << system_page_shift_)),
              protect_size, protect_system_page_first, i, protect_virtual_start,
              protect_physical_start);
        }
        xe::memory::Protect(
  ...
  if (protect_system_page_first != UINT32_MAX) {
    ...
    if (cvars::memory_log_physical_callback_mprotect) {
      XELOGI(
          ...
          static_cast<const void*>(
              protect_base + (protect_system_page_first << system_page_shift_)),
          protect_size, protect_system_page_first, system_page_last,
          protect_virtual_start, protect_physical_start);
    }
    xe::memory::Protect(
        protect_base + (protect_system_page_first << system_page_shift_),
        protect_size,
...
bool PhysicalHeap::TriggerCallbacks(
  ...
  if (cvars::memory_log_physical_callback_mprotect) {
    uint32_t physical_address_offset = GetPhysicalAddress(heap_base_);
    uint32_t physical_address_start_log =
        xe::sat_sub(system_page_first << system_page_shift_, host_address_offset()) +
        physical_address_offset;
    uint32_t physical_length_log = std::min(
        xe::sat_sub((system_page_last << system_page_shift_) + system_page_size_,
                    host_address_offset()) +
            physical_address_offset - physical_address_start_log,
        heap_size_ - (physical_address_start_log - physical_address_offset));
    XELOGI(
        "MEMORY WATCH: trigger callbacks virtual={:08X} length={:08X} "
        "is_write={} unprotect={} unwatch_exact={} system_pages=[{}..{}] "
        "physical=[{:08X}..{:08X})",
        virtual_address, length, is_write ? "YES" : "NO",
        unprotect ? "YES" : "NO", unwatch_exact_range ? "YES" : "NO",
        system_page_first, system_page_last, physical_address_start_log,
        physical_address_start_log + physical_length_log);
  }
  ...
  if (unprotect) {
    ...
    for (uint32_t i = system_page_first; i <= system_page_last; ++i) {
      ...
      if (unprotect_page) {
        ...
        if (unprotect_system_page_first != UINT32_MAX) {
          uint32_t unprotect_page_count = i - unprotect_system_page_first;
          uint32_t unprotect_size = unprotect_page_count << system_page_shift_;
          uint32_t unprotect_virtual_start =
              xe::sat_sub(unprotect_system_page_first << system_page_shift_,
                          host_address_offset()) +
              heap_base_;
          uint32_t unprotect_physical_start =
              GetPhysicalAddress(unprotect_virtual_start);
          if (cvars::memory_log_physical_callback_mprotect) {
            XELOGI(
                "MEMORY WATCH: unprotect mprotect access=ReadWrite host={} "
                "size={:08X} system_pages=[{}..{}) virtual_start={:08X} "
                "physical_start={:08X}",
                static_cast<const void*>(
                    protect_base +
                    (unprotect_system_page_first << system_page_shift_)),
                unprotect_size, unprotect_system_page_first, i,
                unprotect_virtual_start, unprotect_physical_start);
          }
          xe::memory::Protect(
              protect_base +
                  (unprotect_system_page_first << system_page_shift_),
    ...
    if (unprotect_system_page_first != UINT32_MAX) {
      ...
      if (cvars::memory_log_physical_callback_mprotect) {
        XELOGI(
            ...
            unprotect_size, unprotect_system_page_first, system_page_last,
            unprotect_virtual_start, unprotect_physical_start);
      }
      xe::memory::Protect(
          protect_base + (unprotect_system_page_first << system_page_shift_),
