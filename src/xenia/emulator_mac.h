...
#ifndef XENIA_EMULATOR_MAC_H_
#define XENIA_EMULATOR_MAC_H_
...
class Emulator {
 public:
  ...
  bool HasGuestMainThread() const;
  bool IsGuestMainThreadRunning() const;
  bool EnsureGuestMainThreadRunning(const char* reason = nullptr);
  void LogMainThreadGpuDispatchStallSample(const char* reason,
                                           uint32_t thunk_01C3,
                                           uint32_t thunk_01D5,
                                           uint32_t thunk_025B) const;
  bool GetLastGpuDispatchStallSnapshot(std::string* out_state,
                                       std::string* out_detail,
                                       std::string* out_chain,
                                       std::string* out_target_summaries,
                                       uint32_t* out_repeat_samples,
                                       uint64_t* out_now_ms) const;
  void CloseModuleLoadGate(const std::string_view reason);
  void OpenModuleLoadGate(const std::string_view reason);
  bool IsModuleLoadGateClosed() const;
 ...
 private:
  ...
  void LogMainThreadPcSample(const char* reason) const;
  ...
  std::atomic<bool> module_load_gate_closed_{false};
