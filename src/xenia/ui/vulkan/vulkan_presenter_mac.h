...
#ifndef XENIA_UI_VULKAN_VULKAN_PRESENTER_MAC_H_
#define XENIA_UI_VULKAN_VULKAN_PRESENTER_MAC_H_
...
class VulkanPresenter final : public Presenter {
 public:
  ...
  static std::unique_ptr<VulkanPresenter> Create(
    ...
    XELOGI("DEBUG: VulkanPresenter::Create() STARTED - EXTREME DEBUG MODE");
    XELOGI("DEBUG: THREAD ID: {}", std::this_thread::get_id());
    XELOGI("DEBUG: host_gpu_loss_callback: {}", static_cast<void*>(&host_gpu_loss_callback));
    XELOGI("DEBUG: vulkan_device: {}", static_cast<const void*>(vulkan_device));
    XELOGI("DEBUG: ui_samplers: {}", static_cast<const void*>(ui_samplers));
    auto thread_id = std::this_thread::get_id();
    fprintf(stderr, "DEBUG: VULKAN PRESENTER CREATE CALLED! Thread: %p\n", 
            reinterpret_cast<const void*>(&thread_id));
    
    if (!vulkan_device) {
      XELOGE("DEBUG: VulkanPresenter::Create(): vulkan_device is nullptr");
      return nullptr;
    }
    
    if (!ui_samplers) {
      XELOGE("DEBUG: VulkanPresenter::Create(): ui_samplers is nullptr");
      return nullptr;
    }
    
    try {
      XELOGI("DEBUG: VulkanPresenter::Create(): Allocating VulkanPresenter instance");
      auto presenter = std::unique_ptr<VulkanPresenter>(new VulkanPresenter(
          host_gpu_loss_callback, vulkan_device, ui_samplers));
      XELOGI("DEBUG: VulkanPresenter::Create(): VulkanPresenter instance allocated at {}", 
              static_cast<void*>(presenter.get()));
      
      XELOGI("DEBUG: VulkanPresenter::Create(): Calling InitializeSurfaceIndependent()");
      bool init_result = presenter->InitializeSurfaceIndependent();
      XELOGI("DEBUG: VulkanPresenter::Create(): InitializeSurfaceIndependent() returned: {}", init_result);
      
      if (!init_result) {
        XELOGE("DEBUG: VulkanPresenter::Create(): InitializeSurfaceIndependent() failed");
        return nullptr;
      }
      
      XELOGI("DEBUG: VulkanPresenter::Create(): SUCCESS - presenter created at {}", 
              static_cast<void*>(presenter.get()));
      return presenter;
      
    } catch (const std::exception& e) {
      XELOGE("DEBUG: VulkanPresenter::Create(): Exception during creation: {}", e.what());
      return nullptr;
    } catch (...) {
      XELOGE("DEBUG: VulkanPresenter::Create(): Unknown exception during creation");
      return nullptr;
    }
  }
 ...
 private:
  ...
  // Comprehensive validation for macOS Metal surface binding
  void ValidateMetalSurfaceBinding();
