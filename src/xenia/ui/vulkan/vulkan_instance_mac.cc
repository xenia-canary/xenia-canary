...
DEFINE_bool(
    ...
    "Vulkan");
DEFINE_bool(
    vulkan_disable_gfxrecon_layer, false,
    "Disable the GFXReconstruct Vulkan layer "
    "(VK_LAYER_LUNARG_gfxreconstruct) by setting VK_LOADER_LAYERS_DISABLE.",
    "Vulkan");
DEFINE_string(
    vulkan_disable_layers, "",
    "Comma-separated Vulkan layer names to disable for this process (sets "
    "VK_LOADER_LAYERS_DISABLE before instance creation).",
    "Vulkan");
...
std::unique_ptr<VulkanInstance> VulkanInstance::Create(
  ...
  auto append_layer = [](std::string* list, const std::string& layer) {
    if (!list || layer.empty()) {
      return;
    }
    if (list->find(layer) != std::string::npos) {
      return;
    }
    if (!list->empty()) {
      list->append(",");
  ...
  if (const char* env_layers = std::getenv("VK_INSTANCE_LAYERS")) {
    if (*env_layers) {
      XELOGI("Vulkan env VK_INSTANCE_LAYERS={}", env_layers);
    }
  }
  if (const char* env_settings = std::getenv("VK_LAYER_SETTINGS_PATH")) {
    if (*env_settings) {
      XELOGI("Vulkan env VK_LAYER_SETTINGS_PATH={}", env_settings);
    }
  }
  if (const char* env_settings_file =
          std::getenv("VK_LAYER_SETTINGS_FILENAME")) {
    if (*env_settings_file) {
      XELOGI("Vulkan env VK_LAYER_SETTINGS_FILENAME={}", env_settings_file);
    }
  }

  std::string disable_layers = cvars::vulkan_disable_layers;
  if (cvars::vulkan_disable_gfxrecon_layer) {
    append_layer(&disable_layers, "VK_LAYER_LUNARG_gfxreconstruct");
  }
  if (const char* existing_disable =
          std::getenv("VK_LOADER_LAYERS_DISABLE")) {
    if (*existing_disable) {
      if (!disable_layers.empty()) {
        disable_layers =
            std::string(existing_disable) + "," + disable_layers;
      } else {
        disable_layers = existing_disable;
      }
    }
  }
  if (!disable_layers.empty()) {
    set_env("VK_LOADER_LAYERS_DISABLE", disable_layers);
    XELOGI("Vulkan: VK_LOADER_LAYERS_DISABLE={}", disable_layers);
  }
