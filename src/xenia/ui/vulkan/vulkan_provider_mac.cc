...
std::unique_ptr<Presenter> VulkanProvider::CreatePresenter(
  ...
  XELOGI("VulkanProvider::CreatePresenter() CALLED!");
  {
    auto thread_id = std::this_thread::get_id();
    XELOGI("THREAD ID: {}", reinterpret_cast<const void*>(&thread_id));
  }  XELOGI("vulkan_device(): {}", static_cast<void*>(vulkan_device()));
  XELOGI("xenia thread id: {}", xe::threading::current_thread_id());
  XELOGI("ui_samplers(): {}", static_cast<const void*>(ui_samplers()));
  XELOGI("host_gpu_loss_callback: {}", static_cast<const void*>(&host_gpu_loss_callback));
  {
    auto thread_id = std::this_thread::get_id();
    fprintf(stderr, "VULKAN PROVIDER CREATE PRESENTER CALLED! Thread: %p\n", 
            reinterpret_cast<const void*>(&thread_id));
  }
  
  auto result = VulkanPresenter::Create(host_gpu_loss_callback, vulkan_device(), ui_samplers());
  XELOGI("VulkanPresenter::Create() returned: {}", static_cast<void*>(result.get()));
  fprintf(stderr, "VULKAN PRESENTER CREATE RESULT: %p\n", static_cast<void*>(result.get()));
