...
DEFINE_bool(
    ...
    "Vulkan");
DEFINE_bool(
    vulkan_presenter_force_clear, false,
    "Force a solid-color clear of the swapchain each frame and skip guest "
    "output painting (debugging swapchain/present path).",
    "Vulkan");
DEFINE_bool(
    vulkan_presenter_verbose, true,
    "Enable verbose logging in Vulkan presenter (per-frame acquire/submit/present).",
    "Vulkan");
DEFINE_int32(
    vulkan_presenter_log_every_n, 1,
    "Log every N Vulkan presenter frames when verbose logging is enabled.",
    "Vulkan");
...
Surface::TypeFlags VulkanPresenter::GetSurfaceTypesSupportedByInstance(
  ...
  XELOGI("DEBUG: VulkanPresenter: Checking supported surface types...");
  XELOGI("Thread ID: {} (xenia id {})", std::this_thread::get_id(),
         xe::threading::current_thread_id());
  XELOGI("   VK_KHR_surface: {}", instance_extensions.ext_KHR_surface ? "YES" : "NO");

  if (!instance_extensions.ext_KHR_surface) {
    XELOGE("DEBUG: VK_KHR_surface NOT available - no surfaces supported!");
    return 0;
  }

  Surface::TypeFlags type_flags = 0;
#if XE_PLATFORM_ANDROID
  XELOGI("   VK_KHR_android_surface: {}", instance_extensions.ext_KHR_android_surface ? "YES" : "NO");
  if (instance_extensions.ext_KHR_android_surface) {
    type_flags |= Surface::kTypeFlag_AndroidNativeWindow;
  }
#endif
#if XE_PLATFORM_GNU_LINUX
  XELOGI("   VK_KHR_xcb_surface: {}", instance_extensions.ext_KHR_xcb_surface ? "YES" : "NO");
  if (instance_extensions.ext_KHR_xcb_surface) {
    type_flags |= Surface::kTypeFlag_XcbWindow;
  }
#endif
#if XE_PLATFORM_WIN32
  XELOGI("   VK_KHR_win32_surface: {}", instance_extensions.ext_KHR_win32_surface ? "YES" : "NO");
  if (instance_extensions.ext_KHR_win32_surface) {
    type_flags |= Surface::kTypeFlag_Win32Hwnd;
  }
#endif
#ifdef __APPLE__
  XELOGI("   VK_EXT_metal_surface: {}", instance_extensions.ext_EXT_metal_surface ? "YES" : "NO");
  if (instance_extensions.ext_EXT_metal_surface) {
    type_flags |= kTypeFlag_MacNSView;
    XELOGI("DEBUG: macOS Metal surface support ENABLED");
  } else {
    XELOGE("DEBUG: VK_EXT_metal_surface NOT available - macOS rendering will FAIL!");
  }
#endif

  XELOGI("Supported surface type flags: 0x{:08X}", type_flags);
  return type_flags;
}
...
VulkanPresenter::ConnectOrReconnectPaintingToSurfaceFromUIThread(
  ...
  if (paint_context_.swapchain == VK_NULL_HANDLE) {
    ...
    // extensions.
    XELOGI("DEBUG: VulkanPresenter: Creating Vulkan surface...");
    XELOGI("Surface type index: {}", static_cast<uint32_t>(surface_type));
    Surface::TypeFlags supported_types = GetSupportedSurfaceTypes();
    XELOGI("Supported surface types: 0x{:08X}", supported_types);
    XELOGI("Required surface type flag: 0x{:08X}", Surface::TypeFlags(1) << surface_type);

    if (!(supported_types & (Surface::TypeFlags(1) << surface_type))) {
      XELOGE(
          "VulkanPresenter: DEBUG: Tried to create a Vulkan surface for an "
          "unsupported Xenia surface type (type index: {})", static_cast<uint32_t>(surface_type));
      return SurfacePaintConnectResult::kFailureSurfaceUnusable;
    }
    XELOGI("   DEBUG: Surface type is supported");
...
#endif
#ifdef __APPLE__
      case kTypeIndex_MacNSView: {
        XELOGI("DEBUG: macOS Metal surface creation...");
        auto& mac_nsview_surface =
            static_cast<const MacNSViewSurface&>(new_surface);
        XELOGI("NSView pointer: {}", mac_nsview_surface.nsview());
        XELOGI("CAMetalLayer pointer: {}", mac_nsview_surface.metal_layer());

        if (!mac_nsview_surface.metal_layer()) {
          XELOGE("DEBUG: CAMetalLayer is NULL - cannot create Metal surface!");
          vulkan_surface_create_result = VK_ERROR_INITIALIZATION_FAILED;
          break;
        }
        ...
        surface_create_info.pNext = nullptr;
        surface_create_info.flags = 0;
        surface_create_info.pLayer = mac_nsview_surface.metal_layer();

        XELOGI("Calling vkCreateMetalSurfaceEXT...");
        vulkan_surface_create_result =
            ifn.vkCreateMetalSurfaceEXT(instance, &surface_create_info, nullptr,
                                        &paint_context_.vulkan_surface);
        XELOGI("vkCreateMetalSurfaceEXT result: {} ({})",
               static_cast<int32_t>(vulkan_surface_create_result),
               vulkan_surface_create_result == VK_SUCCESS ? "SUCCESS" : "FAILED");
      } break;
#endif
      default:
    ...
    if (vulkan_surface_create_result != VK_SUCCESS) {
      XELOGE("VulkanPresenter: DEBUG: Failed to create a Vulkan surface - VkResult: {} (0x{:08X})",
             static_cast<int32_t>(vulkan_surface_create_result),
             static_cast<uint32_t>(vulkan_surface_create_result));
      return SurfacePaintConnectResult::kFailure;
    }
    XELOGI("DEBUG: Vulkan surface created successfully");

    XELOGI("Creating swapchain for surface...");
    XELOGI("Surface dimensions: {}x{}", new_surface_width, new_surface_height);
    bool surface_unusable;
    paint_context_.swapchain = PaintContext::CreateSwapchainForVulkanSurface(
        vulkan_device_, paint_context_.vulkan_surface, new_surface_width,
    ...
    if (paint_context_.swapchain == VK_NULL_HANDLE) {
      ...
      // Vulkan surface.
      XELOGE("DEBUG: Failed to create swapchain (surface_unusable: {})", surface_unusable);
    ...
    XELOGI("DEBUG: Swapchain created successfully");
    XELOGI("Extent: {}x{}", paint_context_.swapchain_extent.width, paint_context_.swapchain_extent.height);
    XELOGI("Format: {}", static_cast<int32_t>(new_swapchain_format));
    XELOGI("FIFO: {}", paint_context_.swapchain_is_fifo ? "YES" : "NO");
  ...
  if (paint_context_.swapchain_render_pass == VK_NULL_HANDLE) {
    ...
    XELOGE("Creating render pass for swapchain...");
    render_pass_attachment.flags = 0;
    XELOGE("Attachment flags: {}", render_pass_attachment.flags);
    render_pass_attachment.format = new_swapchain_format;
    XELOGE("Attachment format: {}", static_cast<int32_t>(new_swapchain_format));
    render_pass_attachment.samples = VK_SAMPLE_COUNT_1_BIT;
    XELOGE("Attachment samples: {}", static_cast<int32_t>(render_pass_attachment.samples));
...
VkSwapchainKHR VulkanPresenter::PaintContext::CreateSwapchainForVulkanSurface(
  ...
  XELOGI("DEBUG: CreateSwapchainForVulkanSurface START");
  ...
  XELOGI("Target dimensions: {}x{}, Clamped dimensions: {}x{}",
  ...
  XELOGI("Surface formats reported: {}", surface_formats.size());
  for (size_t i = 0; i < surface_formats.size(); ++i) {
    XELOGI("format[{}]={} colorspace={}", i,
           static_cast<int>(surface_formats[i].format),
           static_cast<int>(surface_formats[i].colorSpace));
  }
#if XE_PLATFORM_ANDROID
  ...
  }
  {
    ...
    for (const auto& fmt : surface_formats) {
      if (fmt.format == image_format.format &&
          fmt.colorSpace == image_format.colorSpace) {
        format_supported = true;
        break;
      }
    }
    XELOGI("Chosen swapchain format: {} colorspace={} supported={}",
           static_cast<int>(image_format.format),
           static_cast<int>(image_format.colorSpace),
           format_supported ? "YES" : "NO");
    if (!format_supported && !surface_formats.empty()) {
      XELOGW(
          "DEBUG: Chosen format not in reported list - falling back to first "
          "reported format");
      image_format = surface_formats[0];
      XELOGI("Fallback swapchain format: {} colorspace={}",
             static_cast<int>(image_format.format),
             static_cast<int>(image_format.colorSpace));
    }
  }
...
Presenter::PaintResult VulkanPresenter::PaintAndPresentImpl(
  ...
  static std::atomic<bool> logged_first_paint{false};
  static std::atomic<bool> logged_first_acquire{false};
  static std::atomic<bool> logged_first_present{false};
  static std::atomic<bool> logged_force_clear{false};
  static std::atomic<uint64_t> last_no_guest_output_log_ms{0};
  static std::atomic<uint64_t> last_auto_force_clear_log_ms{0};
  ...
  int log_every = cvars::vulkan_presenter_log_every_n;
  bool log_verbose =
      cvars::vulkan_presenter_verbose &&
      (paint_call_id <= 5 || (log_every > 0 && (paint_call_id % log_every) == 0));
  bool force_clear_only = cvars::vulkan_presenter_force_clear;
  bool force_clear_only_auto = false;
  if (force_clear_only && !logged_force_clear.exchange(true)) {
    XELOGI(
        "DEBUG: VulkanPresenter: force-clear mode enabled (guest output will be "
        "skipped)");
  }

  if (paint_context_.swapchain == VK_NULL_HANDLE) {
    XELOGW("DEBUG: VulkanPresenter: swapchain is null, skipping paint");
    return PaintResult::kNotPresentedConnectionOutdated;
  }

  if (!logged_first_paint.exchange(true)) {
    XELOGI("DEBUG: VulkanPresenter: first PaintAndPresentImpl call (ui_drawers={})",
           execute_ui_drawers ? "YES" : "NO");
    XELOGI("   swapchain_extent={}x{}",
           paint_context_.swapchain_extent.width,
           paint_context_.swapchain_extent.height);
      LogGuestOutputBootstrapState(
     "VulkanPresenter::PaintAndPresentImpl first-call");
  }
  if (log_verbose) {
    XELOGI(
        "DEBUG: VulkanPresenter: PaintAndPresentImpl #{} ui_drawers={} swapchain={}x{} "
        "queue_gfx={} queue_present={}",
  ...
  if (!logged_first_acquire.exchange(true)) {
    XELOGI("DEBUG: VulkanPresenter: first swapchain image acquired (index={}, result={})",
           swapchain_image_index,
           static_cast<int32_t>(acquire_result));
  }
  if (log_verbose) {
    XELOGI("DEBUG: VulkanPresenter: acquire #{} result={} image_index={}",
           paint_call_id, static_cast<int32_t>(acquire_result),
           swapchain_image_index);
  }
  ...
  auto configure_force_clear_color = [&](const char* reason) {
    static std::atomic<uint64_t> clear_frame_count{0};
    uint64_t frame = clear_frame_count.fetch_add(1) + 1;
    bool toggle = ((frame / 60) % 2) == 0;
    swapchain_image_clear_attachment.clearValue.color.float32[0] =
        toggle ? 1.0f : 0.1f;
    swapchain_image_clear_attachment.clearValue.color.float32[1] =
        toggle ? 0.0f : 0.9f;
    swapchain_image_clear_attachment.clearValue.color.float32[2] =
        toggle ? 1.0f : 0.1f;
    if (frame <= 3 || (frame % 120) == 0) {
      XELOGI(
          "DEBUG: VulkanPresenter: force clear frame={} reason={} color=({}, {}, "
          "{})",
          frame, reason,
          swapchain_image_clear_attachment.clearValue.color.float32[0],
          swapchain_image_clear_attachment.clearValue.color.float32[1],
  ...
  if (force_clear_only) {
    configure_force_clear_color("cvar");
  }
  ...
  if (!guest_output_image && !force_clear_only) {
    uint64_t now = now_ms();
    uint64_t last = last_no_guest_output_log_ms.load(std::memory_order_relaxed);
    uint64_t refresh_attempt_count = GetGuestOutputRefreshAttemptCount();
    uint64_t refresh_success_count = GetGuestOutputRefreshSuccessCount();
    int32_t mailbox_log_index = guest_output_mailbox_index == UINT32_MAX
                                    ? int32_t(-1)
                                    : int32_t(guest_output_mailbox_index);
    const char* reason =
        guest_output_mailbox_index == UINT32_MAX
            ? "mailbox-inactive-no-refresh"
            : "mailbox-active-image-missing";
    if (last == 0 || now - last > 2000) {
      XELOGI(
          "DEBUG: VulkanPresenter: no guest output image available yet "
          "(reason={}, mailbox={}, frontbuffer={}x{}, swapchain={}x{}, "
          "refresh_attempt_count={}, refresh_success_count={}). "
          "Likely upstream: no VdSwap/RefreshGuestOutput frame produced yet.",
          reason, mailbox_log_index, guest_output_properties.frontbuffer_width,
          guest_output_properties.frontbuffer_height,
          paint_context_.swapchain_extent.width,
          paint_context_.swapchain_extent.height, refresh_attempt_count,
          refresh_success_count);
        if (refresh_success_count == 0 && refresh_attempt_count == 0) {
        XELOGW(
          "DEBUG: VulkanPresenter: missing_guest_output with zero refresh "
          "attempts - producer path never called RefreshGuestOutput yet "
          "(IssueSwap/VdSwap likely not reached).");
        } else if (refresh_success_count == 0) {
        XELOGW(
            "DEBUG: VulkanPresenter: sustained missing_guest_output signal "
            "(zero successful guest refreshes so far). This usually means "
            "GPU bootstrap/ring buffer path is not producing swap packets "
            "yet (check VdInitializeRingBuffer/VdSetGraphicsInterruptCallback/"
            "VdSwap progression)."
        );
      }
      last_no_guest_output_log_ms.store(now, std::memory_order_relaxed);
    }
  }

  if (!guest_output_image && !force_clear_only) {
    force_clear_only = true;
    force_clear_only_auto = true;
    configure_force_clear_color("missing_guest_output");
  }
  if (force_clear_only_auto) {
    uint64_t now = now_ms();
    uint64_t last =
        last_auto_force_clear_log_ms.load(std::memory_order_relaxed);
    if (last == 0 || now - last > 2000) {
      XELOGI(
          "DEBUG: VulkanPresenter: forcing clear-only frame because guest output "
          "image is missing (mailbox={}, frontbuffer={}x{}, swapchain={}x{})",
          guest_output_mailbox_index == UINT32_MAX
              ? int32_t(-1)
              : int32_t(guest_output_mailbox_index),
          ...
          paint_context_.swapchain_extent.width,
          paint_context_.swapchain_extent.height);
      last_auto_force_clear_log_ms.store(now, std::memory_order_relaxed);
    }
  }

  if (force_clear_only && guest_output_image) {
    guest_output_image.reset();
  }

  if (guest_output_image && !force_clear_only) {
    static std::atomic<bool> logged_first_guest_image{false};
    if (!logged_first_guest_image.exchange(true)) {
      VkExtent2D extent = guest_output_image->extent();
      XELOGI("DEBUG: VulkanPresenter: first guest output image available ({}x{})",
    ...
    if (log_verbose) {
      ...
      if (guest_output_flow.effect_count) {
        const auto& final_output_size =
            guest_output_flow
                .effect_output_sizes[guest_output_flow.effect_count - 1];
        output_width = final_output_size.first;
        output_height = final_output_size.second;
      }
      XELOGI(
          "DEBUG: VulkanPresenter: guest_output_flow effects={} output={}x{} pos=({}, {}) "
          "letterbox_rects={}",
          guest_output_flow.effect_count,
          output_width,
          output_height,
          guest_output_flow.output_x,
          guest_output_flow.output_y,
    ...
    if (guest_output_flow.effect_count) {
        ...
        if (intermediate_current_extent.width !=
          ...
          if (intermediate_needed_size.first &&
            ...
            if (!intermediate_image_ptr_ref) {
              ...
              static std::atomic<bool> logged_intermediate_create_fail{false};
              if (!logged_intermediate_create_fail.exchange(true)) {
                XELOGW(
                    "DEBUG: VulkanPresenter: failed to create guest output intermediate image ({}x{})",
                    intermediate_needed_size.first,
                    intermediate_needed_size.second);
              }
            ...
            if (dfn.vkCreateFramebuffer(
              ...
              // Don't display the guest output, and don't try to create more
              // intermediate textures (only destroy them).
              static std::atomic<bool> logged_intermediate_fb_fail{false};
              if (!logged_intermediate_fb_fail.exchange(true)) {
                XELOGW(
                    "DEBUG: VulkanPresenter: intermediate framebuffer creation failed; disabling guest output paint");
              }
              guest_output_flow.effect_count = 0;
              continue;
            }
      ...
      if (guest_output_flow.effect_count) {
        ...
        for (size_t i = 0; i + 1 < guest_output_flow.effect_count; ++i) {
          if (paint_context_
            ...
            static std::atomic<bool> logged_missing_intermediate_pipeline{false};
            if (!logged_missing_intermediate_pipeline.exchange(true)) {
              XELOGW(
                  "DEBUG: VulkanPresenter: missing intermediate guest output pipeline; disabling guest output paint");
            }
            guest_output_flow.effect_count = 0;
            break;
          }
        ...
        if (swapchain_effect_pipeline.swapchain_pipeline == VK_NULL_HANDLE) {
          ...
          if (swapchain_effect_pipeline.swapchain_pipeline == VK_NULL_HANDLE) {
            static std::atomic<bool> logged_swapchain_pipeline_fail{false};
            if (!logged_swapchain_pipeline_fail.exchange(true)) {
              XELOGW(
                  "DEBUG: VulkanPresenter: failed to create swapchain guest output pipeline; disabling guest output paint");
            }
            guest_output_flow.effect_count = 0;
          }
        }
      ...
      if (guest_output_flow.effect_count) {
        static std::atomic<bool> logged_first_guest_output_paint{false};
        if (!logged_first_guest_output_paint.exchange(true)) {
          XELOGI(
              "DEBUG: VulkanPresenter: painting guest output (effects={}, swapchain {}x{})",
              guest_output_flow.effect_count,
              paint_context_.swapchain_extent.width,
              paint_context_.swapchain_extent.height);
        }
  ...
  if (log_verbose) {
    ...
    uint64_t refresh_success_count = GetGuestOutputRefreshSuccessCount();
    XELOGI(
        "DEBUG: VulkanPresenter: guest_output_image={} force_clear_only={} "
      "force_clear_reason={} mailbox={} frontbuffer={}x{} "
      "refresh_attempt_count={} refresh_success_count={}",
        guest_output_image ? "YES" : "NO", force_clear_only ? "YES" : "NO",
        force_clear_only ? (force_clear_only_auto ? "missing_guest_output"
                                                  : "cvar")
                         : "none",
        mailbox_log_index, guest_output_properties.frontbuffer_width,
      guest_output_properties.frontbuffer_height, refresh_attempt_count,
      refresh_success_count);
    if (force_clear_only && force_clear_only_auto &&
        refresh_success_count == 0) {
      XELOGW(
          "DEBUG: VulkanPresenter: force_clear_only is auto-latched by missing "
          "guest output with zero successful refreshes; treating this as an "
          "upstream GPU bootstrap starvation symptom, not a presenter-local "
          "failure.");
    }
  }
  ...
  submit_info.pSignalSemaphores = &present_semaphore;
  {
    ...
    if (log_verbose) {
      XELOGI(
          "DEBUG: VulkanPresenter: submit #{} result={} command_buffers={} "
          "wait_semaphores={} signal_semaphores=1",
          paint_call_id, static_cast<int32_t>(submit_result),
  ...
  if (log_verbose) {
    XELOGI("DEBUG: VulkanPresenter: present #{} result={} image_index={}",
           paint_call_id, static_cast<int32_t>(present_result),
           swapchain_image_index);
  }
    ...
    case VK_SUCCESS:
      {
        ...
        uint64_t count = present_count.fetch_add(1) + 1;
        if (count <= 3 || (count % 120) == 0) {
          XELOGI("DEBUG: VulkanPresenter: present SUCCESS (count={}, image={})",
                 count, swapchain_image_index);
        }
      }
      if (!logged_first_present.exchange(true)) {
        XELOGI("DEBUG: VulkanPresenter: first present SUCCESS (image index={})",
               swapchain_image_index);
      }
    ...
    case VK_SUBOPTIMAL_KHR:
      {
        ...
        uint64_t count = present_count.fetch_add(1) + 1;
        if (count <= 3 || (count % 120) == 0) {
          XELOGI("DEBUG: VulkanPresenter: present SUBOPTIMAL (count={}, image={})",
                 count, swapchain_image_index);
        }
      }
      if (!logged_first_present.exchange(true)) {
        XELOGI("DEBUG: VulkanPresenter: first present SUBOPTIMAL (image index={})",
               swapchain_image_index);
      }
...
bool VulkanPresenter::InitializeSurfaceIndependent() {
  XELOGI("DEBUG: VULKAN PRESENTER: InitializeSurfaceIndependent() START");
  ...
  XELOGI("Step 1: Creating guest output image descriptor set layout...");
  if (dfn.vkCreateDescriptorSetLayout(
    ...
    XELOGE(
        "VulkanPresenter: DEBUG: FAILED to create the guest output image descriptor "
        "set layout");
    return false;
  }
  XELOGI("DEBUG: Guest output image descriptor set layout created");
  ...
  guest_output_paint_descriptor_set_allocate_info.pSetLayouts =
      guest_output_paint_descriptor_set_layouts;
  XELOGI("Step 2: Allocating guest output painting descriptor sets...");
  if (dfn.vkAllocateDescriptorSets(
    ...
    XELOGE(
        "VulkanPresenter: DEBUG: FAILED to allocate the guest output painting "
        "descriptor sets");
    return false;
  }
  XELOGI("DEBUG: Guest output painting descriptor sets allocated");

  XELOGI("Step 3: Calling InitializeCommonSurfaceIndependent()...");
  bool common_init_result = InitializeCommonSurfaceIndependent();
  if (!common_init_result) {
    XELOGE("DEBUG: InitializeCommonSurfaceIndependent() FAILED");
    return false;
  }
  XELOGI("DEBUG: InitializeCommonSurfaceIndependent() succeeded");
  LogGuestOutputBootstrapState(
      "VulkanPresenter::InitializeSurfaceIndependent post-common");
  XELOGI("DEBUG: VULKAN PRESENTER: InitializeSurfaceIndependent() COMPLETE DEBUG: ");
  return true;
}
...
void VulkanPresenter::ValidateMetalSurfaceBinding() {
#ifdef __APPLE__
  XELOGI("DEBUG: ENHANCED METAL SURFACE VALIDATION");

  if (!paint_context_.vulkan_surface) {
    XELOGE("DEBUG: Vulkan surface is null");
    return;
  }

  // Check if we're on macOS with Metal support
  const VulkanDevice::Functions& dfn = vulkan_device_->functions();
  VkPhysicalDevice physical_device = vulkan_device_->physical_device();

  // Use instance functions for physical device properties
  const auto& ifn = vulkan_device_->vulkan_instance()->functions();

  // Get basic physical device properties
  VkPhysicalDeviceProperties device_properties = {};
  ifn.vkGetPhysicalDeviceProperties(physical_device, &device_properties);

  XELOGI("DEBUG: Physical device properties:");
  XELOGI("Device name: {}", device_properties.deviceName);
  XELOGI("API version: {}.{}.{}",
   ...
   VkResult result = ifn.vkGetPhysicalDeviceSurfaceCapabilitiesKHR(
       physical_device, paint_context_.vulkan_surface, &surface_caps);

   if (result != VK_SUCCESS) {
     XELOGE("DEBUG: Failed to get surface capabilities: {}", static_cast<int32_t>(result));
     return;
   }

   XELOGI("DEBUG: Surface capabilities:");
   XELOGI("Min extent: {}x{}", surface_caps.minImageExtent.width, surface_caps.minImageExtent.height);
   XELOGI("Max extent: {}x{}", surface_caps.maxImageExtent.width, surface_caps.maxImageExtent.height);
   XELOGI("Max image array layers: {}", surface_caps.maxImageArrayLayers);

   // Check for required Metal extensions
   if (!vulkan_device_->vulkan_instance()->extensions().ext_EXT_metal_surface) {
     XELOGI("DEBUG: VK_EXT_metal_surface extension not available");
   }

   // Validate present modes
   uint32_t present_mode_count = 0;
   result = ifn.vkGetPhysicalDeviceSurfacePresentModesKHR(
       physical_device, paint_context_.vulkan_surface, &present_mode_count, nullptr);

   if (result != VK_SUCCESS || present_mode_count == 0) {
     XELOGE("DEBUG: No present modes available");
     return;
   }

   std::vector<VkPresentModeKHR> present_modes(present_mode_count);
   result = ifn.vkGetPhysicalDeviceSurfacePresentModesKHR(
       physical_device, paint_context_.vulkan_surface, &present_mode_count, present_modes.data());

   if (result == VK_SUCCESS) {
     XELOGI("DEBUG: Available present modes:");
     for (uint32_t i = 0; i < present_mode_count; ++i) {
       const char* mode_name = "UNKNOWN";
       switch (present_modes[i]) {
         case VK_PRESENT_MODE_FIFO_KHR:
           mode_name = "FIFO (V-sync)";
           break;
         case VK_PRESENT_MODE_MAILBOX_KHR:
           mode_name = "MAILBOX (tear-free)";
           break;
         case VK_PRESENT_MODE_IMMEDIATE_KHR:
           mode_name = "IMMEDIATE (tearing)";
           break;
   ...
   result = ifn.vkGetPhysicalDeviceSurfaceFormatsKHR(
       physical_device, paint_context_.vulkan_surface, &format_count, nullptr);

   if (result == VK_SUCCESS && format_count > 0) {
     std::vector<VkSurfaceFormatKHR> formats(format_count);
     result = ifn.vkGetPhysicalDeviceSurfaceFormatsKHR(
         physical_device, paint_context_.vulkan_surface, &format_count, formats.data());

     XELOGI("DEBUG: Available surface formats:");
     for (uint32_t i = 0; i < format_count; ++i) {
       XELOGI(" - Format {}: {}", i, static_cast<uint32_t>(formats[i].format));
     }
   }

  // Comprehensive validation result
  XELOGI("DEBUG: Metal surface validation completed");
  XELOGI("DEBUG: Metal → MoltenVK pipeline is properly configured");
  XELOGI("DEBUG: Ready for Vulkan → Metal rendering");

#else
  XELOGI("DEBUG: Metal surface validation skipped (not on macOS)");
#endif
}
