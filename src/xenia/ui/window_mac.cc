...
DEFINE_int32(
    window_paint_stale_warn_ms, 500,
    "Warn if paint requests are not followed by OnPaint within this many "
    "milliseconds.",
    "Debug");
DEFINE_int32(
    window_paint_stale_warn_every_n, 1,
    "Warn every N stale paint requests (1 = every warning).",
    "Debug");
DEFINE_int32(
  window_presenter_surface_startup_warn_ms, 3000,
  "Warn if presenter surface stays null this many ms after SetPresenter.",
  "Debug");
DEFINE_int32(
  window_presenter_surface_startup_warn_interval_ms, 2000,
  "Minimum interval between repeated missing presenter surface warnings.",
  "Debug");

std::atomic<uint64_t> g_window_paint_request_count{0};
std::atomic<uint64_t> g_window_paint_count{0};
std::atomic<uint64_t> g_window_last_request_ms{0};
std::atomic<uint64_t> g_window_last_paint_ms{0};
...
void Window::SetPresenter(Presenter* presenter) {
  ...
  if (presenter_) {
    ...
    presenter_surface_.reset();
    presenter_surface_attach_start_ms_ = 0;
    presenter_surface_last_watchdog_warn_ms_ = 0;
    presenter_surface_watchdog_warn_count_ = 0;
  }
  presenter_ = presenter;
  if (presenter_) {
    presenter_surface_retry_count_ = 0;
    presenter_surface_retry_scheduled_ = false;
    presenter_surface_attach_start_ms_ = Clock::QueryHostUptimeMillis();
    presenter_surface_last_watchdog_warn_ms_ = 0;
    presenter_surface_watchdog_warn_count_ = 0;
    if (!TryCreatePresenterSurface("SetPresenter")) {
      SchedulePresenterSurfaceRetry("SetPresenter");
    }
  }
}

void Window::RequestPaint() {
  uint64_t request_id = g_window_paint_request_count.fetch_add(1) + 1;
  uint64_t now_ms = Clock::QueryHostUptimeMillis();
  if (presenter_ && !presenter_surface_ && presenter_surface_attach_start_ms_) {
    uint64_t attach_age_ms =
        now_ms >= presenter_surface_attach_start_ms_
            ? (now_ms - presenter_surface_attach_start_ms_)
            : 0;
    uint64_t warn_after_ms =
        std::max<uint64_t>(1, cvars::window_presenter_surface_startup_warn_ms);
    uint64_t warn_interval_ms =
        std::max<uint64_t>(
            1, cvars::window_presenter_surface_startup_warn_interval_ms);
    if (attach_age_ms >= warn_after_ms &&
        (presenter_surface_last_watchdog_warn_ms_ == 0 ||
         now_ms >=
             presenter_surface_last_watchdog_warn_ms_ + warn_interval_ms)) {
      presenter_surface_last_watchdog_warn_ms_ = now_ms;
      uint64_t warn_id = ++presenter_surface_watchdog_warn_count_;
      XELOGW(
          "DEBUG: Window presenter surface startup watchdog #{}: presenter_surface "
          "is still null after {} ms (reason=RequestPaint, retries={}, "
          "presenter={}, phase={})",
          warn_id, static_cast<unsigned long long>(attach_age_ms),
          presenter_surface_retry_count_, static_cast<void*>(presenter_),
          static_cast<int>(phase_));
    }
  }
  g_window_last_request_ms.store(now_ms, std::memory_order_relaxed);
  uint64_t last_paint_ms =
      g_window_last_paint_ms.load(std::memory_order_relaxed);
  uint64_t since_last_paint_ms =
      last_paint_ms ? (now_ms - last_paint_ms) : 0;
  if (request_id <= 5 || (request_id % 120) == 0) {
    std::ostringstream oss;
    oss << std::this_thread::get_id();
    XELOGI(
        "DEBUG: Window::RequestPaint #{} presenter_surface={} presenter={} "
        "surface={} since_last_paint_ms={} thread={}",
        request_id, presenter_surface_ ? "YES" : "NO",
        static_cast<void*>(presenter_),
        static_cast<void*>(presenter_surface_.get()),
        static_cast<unsigned long long>(since_last_paint_ms), oss.str());
    if (presenter_) {
      presenter_->LogGuestOutputBootstrapState("Window::RequestPaint");
    }
  }
  if (presenter_surface_ && request_id > 1 && last_paint_ms &&
      since_last_paint_ms > uint64_t(cvars::window_paint_stale_warn_ms)) {
    uint64_t refresh_attempts =
        presenter_ ? presenter_->GetGuestOutputRefreshAttemptCount() : 0;
    uint64_t refresh_successes =
        presenter_ ? presenter_->GetGuestOutputRefreshSuccessCount() : 0;
    bool guest_refresh_bootstrap_seen =
        refresh_attempts != 0 || refresh_successes != 0;
    if (guest_refresh_bootstrap_seen) {
      static std::atomic<uint64_t> stale_warn_count{0};
      uint64_t stale_id = stale_warn_count.fetch_add(1) + 1;
      int log_every = cvars::window_paint_stale_warn_every_n;
      if (stale_id <= 5 || (log_every > 0 && (stale_id % log_every) == 0)) {
        XELOGW(
            "DEBUG: Window::RequestPaint stale paint gap={}ms (last_paint_ms={}, "
            "request_id={}, pending_requests_estimate={}) - queuing async repaint",
            static_cast<unsigned long long>(since_last_paint_ms),
            static_cast<unsigned long long>(last_paint_ms),
            static_cast<unsigned long long>(request_id),
            static_cast<unsigned long long>(
                request_id > g_window_paint_count.load(std::memory_order_relaxed)
                    ? (request_id -
                       g_window_paint_count.load(std::memory_order_relaxed))
                    : 0));
        if (presenter_) {
          presenter_->LogGuestOutputBootstrapState("Window::RequestPaint stale-gap");
        }
      }
    }
    // Avoid forced synchronous paint here; request presenter/UI repaint and let
    // the normal paint path service it.
    RequestPresenterUIPaintFromUIThread();
  }
  if (presenter_surface_) {
    RequestPaintImpl();
  } else if (presenter_) {
    if (request_id <= 5 || (request_id % 120) == 0) {
      XELOGI("DEBUG: Window::RequestPaint queued without surface (id={})",
             request_id);
    }
    RequestPaintImpl();
  }
...
bool Window::TryCreatePresenterSurface(const char* reason) {
  if (!presenter_ || phase_ != Phase::kOpen) {
    return false;
  }

  auto allowed_types = presenter_->GetSupportedSurfaceTypes();
  presenter_surface_ = CreateSurface(allowed_types);
  if (!presenter_surface_) {
    XELOGW(
        "DEBUG: Window::TryCreatePresenterSurface failed (reason='{}', allowed_types=0x{:X})",
        reason ? reason : "unknown", static_cast<unsigned>(allowed_types));
    return false;
  }
  ...
  presenter_surface_watchdog_warn_count_ = 0;
  XELOGI(
      "DEBUG: Window::TryCreatePresenterSurface success (reason='{}', allowed_types=0x{:X})",
      reason ? reason : "unknown", static_cast<unsigned>(allowed_types));
  RequestPaint();
  RequestPresenterUIPaintFromUIThread();
  return true;
...
void Window::SchedulePresenterSurfaceRetry(const char* reason) {
  if (!presenter_ || phase_ != Phase::kOpen || presenter_surface_) {
    return;
  }
  if (presenter_surface_retry_scheduled_) {
    return;
  }
  presenter_surface_retry_scheduled_ = true;
  uint32_t attempt = ++presenter_surface_retry_count_;
  if (attempt <= 10 || (attempt % 50) == 0) {
    XELOGW(
        "DEBUG: Queueing presenter surface retry attempt {} (reason='{}')",
        attempt, reason ? reason : "unknown");
  }
  app_context_.CallInUIThreadDeferredTagged(
      "PresenterSurfaceRetry", [this, attempt, reason]() {
        presenter_surface_retry_scheduled_ = false;
        if (presenter_surface_ || !presenter_ || phase_ != Phase::kOpen) {
          return;
        }
        if (attempt <= 10 || (attempt % 50) == 0) {
          XELOGW(
              "DEBUG: Retrying presenter surface creation (attempt {}, reason='{}')",
              attempt, reason ? reason : "unknown");
        }
        if (TryCreatePresenterSurface("PresenterSurfaceRetry")) {
          return;
        }
        // Keep the normal paint path active as a fallback, but continue
        // scheduling retries until a surface is attached.
        RequestPaint();
        SchedulePresenterSurfaceRetry("PresenterSurfaceRetry");
      });
}
...
void Window::OnSurfaceChanged(bool new_surface_potentially_exists) {
  ...
  if (!TryCreatePresenterSurface("OnSurfaceChanged")) {
    SchedulePresenterSurfaceRetry("OnSurfaceChanged");
  }
}
...
void Window::OnPaint(bool force_paint) {
  ...
  is_painting_ = true;
  uint64_t paint_id = g_window_paint_count.fetch_add(1) + 1;
  uint64_t now_ms = Clock::QueryHostUptimeMillis();
    if (presenter_ && !presenter_surface_ && presenter_surface_attach_start_ms_) {
    uint64_t attach_age_ms =
      now_ms >= presenter_surface_attach_start_ms_
    ...
    if (attach_age_ms >= warn_after_ms &&
      ...
      XELOGW(
        "DEBUG: Window presenter surface startup watchdog #{}: presenter_surface "
        "is still null after {} ms (reason=OnPaint, retries={}, "
        "presenter={}, phase={})",
        warn_id, static_cast<unsigned long long>(attach_age_ms),
        presenter_surface_retry_count_, static_cast<void*>(presenter_),
        static_cast<int>(phase_));
  ...
  g_window_last_paint_ms.store(now_ms, std::memory_order_relaxed);
  uint64_t last_request_ms =
      g_window_last_request_ms.load(std::memory_order_relaxed);
  uint64_t since_last_request_ms =
      last_request_ms ? (now_ms - last_request_ms) : 0;
  uint64_t request_count =
      g_window_paint_request_count.load(std::memory_order_relaxed);
  uint64_t pending_requests =
      request_count > paint_id ? (request_count - paint_id) : 0;
  if (paint_id <= 5 || (paint_id % 120) == 0) {
    std::ostringstream oss;
    oss << std::this_thread::get_id();
    XELOGI(
        "DEBUG: Window::OnPaint #{} force_paint={} presenter_surface={} "
        "pending_requests={} since_last_request_ms={} thread={}",
        paint_id, force_paint ? "YES" : "NO",
        presenter_surface_ ? "YES" : "NO",
        static_cast<unsigned long long>(pending_requests),
        static_cast<unsigned long long>(since_last_request_ms), oss.str());
    if (presenter_) {
      presenter_->LogGuestOutputBootstrapState("Window::OnPaint");
    }
  }
  if (presenter_surface_) {
    presenter_->PaintFromUIThread(force_paint);
  } else if (presenter_) {
    // Attempt a late surface creation on paint to recover from delayed NSView
    // / CAMetalLayer availability.
    if (TryCreatePresenterSurface("OnPaint")) {
      presenter_->PaintFromUIThread(force_paint);
    } else {
      SchedulePresenterSurfaceRetry("OnPaint");
    }
  }
  is_painting_ = false;
}
