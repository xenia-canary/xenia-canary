...
DEFINE_int32(
    ...
    "Display");
DEFINE_bool(
    presenter_log_guest_output_refresh, true,
    "Log guest output refresh activity each time it is updated.",
    "Debug");
DEFINE_int32(
    presenter_log_guest_output_every_n, 1,
    "Log every N guest output refreshes (1 = every refresh).",
    "Debug");
DEFINE_bool(
    presenter_log_guest_output_mailbox, true,
    "Log guest output mailbox state transitions.",
    "Debug");
DEFINE_bool(
    presenter_log_state_transitions, true,
    "Log presenter paint mode and surface connection state transitions.",
    "Debug");
DEFINE_int32(
    presenter_log_state_every_n, 1,
    "Log every N presenter state changes (1 = every change).",
    "Debug");
DEFINE_bool(
    presenter_log_surface_updates, true,
    "Log presenter window/surface attachment updates.",
    "Debug");
DEFINE_int32(
    presenter_log_surface_every_n, 1,
    "Log every N presenter surface updates (1 = every update).",
    "Debug");
DEFINE_bool(
    presenter_log_paint_calls, true,
    "Log presenter paint requests and PaintAndPresent calls.",
    "Debug");
DEFINE_int32(
    presenter_log_paint_every_n, 1,
    "Log every N presenter paint calls (1 = every call).",
    "Debug");
DEFINE_bool(
  presenter_log_bootstrap_state, true,
  "Log presenter bootstrap state snapshots around first paints and surface "
  "initialization.",
  "Debug");
DEFINE_int32(
  presenter_log_bootstrap_every_n, 1,
  "Log every N presenter bootstrap snapshots (1 = every snapshot).",
  "Debug");
DEFINE_uint32(
  presenter_bootstrap_refresh_watchdog_ms, 250,
  "If the presenter is already connected/paintable but guest refresh "
  "attempts are still zero after this many ms since presenter init, log an "
  "error-level bootstrap starvation warning.",
  "Debug");
DEFINE_int32(
  presenter_bootstrap_refresh_watchdog_every_n, 60,
  "Log every N bootstrap starvation watchdog hits (1 = every hit).",
  "Debug");
DEFINE_bool(
  presenter_bootstrap_mailbox_nudge, false,
  "If bootstrap watchdog detects connected paintable surface with zero "
  "guest refresh attempts, issue a one-time synthetic guest refresh to "
  "activate mailbox flow for diagnostics.",
  "Debug");

namespace xe {
namespace ui {

const char* Presenter::PaintResultToString(PaintResult result) {
  switch (result) {
...
void Presenter::LogGuestOutputBootstrapState(const char* reason) {
  static std::atomic<uint64_t> snapshot_count{0};
  if (!cvars::presenter_log_bootstrap_state) {
    return;
  }
  uint64_t snapshot_id = snapshot_count.fetch_add(1) + 1;
  int log_every = cvars::presenter_log_bootstrap_every_n;
  if (!(snapshot_id <= 8 || (log_every > 0 && (snapshot_id % log_every) == 0))) {
    return;
  }

  PaintMode paint_mode_snapshot;
  SurfacePaintConnectionState connection_snapshot;
  {
    std::lock_guard<std::mutex> paint_mode_lock(paint_mode_mutex_);
    paint_mode_snapshot = paint_mode_;
    connection_snapshot = surface_paint_connection_state_;
  }

  uint32_t acquired_and_ready =
      guest_output_mailbox_acquired_and_ready_.load(std::memory_order_relaxed);
  uint32_t acquired_mailbox = acquired_and_ready & 3;
  uint32_t ready_mailbox = (acquired_and_ready >> 2) & 3;
  uint64_t refresh_attempt_count =
      guest_output_refresh_attempt_count_.load(std::memory_order_relaxed);
  uint64_t refresh_success_count =
      guest_output_refresh_success_count_.load(std::memory_order_relaxed);
    uint64_t bootstrap_start_ms =
      presenter_bootstrap_start_ms_.load(std::memory_order_relaxed);
    uint64_t first_refresh_attempt_ms =
      first_guest_refresh_attempt_ms_.load(std::memory_order_relaxed);
    uint64_t first_refresh_success_ms =
      first_guest_refresh_success_ms_.load(std::memory_order_relaxed);
    uint64_t now_ms = Clock::QueryHostUptimeMillis();
    uint64_t since_bootstrap_ms =
      (bootstrap_start_ms && now_ms >= bootstrap_start_ms)
        ? (now_ms - bootstrap_start_ms)
        : 0;
    uint64_t to_first_attempt_ms =
      (bootstrap_start_ms && first_refresh_attempt_ms &&
       first_refresh_attempt_ms >= bootstrap_start_ms)
        ? (first_refresh_attempt_ms - bootstrap_start_ms)
        : 0;
    uint64_t to_first_success_ms =
      (bootstrap_start_ms && first_refresh_success_ms &&
       first_refresh_success_ms >= bootstrap_start_ms)
        ? (first_refresh_success_ms - bootstrap_start_ms)
        : 0;
  bool ui_paint_requested =
      ui_thread_paint_requested_.load(std::memory_order_relaxed);

  uint32_t writable_mailbox = 0;
  bool active_last_refresh = false;
  {
    std::lock_guard<std::mutex> mailbox_lock(guest_output_mailbox_consumer_mutex_);
    writable_mailbox = guest_output_mailbox_writable_;
    active_last_refresh = guest_output_active_last_refresh_;
  }

  XELOGI(
      "DEBUG: Presenter bootstrap snapshot #{} reason={} paint_mode={} "
      "connection_state={} mailbox(acquired={},ready={},writable={}) "
      "refresh_attempts={} refresh_successes={} active_last_refresh={} "
      "ui_paint_requested={} since_bootstrap_ms={} "
      "to_first_attempt_ms={} to_first_success_ms={}",
      snapshot_id, reason ? reason : "<unspecified>",
      PaintModeToString(paint_mode_snapshot),
      SurfaceConnectionStateToString(connection_snapshot), acquired_mailbox,
      ready_mailbox, writable_mailbox, refresh_attempt_count,
      refresh_success_count, active_last_refresh ? "YES" : "NO",
      ui_paint_requested ? "YES" : "NO",
      static_cast<unsigned long long>(since_bootstrap_ms),
      static_cast<unsigned long long>(to_first_attempt_ms),
      static_cast<unsigned long long>(to_first_success_ms));

  if (refresh_attempt_count == 0) {
    XELOGW(
        "DEBUG: Presenter bootstrap snapshot: zero guest refresh attempts so far; "
        "producer path (IssueSwap/VdSwap -> RefreshGuestOutput) has not run yet.");
  } else if (refresh_success_count == 0) {
    XELOGW(
        "DEBUG: Presenter bootstrap snapshot: guest refresh attempts exist but no "
        "successful refreshes yet.");
  }
}
...
void Presenter::SetWindowSurfaceFromUIThread(Window* new_window,
  ...
  static std::atomic<uint64_t> surface_update_count{0};
  uint64_t surface_update_id = surface_update_count.fetch_add(1) + 1;
  int surface_log_every = cvars::presenter_log_surface_every_n;
  bool log_surface_update =
      cvars::presenter_log_surface_updates &&
      (surface_update_id <= 5 ||
       (surface_log_every > 0 && (surface_update_id % surface_log_every) == 0));
  if (log_surface_update) {
    XELOGI(
        "Presenter::SetWindowSurfaceFromUIThread #{} window={} surface={} -> "
        "new_window={} new_surface={}",
        surface_update_id, static_cast<void*>(window_),
        static_cast<void*>(surface_), static_cast<void*>(new_window),
  ...
  if (surface_) {
    ...
    // becomes available.
    if (log_surface_update) {
      XELOGI("Presenter::SetWindowSurfaceFromUIThread disconnecting surface");
    }
  ...
  if (window_ != new_window) {
    ...
    if (window_) {
      ...
      window_ = nullptr;
      if (log_surface_update) {
        XELOGI("Presenter::SetWindowSurfaceFromUIThread detaching old window={}",
               static_cast<void*>(old_window));
      }
      old_window->SetPresenter(nullptr);
    }
    ...
    window_ = new_window;
    if (log_surface_update) {
      XELOGI("Presenter::SetWindowSurfaceFromUIThread attached window={}",
  ...
  if (new_surface) {
    ...
    if (log_surface_update) {
      XELOGI("Presenter::SetWindowSurfaceFromUIThread attached surface={}",
...
void Presenter::PaintFromUIThread(bool force_paint) {
  ...
  if (!InSurfaceOnMonitorFromUIThread()) {
    static std::atomic<uint64_t> paint_skip_count{0};
    uint64_t skip_id = paint_skip_count.fetch_add(1) + 1;
    if (skip_id <= 5 || (skip_id % 120) == 0) {
      XELOGI(
          "DEBUG: Presenter::PaintFromUIThread skipped (no surface/monitor) "
          "force_paint={} paint_mode={} connection_state={}",
          force_paint ? "YES" : "NO", PaintModeToString(paint_mode_),
          SurfaceConnectionStateToString(surface_paint_connection_state_));
    }
    return;
  }
  ...
  // for example.
  bool ui_request =
      ui_thread_paint_requested_.exchange(false, std::memory_order_relaxed);
  if (ui_request) {
    do_paint = true;
  }
  static std::atomic<uint64_t> ui_paint_count{0};
  uint64_t ui_paint_id = ui_paint_count.fetch_add(1) + 1;
    uint64_t bootstrap_start_ms =
      presenter_bootstrap_start_ms_.load(std::memory_order_relaxed);
    uint64_t refresh_attempt_count_snapshot =
      guest_output_refresh_attempt_count_.load(std::memory_order_relaxed);
    if (do_paint &&
      surface_paint_connection_state_ ==
        SurfacePaintConnectionState::kConnectedPaintable &&
      refresh_attempt_count_snapshot == 0 && bootstrap_start_ms != 0) {
    uint64_t now_ms = Clock::QueryHostUptimeMillis();
    uint64_t since_bootstrap_ms =
      now_ms >= bootstrap_start_ms ? (now_ms - bootstrap_start_ms) : 0;
    uint64_t watchdog_ms =
      std::max<uint64_t>(1, cvars::presenter_bootstrap_refresh_watchdog_ms);
    if (since_bootstrap_ms >= watchdog_ms) {
      if (cvars::presenter_bootstrap_mailbox_nudge &&
        !bootstrap_mailbox_nudge_done_.load(std::memory_order_relaxed)) {
      uint32_t nudge_width = surface_width_in_paint_connection_;
      uint32_t nudge_height = surface_height_in_paint_connection_;
      if (nudge_width != 0 && nudge_height != 0 &&
        !bootstrap_mailbox_nudge_done_.exchange(
          true, std::memory_order_relaxed)) {
        static std::atomic<uint64_t> mailbox_nudge_count{0};
        uint64_t nudge_id = mailbox_nudge_count.fetch_add(1) + 1;
        XELOGW(
          "DEBUG: Presenter mailbox nudge #{}: issuing one-time synthetic "
          "RefreshGuestOutput ({}x{}) after {}ms bootstrap age",
          nudge_id, nudge_width, nudge_height,
          static_cast<unsigned long long>(since_bootstrap_ms));
        bool nudge_ok = RefreshGuestOutput(
          nudge_width, nudge_height, nudge_width, nudge_height,
          [](GuestOutputRefreshContext&) { return true; });
        XELOGW(
          "DEBUG: Presenter mailbox nudge #{} result={} "
          "(synthetic refresh callback)",
          nudge_id, nudge_ok ? "SUCCESS" : "FAIL");
        LogGuestOutputBootstrapState(
          "Presenter::PaintFromUIThread mailbox-nudge");
      }
      }

      static std::atomic<uint64_t> watchdog_hit_count{0};
      uint64_t watchdog_id = watchdog_hit_count.fetch_add(1) + 1;
      int log_every = cvars::presenter_bootstrap_refresh_watchdog_every_n;
      if (watchdog_id <= 8 ||
        (log_every > 0 && (watchdog_id % log_every) == 0)) {
      XELOGE(
        "DEBUG: Presenter bootstrap watchdog #{}: connected_paintable with "
        "zero guest refresh attempts after {} ms (threshold={} ms). "
        "Producer path has not called RefreshGuestOutput yet.",
        watchdog_id, static_cast<unsigned long long>(since_bootstrap_ms),
        static_cast<unsigned long long>(watchdog_ms));
      LogGuestOutputBootstrapState(
        "Presenter::PaintFromUIThread watchdog");
      }
    }
    }
  if (ui_paint_id <= 5 || (ui_paint_id % 120) == 0) {
    XELOGI(
        "DEBUG: Presenter::PaintFromUIThread #{} force_paint={} draw_ui={} "
        "ui_request={} do_paint={} paint_mode={} connection_state={}",
        ui_paint_id, force_paint ? "YES" : "NO", draw_ui ? "YES" : "NO",
        ui_request ? "YES" : "NO", do_paint ? "YES" : "NO",
        PaintModeToString(paint_mode_),
        SurfaceConnectionStateToString(surface_paint_connection_state_));
  }
  ...
  if (ui_paint_id <= 5 || (ui_paint_id % 120) == 0) {
    XELOGI(
        "DEBUG: Presenter::PaintFromUIThread result={} request_repaint_immediately="
        "{} request_repaint_at_tick={}",
        PaintResultToString(paint_result),
        request_repaint_immediately ? "YES" : "NO",
        request_repaint_at_tick ? "YES" : "NO");
  }
}
...
bool Presenter::RefreshGuestOutput(
  ...
  static std::atomic<bool> logged_first_active{false};
  static std::atomic<bool> logged_first_inactive{false};
  static std::atomic<uint64_t> refresh_count{0};
  static std::atomic<uint64_t> inactive_refresh_count{0};
    uint64_t now_ms = Clock::QueryHostUptimeMillis();
    uint64_t previous_attempt_count =
      guest_output_refresh_attempt_count_.fetch_add(1, std::memory_order_relaxed);
    if (previous_attempt_count == 0) {
    first_guest_refresh_attempt_ms_.store(now_ms, std::memory_order_relaxed);
    uint64_t bootstrap_start_ms =
      presenter_bootstrap_start_ms_.load(std::memory_order_relaxed);
    uint64_t delta_ms =
      (bootstrap_start_ms && now_ms >= bootstrap_start_ms)
        ...
        : 0;
    XELOGI(
      "DEBUG: Presenter bootstrap: first RefreshGuestOutput attempt observed "
      "after {} ms",
      static_cast<unsigned long long>(delta_ms));
    }
  uint64_t refresh_id = refresh_count.fetch_add(1) + 1;
  int log_every = cvars::presenter_log_guest_output_every_n;
  bool log_refresh =
      cvars::presenter_log_guest_output_refresh &&
      (refresh_id <= 5 || (log_every > 0 && (refresh_id % log_every) == 0));
  PaintMode paint_mode_snapshot;
  SurfacePaintConnectionState connection_snapshot;
  {
    std::lock_guard<std::mutex> paint_mode_lock(paint_mode_mutex_);
  ...
  if (log_refresh) {
    ...
    XELOGI(
        "DEBUG: GuestOutputRefresh #{} active_last_refresh={} mailbox: "
        "writable={} acquired={} ready={} paint_mode={} connection_state={} "
      "frontbuffer={}x{} aspect={}x{} thread={}",
        refresh_id, guest_output_active_last_refresh_ ? "YES" : "NO",
        guest_output_mailbox_writable_, acquired_mailbox, ready_mailbox,
        PaintModeToString(paint_mode_snapshot),
        SurfaceConnectionStateToString(connection_snapshot),
        frontbuffer_width, frontbuffer_height, display_aspect_ratio_x,
      display_aspect_ratio_y, tid.str());
  }
  ...
  if (is_active && !logged_first_active.exchange(true)) {
    XELOGI("DEBUG: Guest output active: mailbox={} size={}x{} aspect={}x{}",
           guest_output_mailbox_writable_, frontbuffer_width,
           frontbuffer_height, display_aspect_ratio_x,
           display_aspect_ratio_y);
  } else if (!is_active && !logged_first_inactive.exchange(true)) {
    XELOGI("DEBUG: Guest output inactive (no valid frontbuffer yet)");
  }
  if (is_active) {
    bool refresh_ok = RefreshGuestOutputImpl(guest_output_mailbox_writable_,
                                frontbuffer_width, frontbuffer_height,
                                refresher, writable_properties.is_8bpc);
    if (log_refresh) {
      XELOGI("DEBUG: GuestOutputRefresh #{} impl_result={} is_8bpc={}",
             refresh_id, refresh_ok ? "SUCCESS" : "FAIL",
             writable_properties.is_8bpc ? "YES" : "NO");
    }
    if (!refresh_ok) {
      ...
      static std::atomic<uint32_t> refresh_fail_count{0};
      uint32_t fail_count = refresh_fail_count.fetch_add(1) + 1;
      if (fail_count <= 5 || (fail_count % 120) == 0) {
        XELOGW("DEBUG: Guest output refresh failed (count={}, mailbox={}, size={}x{})",
               fail_count, guest_output_mailbox_writable_, frontbuffer_width,
               frontbuffer_height);
      }
      return false;
    }
    static std::atomic<bool> logged_first_refresh{false};
    if (!logged_first_refresh.exchange(true)) {
      XELOGI("DEBUG: Guest output refresh succeeded: mailbox={} size={}x{}",
             guest_output_mailbox_writable_, frontbuffer_width,
             frontbuffer_height);
    }
    guest_output_refresh_success_count_.fetch_add(1,
                                                  std::memory_order_relaxed);
    if (first_guest_refresh_success_ms_.load(std::memory_order_relaxed) == 0) {
      first_guest_refresh_success_ms_.store(now_ms, std::memory_order_relaxed);
      uint64_t bootstrap_start_ms =
        presenter_bootstrap_start_ms_.load(std::memory_order_relaxed);
      uint64_t delta_ms =
        (bootstrap_start_ms && now_ms >= bootstrap_start_ms)
          ...
          : 0;
      XELOGI(
        "DEBUG: Presenter bootstrap: first successful guest refresh observed "
        "after {} ms",
        static_cast<unsigned long long>(delta_ms));
    }
    ...
    uint64_t inactive_id =
        inactive_refresh_count.fetch_add(1, std::memory_order_relaxed) + 1;
    const char* inactive_reason = "unknown";
    if (frontbuffer_width == 0 || frontbuffer_height == 0) {
      inactive_reason = "zero_frontbuffer_dimensions";
    } else if (display_aspect_ratio_x == 0 || display_aspect_ratio_y == 0) {
      inactive_reason = "zero_display_aspect_ratio";
    } else {
      inactive_reason = "inactive_guest_output_properties";
    }
    if (inactive_id <= 5 || (inactive_id % 120) == 0) {
      XELOGW(
          "DEBUG: GuestOutputRefresh #{} inactive input: frontbuffer={}x{} "
          "aspect={}x{} active_last_refresh={} mailbox_writable={} "
          "reason={} refresh_attempt_count={} refresh_success_count={}",
          refresh_id, frontbuffer_width, frontbuffer_height,
          display_aspect_ratio_x, display_aspect_ratio_y,
          guest_output_active_last_refresh_ ? "YES" : "NO",
          guest_output_mailbox_writable_, inactive_reason,
          guest_output_refresh_attempt_count_.load(std::memory_order_relaxed),
          guest_output_refresh_success_count_.load(std::memory_order_relaxed));
    }
    ...
    if (!guest_output_active_last_refresh_) {
      if (log_refresh) {
        XELOGI(
            "DEBUG: GuestOutputRefresh #{} inactive (no prior active refresh) - "
            "skipping mailbox update",
            refresh_id);
      }
      if (inactive_id <= 5 || (inactive_id % 120) == 0) {
        XELOGW(
            "DEBUG: GuestOutputRefresh #{} skipped: no active guest frame has "
            "ever been published yet, mailbox remains without ready image "
            "(reason={}).",
            refresh_id, inactive_reason);
      }
      return false;
    }
  ...
  if (log_refresh && cvars::presenter_log_guest_output_mailbox) {
    uint32_t new_acquired_and_ready =
        guest_output_mailbox_acquired_and_ready_.load(
            std::memory_order_relaxed);
    uint32_t new_acquired = new_acquired_and_ready & 3;
    uint32_t new_ready = (new_acquired_and_ready >> 2) & 3;
    XELOGI(
        "DEBUG: GuestOutputMailbox #{} updated: acquired={} ready={} writable={}",
        refresh_id, new_acquired, new_ready, guest_output_mailbox_writable_);
  }
  ...
  if (log_refresh && cvars::presenter_log_guest_output_mailbox) {
    XELOGI("DEBUG: GuestOutputMailbox #{} next writable={}", refresh_id,
           guest_output_mailbox_writable_);
  }

  // Trigger the presentation on the host.
  PaintResult paint_result = PaintResult::kNotPresented;
  bool requested_paint = false;
  {
  ...
  if (log_refresh) {
    XELOGI(
        "DEBUG: GuestOutputRefresh #{} paint_result={} requested_paint={} "
        "paint_mode={} connection_state={}",
        refresh_id, PaintResultToString(paint_result),
        requested_paint ? "YES" : "NO",
        PaintModeToString(paint_mode_snapshot),
        SurfaceConnectionStateToString(connection_snapshot));
  }
...
bool Presenter::InitializeCommonSurfaceIndependent() {
  uint64_t now_ms = Clock::QueryHostUptimeMillis();
  presenter_bootstrap_start_ms_.store(now_ms, std::memory_order_relaxed);
  first_guest_refresh_attempt_ms_.store(0, std::memory_order_relaxed);
  first_guest_refresh_success_ms_.store(0, std::memory_order_relaxed);
  bootstrap_mailbox_nudge_done_.store(false, std::memory_order_relaxed);
...
std::unique_lock<std::mutex> Presenter::ConsumeGuestOutput(
  ...
  static std::atomic<bool> logged_first_consume{false};
  static std::atomic<bool> logged_pre_refresh_inactive{false};
  static std::atomic<uint64_t> inactive_consume_count{0};
  if (paint_config_out) {
  ...
  if (properties.IsActive() && !logged_first_consume.exchange(true)) {
    XELOGI("DEBUG: Guest output consumed: mailbox={} size={}x{} aspect={}x{}",
           mailbox_index, properties.frontbuffer_width,
    ...
    uint64_t refresh_success_count =
        guest_output_refresh_success_count_.load(std::memory_order_relaxed);
    bool pre_refresh_bootstrap_inactive =
        refresh_attempt_count == 0 && refresh_success_count == 0;
    if (pre_refresh_bootstrap_inactive) {
      if (!logged_pre_refresh_inactive.exchange(true,
                                                std::memory_order_relaxed)) {
        XELOGI(
            "DEBUG: ConsumeGuestOutput bootstrap-inactive: mailbox has no ready "
            "guest frame yet (expected before first RefreshGuestOutput)."
            " mailbox={} refresh_attempt_count={} refresh_success_count={}",
            mailbox_index, refresh_attempt_count, refresh_success_count);
      }
    } else {
      uint64_t inactive_consume_id =
          inactive_consume_count.fetch_add(1, std::memory_order_relaxed) + 1;
      if (inactive_consume_id <= 5 || (inactive_consume_id % 120) == 0) {
        uint32_t acquired_and_ready = guest_output_mailbox_acquired_and_ready_
                                          .load(std::memory_order_relaxed);
        uint32_t acquired_mailbox = acquired_and_ready & 3;
        uint32_t ready_mailbox = (acquired_and_ready >> 2) & 3;
        XELOGW(
            "DEBUG: ConsumeGuestOutput inactive #{} mailbox={} acquired={} ready={} "
            "frontbuffer={}x{} aspect={}x{} refresh_attempt_count={} "
            "refresh_success_count={}",
            inactive_consume_id, mailbox_index, acquired_mailbox, ready_mailbox,
            properties.frontbuffer_width, properties.frontbuffer_height,
            properties.display_aspect_ratio_x,
            properties.display_aspect_ratio_y, refresh_attempt_count,
            refresh_success_count);
      }
    }
  }
...
void Presenter::ExecuteUIDrawersFromUIThread(UIDrawContext& ui_draw_context) {
  ...
  assert_false(is_executing_ui_drawers_);
  static std::atomic<uint64_t> ui_draw_call_count{0};
  uint64_t call_id = ui_draw_call_count.fetch_add(1) + 1;
  if (call_id <= 5 || (call_id % 120) == 0) {
    XELOGI("DEBUG: ExecuteUIDrawersFromUIThread #{} drawers={}",
           call_id, ui_drawers_.size());
  }
...
void Presenter::SetPaintModeFromUIThread(PaintMode new_mode) {
  ...
  PaintMode old_mode = paint_mode_;
  {
    std::lock_guard<std::mutex> lock(paint_mode_mutex_);
    paint_mode_ = new_mode;
  }
  static std::atomic<uint64_t> paint_mode_change_count{0};
  uint64_t change_id = paint_mode_change_count.fetch_add(1) + 1;
  int log_every = cvars::presenter_log_state_every_n;
  if (cvars::presenter_log_state_transitions &&
      (change_id <= 5 || (log_every > 0 && (change_id % log_every) == 0))) {
    XELOGI("Presenter::SetPaintModeFromUIThread #{} {} -> {}",
           change_id, PaintModeToString(old_mode),
           PaintModeToString(new_mode));
  }
...
void Presenter::UpdateSurfacePaintConnectionFromUIThread(
  ...
  SurfacePaintConnectionState old_state = surface_paint_connection_state_;
  uint32_t surface_width = 0;
  uint32_t surface_height = 0;
  bool surface_area_available = false;
  ...
  if (surface_paint_connection_state_ !=
    ...
    surface_area_available = surface_->GetSize(surface_width, surface_height);
    if (!surface_area_available) {
      ...
      connect_result = ConnectOrReconnectPaintingToSurfaceFromUIThread(
  ...
  static std::atomic<uint64_t> connection_update_count{0};
  uint64_t connection_update_id = connection_update_count.fetch_add(1) + 1;
  int log_every = cvars::presenter_log_state_every_n;
  if (cvars::presenter_log_state_transitions &&
      (connection_update_id <= 5 ||
       (log_every > 0 && (connection_update_id % log_every) == 0))) {
    const char* connect_result_str = "not_attempted";
    if (attempted_connect) {
      switch (connect_result) {
        case SurfacePaintConnectResult::kSuccess:
          connect_result_str = "success";
          break;
        case SurfacePaintConnectResult::kSuccessUnchanged:
          connect_result_str = "success_unchanged";
          break;
        case SurfacePaintConnectResult::kFailure:
          connect_result_str = "failure";
          break;
        case SurfacePaintConnectResult::kFailureSurfaceUnusable:
          connect_result_str = "failure_surface_unusable";
          break;
      }
    }
    XELOGI(
        "Presenter::UpdateSurfacePaintConnection #{} state {} -> {} "
        "surface_size={}x{} area_available={} attempted_connect={} "
        "connect_result={} vsync_implicit={} repaint_needed={}",
        connection_update_id, SurfaceConnectionStateToString(old_state),
        SurfaceConnectionStateToString(surface_paint_connection_state_),
        surface_width, surface_height, surface_area_available ? "YES" : "NO",
        attempted_connect ? "YES" : "NO", connect_result_str,
        is_vsync_implicit ? "YES" : "NO",
        (repaint_needed_out && *repaint_needed_out) ? "YES" : "NO");
  }
}

bool Presenter::RequestPaintOrConnectionRecoveryViaWindow(
    bool force_ui_thread_paint_tick) {
  static std::atomic<uint64_t> paint_request_count{0};
  ...
  assert_not_null(surface_);
  uint64_t request_id = paint_request_count.fetch_add(1) + 1;
  if (ui_thread_paint_requested_.exchange(true, std::memory_order_relaxed)) {
    // Invalidation pending already, no need to do it twice.
    int log_every = cvars::presenter_log_paint_every_n;
    if (cvars::presenter_log_paint_calls &&
        (request_id <= 5 || (log_every > 0 && (request_id % log_every) == 0))) {
      XELOGI(
          "Presenter::RequestPaintOrConnectionRecoveryViaWindow #{} skipped "
          "(already pending) force_tick={} paint_mode={} connection_state={}",
          request_id, force_ui_thread_paint_tick ? "YES" : "NO",
          PaintModeToString(paint_mode_),
          SurfaceConnectionStateToString(surface_paint_connection_state_));
    }
    return false;
  ...
  if (cvars::presenter_log_paint_calls &&
      (request_id <= 5 || (log_every > 0 && (request_id % log_every) == 0))) {
    XELOGI(
        "Presenter::RequestPaintOrConnectionRecoveryViaWindow #{} "
        "force_tick={} paint_mode={} connection_state={} window={} surface={}",
        request_id, force_ui_thread_paint_tick ? "YES" : "NO",
        PaintModeToString(paint_mode_),
        SurfaceConnectionStateToString(surface_paint_connection_state_),
        static_cast<void*>(window_), static_cast<void*>(surface_));
  }
...
Presenter::PaintResult Presenter::PaintAndPresent(bool execute_ui_drawers) {
  ...
  assert_true(surface_paint_connection_state_ ==
              SurfacePaintConnectionState::kConnectedPaintable);
  static std::atomic<uint64_t> paint_call_count{0};
  uint64_t paint_call_id = paint_call_count.fetch_add(1) + 1;
  int log_every = cvars::presenter_log_paint_every_n;
  if (cvars::presenter_log_paint_calls &&
      (paint_call_id <= 5 || (log_every > 0 && (paint_call_id % log_every) == 0))) {
    XELOGI(
        "Presenter::PaintAndPresent #{} execute_ui_drawers={} paint_mode={} "
        "connection_state={}",
        paint_call_id, execute_ui_drawers ? "YES" : "NO",
        PaintModeToString(paint_mode_),
        SurfaceConnectionStateToString(surface_paint_connection_state_));
  }
  ...
  if (cvars::presenter_log_paint_calls &&
      (paint_call_id <= 5 || (log_every > 0 && (paint_call_id % log_every) == 0))) {
    XELOGI("Presenter::PaintAndPresent #{} result={}", paint_call_id,
           PaintResultToString(result));
  }
  return result;
}
