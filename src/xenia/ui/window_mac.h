...
class Window {
 ...
 private:
  ...
  // Surface creation on macOS may need retries if the NSView/CAMetalLayer
  // isn't fully ready yet. Keep this lightweight and bounded.
  bool TryCreatePresenterSurface(const char* reason);
  void SchedulePresenterSurfaceRetry(const char* reason);
  uint32_t presenter_surface_retry_count_ = 0;
  bool presenter_surface_retry_scheduled_ = false;
  uint64_t presenter_surface_attach_start_ms_ = 0;
  uint64_t presenter_surface_last_watchdog_warn_ms_ = 0;
  uint64_t presenter_surface_watchdog_warn_count_ = 0;
