...
// Parse a hotkey string like "Ctrl+O", "Alt+F4", "F11" into NSMenuItem
// key equivalent and modifier mask. Ctrl maps to Cmd on macOS.
static void ParseHotkey(const std::string& hotkey, NSString** out_key,
                        NSEventModifierFlags* out_modifiers) {
  *out_key = @"";
  *out_modifiers = 0;
  ...
  // Map the key name to an NSMenuItem key equivalent.
  // Function keys use Unicode private use area characters.
  NSString* key = nil;
  if (remaining.size() == 1) {
    ...
    if (modifiers == 0) modifiers = NSEventModifierFlagCommand;
  } else if (remaining[0] == 'F' && remaining.size() >= 2 && remaining.size() <= 3) {
    int fnum = std::atoi(remaining.c_str() + 1);
    if (fnum >= 1 && fnum <= 12) {
      unichar fkey = NSF1FunctionKey + (fnum - 1);
      key = [NSString stringWithCharacters:&fkey length:1];
      // Function keys need no modifier by default.
    }
  } else if (remaining == "Pause/Break") {
    key = @"";  // No macOS equivalent
  } else if (remaining == "Numpad *") {
    key = @"*";
  } else if (remaining == "Numpad -") {
    key = @"-";
  } else if (remaining == "Numpad +") {
...
bool MacWindow::OpenImpl() {
  ...
  // setNeedsDisplay triggers drawRect: which calls RequestPaint().
  MacWindow* self = this;
  refresh_timer_ = [NSTimer scheduledTimerWithTimeInterval:1.0 / 60.0
                                                   repeats:YES
                                                     block:^(NSTimer* timer) {
                                                       if (self->content_view_) {
                                                         [self->content_view_ setNeedsDisplay:YES];
                                                       }
                                                     }];
...
NSMenuItem* MacWindow::CreateNSMenuItemFromMenuItem(MenuItem* menu_item) {
  ...
  switch (menu_item->type()) {
    ...
    case MenuItem::Type::kString: {
      ...
      ParseHotkey(menu_item->hotkey(), &key_equivalent, &modifier_mask);
...
void MacMenuItem::SetEnabled(bool enabled) {
  ...
  if (!main_menu) return;
  for (NSInteger i = 0; i < [main_menu numberOfItems]; ++i) {
    [[main_menu itemAtIndex:i] setEnabled:enabled];
  }
}
